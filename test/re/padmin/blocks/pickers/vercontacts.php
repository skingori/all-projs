...
/**
*Lista de contactos de clientes con formulario de busqueda.
*Returns html into var html_out
*@package blocks_admin
**/
$PHP_SELF = $_SERVER['PHP_SELF'];
if (preg_match("/vercontacts.php/i",$PHP_SELF)) {
...
$form->add_hidden("data");
 ...
 if (isset($keywords)) {parse_str(preg_replace("/,/","&",$keywords),$fields);
  ...
  $form->fields["data"]->value = $this->txt_encrypt("pg=vercontacts&view=$view&pk=$pk&pos=$pos&from=0");
 } else {
   reset($form->fields);
   $keywords="";
...
if (isset($fields)) {
  ...
  if ($account->ver_accs($fields,$id_org,$id_position, $view,$from,20,$order_by, FALSE, TRUE)){
  	 $fields=$account->select_array();
     $list = new pklist;
     
     $this->html_out .= $list->print_list($fields, $from,20,"pg=vercontacts&view=$view","keywords=$keywords",$pk, $pos, $account->found_rows());
