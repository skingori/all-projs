...
// End of modification Disable/Enable Registration Page, by Masino Sinaga, May 14, 2012
	...
	//
	function Page_Main() {
		global $conn, $Security, $Language, $gsFormError, $objForm, $UserProfile;  // $UserProfile added by Masino Sinaga, November 6, 2011;
		global $Breadcrumb;
	...
	function CheckPasswordStrength() {
		...
		if (MS_PASSWORD_MUST_COMPLY_WITH_MIN_LENGTH==TRUE) {
			if( strlen($this->Password->FormValue) < MS_PASSWORD_MINIMUM_LENGTH ) {
		...
		if (MS_PASSWORD_MUST_COMPLY_WITH_MAX_LENGTH==TRUE) {
			if( strlen($this->Password->FormValue) > MS_PASSWORD_MAXIMUM_LENGTH ) {
		...
		if (MS_PASSWORD_MUST_INCLUDE_AT_LEAST_ONE_NUMBER==TRUE) {
			if( !preg_match("#[0-9]+#", $this->Password->FormValue) ) {
		...
		if (MS_PASSWORD_MUST_INCLUDE_AT_LEAST_ONE_LETTER==TRUE) {
			if( !preg_match("#[a-z]+#", $this->Password->FormValue) ) {
		...
		if (MS_PASSWORD_MUST_INCLUDE_AT_LEAST_ONE_SYMBOL==TRUE) {
			if( !preg_match("#\W+#", $this->Password->FormValue) ) {
...
<input type="hidden" data-field="x_Username" name="x_Username" id="x_Username" value="<?php echo ew_HtmlEncode($users->Username->FormValue) ?>">
<?php } ?>
...
<input type="hidden" data-field="x_First_Name" name="x_First_Name" id="x_First_Name" value="<?php echo ew_HtmlEncode($users->First_Name->FormValue) ?>">
<?php } ?>
...
<input type="hidden" data-field="x_Last_Name" name="x_Last_Name" id="x_Last_Name" value="<?php echo ew_HtmlEncode($users->Last_Name->FormValue) ?>">
<?php } ?>
...
<input type="hidden" data-field="x__Email" name="x__Email" id="x__Email" value="<?php echo ew_HtmlEncode($users->_Email->FormValue) ?>">
<?php } ?>
