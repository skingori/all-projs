...
$view_purchases_details = NULL;

//
// Table class for view_purchases_details
//
class cview_purchases_details extends cTable {
	var $Purchase_ID;
	var $Purchase_Number;
	var $Supplier_Number;
	var $Stock_Item;
	...
	function __construct() {
		...
		$this->TableVar = 'view_purchases_details';
		$this->TableName = 'view_purchases_details';
		$this->TableType = 'VIEW';
...
// End of mofidication Flexibility of Export Records Options, by Masino Sinaga, May 14, 2012
		...
		// Purchase_ID
		$this->Purchase_ID = new cField('view_purchases_details', 'view_purchases_details', 'x_Purchase_ID', 'Purchase_ID', '`Purchase_ID`', '`Purchase_ID`', 3, -1, FALSE, '`Purchase_ID`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->Purchase_ID->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['Purchase_ID'] = &$this->Purchase_ID;

		// Purchase_Number
		$this->Purchase_Number = new cField('view_purchases_details', 'view_purchases_details', 'x_Purchase_Number', 'Purchase_Number', '`Purchase_Number`', '`Purchase_Number`', 200, -1, FALSE, '`Purchase_Number`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['Purchase_Number'] = &$this->Purchase_Number;

		// Supplier_Number
		$this->Supplier_Number = new cField('view_purchases_details', 'view_purchases_details', 'x_Supplier_Number', 'Supplier_Number', '`Supplier_Number`', '`Supplier_Number`', 200, -1, FALSE, '`Supplier_Number`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['Supplier_Number'] = &$this->Supplier_Number;

		// Stock_Item
		$this->Stock_Item = new cField('view_purchases_details', 'view_purchases_details', 'x_Stock_Item', 'Stock_Item', '`Stock_Item`', '`Stock_Item`', 200, -1, FALSE, '`Stock_Item`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['Stock_Item'] = &$this->Stock_Item;

		// Purchasing_Quantity
		$this->Purchasing_Quantity = new cField('view_purchases_details', 'view_purchases_details', 'x_Purchasing_Quantity', 'Purchasing_Quantity', '`Purchasing_Quantity`', '`Purchasing_Quantity`', 5, -1, FALSE, '`Purchasing_Quantity`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['Purchasing_Quantity'] = &$this->Purchasing_Quantity;

		// Purchasing_Price
		$this->Purchasing_Price = new cField('view_purchases_details', 'view_purchases_details', 'x_Purchasing_Price', 'Purchasing_Price', '`Purchasing_Price`', '`Purchasing_Price`', 5, -1, FALSE, '`Purchasing_Price`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['Purchasing_Price'] = &$this->Purchasing_Price;

		// Selling_Price
		$this->Selling_Price = new cField('view_purchases_details', 'view_purchases_details', 'x_Selling_Price', 'Selling_Price', '`Selling_Price`', '`Selling_Price`', 5, -1, FALSE, '`Selling_Price`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['Selling_Price'] = &$this->Selling_Price;

		// Purchasing_Total_Amount
		$this->Purchasing_Total_Amount = new cField('view_purchases_details', 'view_purchases_details', 'x_Purchasing_Total_Amount', 'Purchasing_Total_Amount', '`Purchasing_Total_Amount`', '`Purchasing_Total_Amount`', 5, -1, FALSE, '`Purchasing_Total_Amount`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
	...
	function GetMasterFilter() {
		...
		if ($this->getCurrentMasterTable() == "a_stock_items") {
			if ($this->Stock_Item->getSessionValue() <> "")
				$sMasterFilter .= "`Stock_Number`=" . ew_QuotedValue($this->Stock_Item->getSessionValue(), EW_DATATYPE_STRING);
			else
				return "";
		}
	...
	function GetDetailFilter() {
		...
		if ($this->getCurrentMasterTable() == "a_stock_items") {
			if ($this->Stock_Item->getSessionValue() <> "")
				$sDetailFilter .= "`Stock_Item`=" . ew_QuotedValue($this->Stock_Item->getSessionValue(), EW_DATATYPE_STRING);
			else
				return "";
		}
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`view_purchases_details`";
	}
	...
	function getSqlOrderBy() { // Order By
		return ($this->_SqlOrderBy <> "") ? $this->_SqlOrderBy : "`Purchase_ID` DESC";
	}
	...
	// Update Table
	var $UpdateTable = "`view_purchases_details`";
	...
	function DeleteSQL(&$rs, $where = "") {
		...
		if ($rs) {
			if (array_key_exists('Purchase_ID', $rs))
				ew_AddFilter($where, ew_QuotedName('Purchase_ID') . '=' . ew_QuotedValue($rs['Purchase_ID'], $this->Purchase_ID->FldDataType));
		}
		$filter = $this->CurrentFilter;
		ew_AddFilter($filter, $where);
		if ($filter <> "")
	...
	function KeyFilter() {
		...
		$sKeyFilter = str_replace("@Purchase_ID@", ew_AdjustSql($this->Purchase_ID->CurrentValue), $sKeyFilter); // Replace key value
		return $sKeyFilter;
	}
	...
	function getReturnUrl() {
		...
		if (@$_SESSION[$name] <> "") {
			...
			return "view_purchases_detailslist.php";
		}
	}
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			return $this->KeyUrl("view_purchases_detailsview.php", $this->UrlParm($parm));
		else
			return $this->KeyUrl("view_purchases_detailsview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	}

	// Add URL
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			return "view_purchases_detailsadd.php?" . $this->UrlParm($parm);
		else
			return "view_purchases_detailsadd.php";
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		return $this->KeyUrl("view_purchases_detailsedit.php", $this->UrlParm($parm));
	}
	...
	function GetCopyUrl($parm = "") {
		return $this->KeyUrl("view_purchases_detailsadd.php", $this->UrlParm($parm));
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("view_purchases_detailsdelete.php", $this->UrlParm());
	}
	...
	function GetRecordKeys() {
		...
		if (isset($_POST["key_m"])) {
			...
			$arKeys[] = @$_GET["Purchase_ID"]; // Purchase_ID
		...
		if (preg_match('/^x(\d)*_Stock_Item$/', $id)) {
			...
			if ($rs = ew_LoadRecordset($sSqlWrk)) {
				while ($rs && !$rs->EOF) {
					...
					$ar[] = (!$this->Selling_Price->AutoFillOriginalValue && !is_null($this->Selling_Price->EditValue)) ? $this->Selling_Price->EditValue : $this->Selling_Price->CurrentValue; // http://www.hkvforums.com/viewtopic.php?f=4&t=35232
					$rowcnt += 1;
					$rsarr[] = $ar;
					$rs->MoveNext();
				}
