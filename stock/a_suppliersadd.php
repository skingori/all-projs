...
// End of modification Disable/Enable Registration Page, by Masino Sinaga, May 14, 2012
		...
		switch ($this->CurrentAction) {
			...
			case "A": // Add new record
				...
				if ($this->AddRow($this->OldRecordset)) { // Add successful
					...
					if (ew_GetPageName($sReturnUrl) == "a_suppliersview.php")
	...
	function AddRow($rsold = NULL) {
		...
		if ($this->Supplier_Number->CurrentValue <> "") { // Check field with unique index
			$sFilter = "(Supplier_Number = '" . ew_AdjustSql($this->Supplier_Number->CurrentValue) . "')";
...
// Page object
var a_suppliers_add = new ew_Page("a_suppliers_add");
a_suppliers_add.PageID = "add"; // Page ID
var EW_PAGE_ID = a_suppliers_add.PageID; // For backward compatibility

// Form object
var fa_suppliersadd = new ew_Form("fa_suppliersadd");
...
?>
<form name="fa_suppliersadd" id="fa_suppliersadd" class="form-horizontal ewForm ewAddForm" action="<?php echo ew_CurrentPage() ?>" method="post">
...
<input type="hidden" data-field="x_Date_Added" name="x_Date_Added" id="x_Date_Added" value="<?php echo ew_HtmlEncode($a_suppliers->Date_Added->CurrentValue) ?>">
</span>
<span id="el_a_suppliers_Added_By">
<input type="hidden" data-field="x_Added_By" name="x_Added_By" id="x_Added_By" value="<?php echo ew_HtmlEncode($a_suppliers->Added_By->CurrentValue) ?>">
</span>
</div>
<?php
...
<script type="text/javascript">
$('#btnAction').attr('onclick', 'return alertifyAdd(this)'); function alertifyAdd(obj) { <?php global $Language; ?> if (fa_suppliersadd.Validate() == true ) { alertify.set({buttonFocus: 'cancel'});alertify.confirm("<?php echo $Language->Phrase('AlertifyAddConfirm'); ?>", function (e) { if (e) { $(window).unbind('beforeunload'); alertify.success("<?php echo $Language->Phrase('AlertifyAdd'); ?>"); $("#fa_suppliersadd").submit(); } else { alertify.error("<?php echo $Language->Phrase('AlertifyCancel'); ?>"); } }, "<?php echo $Language->Phrase('AlertifyConfirm'); ?>"); } return false; }
</script>
<?php } ?>
