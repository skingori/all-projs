...
// End of modification Disable/Enable Registration Page, by Masino Sinaga, May 14, 2012
		...
		switch ($this->CurrentAction) {
			...
			case "A": // Add new record
				...
				if ($this->AddRow($this->OldRecordset)) { // Add successful
					...
					if (ew_GetPageName($sReturnUrl) == "a_payment_transactionsview.php")
	...
	function SetUpMasterParms() {
		...
		if ($bValidMaster) {
			...
			if ($sMasterTblVar <> "view_sales_outstandings") {
				if ($this->Ref_ID->QueryStringValue == "") $this->Ref_ID->setSessionValue("");
			}
			if ($sMasterTblVar <> "view_purchases_outstandings") {
				if ($this->Ref_ID->QueryStringValue == "") $this->Ref_ID->setSessionValue("");
			}
...
// Page object
var a_payment_transactions_add = new ew_Page("a_payment_transactions_add");
a_payment_transactions_add.PageID = "add"; // Page ID
var EW_PAGE_ID = a_payment_transactions_add.PageID; // For backward compatibility

// Form object
var fa_payment_transactionsadd = new ew_Form("fa_payment_transactionsadd");
...
fa_payment_transactionsadd.Lists["x_Customer"] = {"LinkField":"x_Customer_Number","Ajax":null,"AutoFill":false,"DisplayFields":["x_Customer_Name","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
fa_payment_transactionsadd.Lists["x_Supplier"] = {"LinkField":"x_Supplier_Number","Ajax":null,"AutoFill":false,"DisplayFields":["x_Supplier_Name","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
...
?>
<form name="fa_payment_transactionsadd" id="fa_payment_transactionsadd" class="form-horizontal ewForm ewAddForm" action="<?php echo ew_CurrentPage() ?>" method="post">
...
<script type="text/javascript">
fa_payment_transactionsadd.Lists["x_Customer"].Options = <?php echo (is_array($a_payment_transactions->Customer->EditValue)) ? ew_ArrayToJson($a_payment_transactions->Customer->EditValue, 1) : "[]" ?>;
</script>
</span>
...
<script type="text/javascript">
fa_payment_transactionsadd.Lists["x_Supplier"].Options = <?php echo (is_array($a_payment_transactions->Supplier->EditValue)) ? ew_ArrayToJson($a_payment_transactions->Supplier->EditValue, 1) : "[]" ?>;
</script>
</span>
...
<input type="hidden" data-field="x_Date_Added" name="x_Date_Added" id="x_Date_Added" value="<?php echo ew_HtmlEncode($a_payment_transactions->Date_Added->CurrentValue) ?>">
</span>
<span id="el_a_payment_transactions_Added_By">
<input type="hidden" data-field="x_Added_By" name="x_Added_By" id="x_Added_By" value="<?php echo ew_HtmlEncode($a_payment_transactions->Added_By->CurrentValue) ?>">
</span>
</div>
...
<script type="text/javascript">
$('#btnAction').attr('onclick', 'return alertifyAdd(this)'); function alertifyAdd(obj) { <?php global $Language; ?> if (fa_payment_transactionsadd.Validate() == true ) { alertify.set({buttonFocus: 'cancel'});alertify.confirm("<?php echo $Language->Phrase('AlertifyAddConfirm'); ?>", function (e) { if (e) { $(window).unbind('beforeunload'); alertify.success("<?php echo $Language->Phrase('AlertifyAdd'); ?>"); $("#fa_payment_transactionsadd").submit(); } else { alertify.error("<?php echo $Language->Phrase('AlertifyCancel'); ?>"); } }, "<?php echo $Language->Phrase('AlertifyConfirm'); ?>"); } return false; }
</script>
<?php } ?>
