...
$help_categories_delete = NULL; // Initialize page object first

class chelp_categories_delete extends chelp_categories {
	...
	// Page object name
	var $PageObjName = 'help_categories_delete';
...
// End of modification Disable/Enable Registration Page, by Masino Sinaga, May 14, 2012
	...
	function Page_Main() {
		...
		if ($sFilter == "")
			$this->Page_Terminate("help_categorieslist.php"); // Prevent SQL injection, return to list

		// Set up filter (SQL WHHERE clause) and get return SQL
		// SQL constructor in help_categories class, help_categoriesinfo.php
...
// Create page object
if (!isset($help_categories_delete)) $help_categories_delete = new chelp_categories_delete();

// Page init
$help_categories_delete->Page_Init();

// Page main
$help_categories_delete->Page_Main();
...
$help_categories_delete->Page_Render();
?>
<?php include_once "header.php" ?>
<script type="text/javascript">

// Page object
var help_categories_delete = new ew_Page("help_categories_delete");
help_categories_delete.PageID = "delete"; // Page ID
var EW_PAGE_ID = help_categories_delete.PageID; // For backward compatibility

// Form object
var fhelp_categoriesdelete = new ew_Form("fhelp_categoriesdelete");

// Form_CustomValidate event
fhelp_categoriesdelete.Form_CustomValidate = 
...
fhelp_categoriesdelete.ValidateRequired = true;
<?php } else { ?>
fhelp_categoriesdelete.ValidateRequired = false; 
<?php } ?>
...
if ($help_categories_delete->Recordset = $help_categories_delete->LoadRecordset())
	$help_categories_deleteTotalRecs = $help_categories_delete->Recordset->RecordCount(); // Get record count
if ($help_categories_deleteTotalRecs <= 0) { // No record found, exit
	if ($help_categories_delete->Recordset)
		$help_categories_delete->Recordset->Close();
	$help_categories_delete->Page_Terminate("help_categorieslist.php"); // Return to list
}
?>
<div class="ewToolbar">
...
<?php $help_categories_delete->ShowPageHeader(); ?>
<?php
$help_categories_delete->ShowMessage();
?>
<form name="fhelp_categoriesdelete" id="fhelp_categoriesdelete" class="form-inline ewForm ewDeleteForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($help_categories_delete->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $help_categories_delete->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="help_categories">
<input type="hidden" name="a_delete" id="a_delete" value="D">
<?php foreach ($help_categories_delete->RecKeys as $key) { ?>
...
<?php if ($help_categories->Category_ID->Visible) { // Category_ID ?>
		<th><span id="elh_help_categories_Category_ID" class="help_categories_Category_ID"><?php echo $help_categories->Category_ID->FldCaption() ?></span></th>
<?php } ?>
<?php if ($help_categories->Category_Description->Visible) { // Category_Description ?>
		<th><span id="elh_help_categories_Category_Description" class="help_categories_Category_Description"><?php echo $help_categories->Category_Description->FldCaption() ?></span></th>
<?php } ?>
	</tr>
	</thead>
	<tbody>
<?php
$help_categories_delete->RecCnt = 0;
$i = 0;
while (!$help_categories_delete->Recordset->EOF) {
	$help_categories_delete->RecCnt++;
	$help_categories_delete->RowCnt++;

	// Set row properties
	$help_categories->ResetAttrs();
	$help_categories->RowType = EW_ROWTYPE_VIEW; // View

	// Get the field contents
	$help_categories_delete->LoadRowValues($help_categories_delete->Recordset);

	// Render row
	$help_categories_delete->RenderRow();
?>
	<tr<?php echo $help_categories->RowAttributes() ?>>
<?php if ($help_categories->Category_ID->Visible) { // Category_ID ?>
		<td<?php echo $help_categories->Category_ID->CellAttributes() ?>>
<span id="el<?php echo $help_categories_delete->RowCnt ?>_help_categories_Category_ID" class="form-group help_categories_Category_ID">
...
<span id="el<?php echo $help_categories_delete->RowCnt ?>_help_categories_Category_Description" class="form-group help_categories_Category_Description">
...
<?php
	$help_categories_delete->Recordset->MoveNext();
}
$help_categories_delete->Recordset->Close();
?>
</tbody>
...
<script type="text/javascript">
fhelp_categoriesdelete.Init();
</script>
<?php
$help_categories_delete->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<script type="text/javascript">
$('#btnAction').attr('onclick', 'return alertifyDelete(this)'); function alertifyDelete(obj) { <?php global $Language; ?> if (fhelp_categoriesdelete.Validate() == true ) { alertify.set({buttonFocus: 'cancel'});alertify.confirm("<?php echo  $Language->Phrase('AlertifyDeleteConfirm'); ?>", function (e) { if (e) {	$(window).unbind('beforeunload'); alertify.success("<?php echo $Language->Phrase('AlertifyDelete'); ?>"); $("#fhelp_categoriesdelete").submit(); } else { alertify.error("<?php echo $Language->Phrase('AlertifyCancel'); ?>"); } }, "<?php echo $Language->Phrase('AlertifyConfirm'); ?>"); } return false; }
</script>
<?php } ?>
<?php include_once "footer.php" ?>
<?php
$help_categories_delete->Page_Terminate();
?>
