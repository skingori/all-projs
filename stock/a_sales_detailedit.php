...
// Page object
var a_sales_detail_edit = new ew_Page("a_sales_detail_edit");
a_sales_detail_edit.PageID = "edit"; // Page ID
var EW_PAGE_ID = a_sales_detail_edit.PageID; // For backward compatibility

// Form object
var fa_sales_detailedit = new ew_Form("fa_sales_detailedit");
...
fa_sales_detailedit.Lists["x_Supplier_Number"] = {"LinkField":"x_Supplier_Number","Ajax":null,"AutoFill":false,"DisplayFields":["x_Supplier_Name","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
fa_sales_detailedit.Lists["x_Stock_Item"] = {"LinkField":"x_Stock_Number","Ajax":null,"AutoFill":true,"DisplayFields":["x_Stock_Name","","",""],"ParentFields":["x_Supplier_Number"],"FilterFields":["x_Supplier_Number"],"Options":[]};
...
<form name="fa_sales_detailedit" id="fa_sales_detailedit" class="form-horizontal ewForm ewEditForm" action="<?php echo ew_CurrentPage() ?>" method="post">
...
<input type="hidden" data-field="x_Sales_ID" name="x_Sales_ID" id="x_Sales_ID" value="<?php echo ew_HtmlEncode($a_sales_detail->Sales_ID->CurrentValue) ?>">
...
<script type="text/javascript">
fa_sales_detailedit.Lists["x_Supplier_Number"].Options = <?php echo (is_array($a_sales_detail->Supplier_Number->EditValue)) ? ew_ArrayToJson($a_sales_detail->Supplier_Number->EditValue, 1) : "[]" ?>;
</script>
</span>
...
<script type="text/javascript">
fa_sales_detailedit.Lists["x_Stock_Item"].Options = <?php echo (is_array($a_sales_detail->Stock_Item->EditValue)) ? ew_ArrayToJson($a_sales_detail->Stock_Item->EditValue, 1) : "[]" ?>;
</script>
...
<script type="text/javascript">
$('#btnAction').attr('onclick', 'return alertifyEdit(this)'); function alertifyEdit(obj) { <?php global $Language; ?> if (fa_sales_detailedit.Validate() == true ) { alertify.set({buttonFocus: 'cancel'});alertify.confirm("<?php echo $Language->Phrase('AlertifyEditConfirm'); ?>", function (e) {	if (e) { $(window).unbind('beforeunload'); alertify.success("<?php echo $Language->Phrase('AlertifyEdit'); ?>"); $("#fa_sales_detailedit").submit(); } else {	alertify.error("<?php echo $Language->Phrase('AlertifyCancel'); ?>"); }	}, "<?php echo $Language->Phrase('AlertifyConfirm'); ?>");	} return false; }
</script>
<?php } ?>
