...
$view_purchases_details_list = NULL; // Initialize page object first

class cview_purchases_details_list extends cview_purchases_details {
	...
	// Page object name
	var $PageObjName = 'view_purchases_details_list';

	// Grid form hidden field names
	var $FormName = 'fview_purchases_detailslist';
	...
	function __construct() {
		...
		$this->AddUrl = "view_purchases_detailsadd.php";
		$this->InlineAddUrl = $this->PageUrl() . "a=add";
		$this->GridAddUrl = $this->PageUrl() . "a=gridadd";
		$this->GridEditUrl = $this->PageUrl() . "a=gridedit";
		$this->MultiDeleteUrl = "view_purchases_detailsdelete.php";
		$this->MultiUpdateUrl = "view_purchases_detailsupdate.php";
...
// End of modification Customize Navigation/Pager Panel, by Masino Sinaga, May 2, 2012
	...
	function AdvancedSearchWhere($Default = FALSE) {
		...
		$this->BuildSearchSql($sWhere, $this->Purchasing_Quantity, $Default, FALSE); // Purchasing_Quantity
		...
		$this->BuildSearchSql($sWhere, $this->Purchasing_Total_Amount, $Default, FALSE); // Purchasing_Total_Amount
		...
		if (!$Default && $this->Command == "search") {
			...
			$this->Purchasing_Quantity->AdvancedSearch->Save(); // Purchasing_Quantity
			$this->Purchasing_Price->AdvancedSearch->Save(); // Purchasing_Price
			$this->Selling_Price->AdvancedSearch->Save(); // Selling_Price
			$this->Purchasing_Total_Amount->AdvancedSearch->Save(); // Purchasing_Total_Amount
		}
		return $sWhere;
	}
	...
	function CheckSearchParms() {
		...
		if ($this->Stock_Item->AdvancedSearch->IssetSession())
			return TRUE;
		if ($this->Purchasing_Quantity->AdvancedSearch->IssetSession())
			return TRUE;
		if ($this->Purchasing_Price->AdvancedSearch->IssetSession())
			return TRUE;
		if ($this->Selling_Price->AdvancedSearch->IssetSession())
			return TRUE;
		if ($this->Purchasing_Total_Amount->AdvancedSearch->IssetSession())
			return TRUE;
		return FALSE;
	}
	...
	function ResetAdvancedSearchParms() {
		...
		$this->Purchasing_Quantity->AdvancedSearch->UnsetSession();
		$this->Purchasing_Price->AdvancedSearch->UnsetSession();
		$this->Selling_Price->AdvancedSearch->UnsetSession();
		$this->Purchasing_Total_Amount->AdvancedSearch->UnsetSession();
	}
	...
	function RenderListOptions() {
		...
		$oListOpt->Body = "<input type=\"checkbox\" name=\"key_m[]\" value=\"" . ew_HtmlEncode($this->Purchase_ID->CurrentValue) . "\" onclick='ew_ClickMultiCheckbox(event, this);'>";
	...
	function RenderOtherOptions() {
		...
		$options = &$this->OtherOptions;
			...
			foreach ($this->CustomActions as $action => $name) {
				...
				$item->Body = "<a class=\"ewAction ewCustomAction\" href=\"\" onclick=\"ew_SubmitSelected(document.fview_purchases_detailslist, '" . ew_CurrentUrl() . "', null, '" . $action . "');return false;\">" . $name . "</a>";
			}
	...
	function SetupSearchOptions() {
		...
		if ($this->CurrentAction == "edit" || $this->CurrentAction == "copy") {
			...
			$item->Body = "<button type=\"button\" class=\"btn btn-default ewSearchToggle" . $SearchToggleClass . "\" title=\"" . $Language->Phrase("SearchPanel") . "\" data-caption=\"" . $Language->Phrase("SearchPanel") . "\" data-toggle=\"button\" data-form=\"fview_purchases_detailslistsrch\">" . $Language->Phrase("SearchBtn") . "</button>";
			$item->Visible = TRUE;
		}
		...
		$item->Body = "<a class=\"btn btn-default ewAdvancedSearch\" title=\"" . $Language->Phrase("AdvancedSearch") . "\" data-caption=\"" . $Language->Phrase("AdvancedSearch") . "\" href=\"view_purchases_detailssrch.php\">" . $Language->Phrase("AdvancedSearchBtn") . "</a>";
		$item->Visible = TRUE;
	...
	function LoadSearchValues() {
		...
		// Purchasing_Quantity
		$this->Purchasing_Quantity->AdvancedSearch->SearchValue = ew_StripSlashes(@$_GET["x_Purchasing_Quantity"]);
		if ($this->Purchasing_Quantity->AdvancedSearch->SearchValue <> "") $this->Command = "search";
		$this->Purchasing_Quantity->AdvancedSearch->SearchOperator = @$_GET["z_Purchasing_Quantity"];
		$this->Purchasing_Quantity->AdvancedSearch->SearchCondition = @$_GET["v_Purchasing_Quantity"];
		$this->Purchasing_Quantity->AdvancedSearch->SearchValue2 = ew_StripSlashes(@$_GET["y_Purchasing_Quantity"]);
		if ($this->Purchasing_Quantity->AdvancedSearch->SearchValue2 <> "") $this->Command = "search";
		$this->Purchasing_Quantity->AdvancedSearch->SearchOperator2 = @$_GET["w_Purchasing_Quantity"];
		...
		$this->Purchasing_Total_Amount->AdvancedSearch->SearchValue = ew_StripSlashes(@$_GET["x_Purchasing_Total_Amount"]);
		if ($this->Purchasing_Total_Amount->AdvancedSearch->SearchValue <> "") $this->Command = "search";
		$this->Purchasing_Total_Amount->AdvancedSearch->SearchOperator = @$_GET["z_Purchasing_Total_Amount"];
		$this->Purchasing_Total_Amount->AdvancedSearch->SearchCondition = @$_GET["v_Purchasing_Total_Amount"];
		$this->Purchasing_Total_Amount->AdvancedSearch->SearchValue2 = ew_StripSlashes(@$_GET["y_Purchasing_Total_Amount"]);
		if ($this->Purchasing_Total_Amount->AdvancedSearch->SearchValue2 <> "") $this->Command = "search";
		$this->Purchasing_Total_Amount->AdvancedSearch->SearchOperator2 = @$_GET["w_Purchasing_Total_Amount"];
	}
...
// Begin of modification Permission Access for Export To Feature, by Masino Sinaga, May 5, 2012
        global $Language, $Security, $view_purchases_details; // <-- Added $Security variable by Masino Sinaga
        ...
        if ($Security->CanExportToPrint() || $Security->IsAdmin() ) {
			...
			if (MS_EXPORT_RECORD_OPTIONS=="selectedrecords") {
				$item->Body = "<a class=\"ewExportLink ewPrint\" title=\"" . ew_HtmlEncode($Language->Phrase("PrinterFriendlyText")) . "\" data-caption=\"" . ew_HtmlEncode($Language->Phrase("PrinterFriendlyText")) . "\" onclick=\"ew_Export(document.fview_purchases_detailslist,'" . ew_CurrentPage() . "','print',false);\">" . $Language->Phrase("PrinterFriendly") . "</a>";
			} else {
				$item->Body = "<a href=\"" . $this->ExportPrintUrl . "\" class=\"ewExportLink ewPrint\" title=\"" . ew_HtmlEncode($Language->Phrase("PrinterFriendlyText")) . "\"  data-caption=\"" . ew_HtmlEncode($Language->Phrase("PrinterFriendlyText")) . "\">" . $Language->Phrase("PrinterFriendly") . "</a>";
			}
        ...
        if ($Security->CanExportToExcel() || $Security->IsAdmin() ) {
			...
			if (MS_EXPORT_RECORD_OPTIONS=="selectedrecords") {
				$item->Body = "<a class=\"ewExportLink ewExcel\" title=\"" . ew_HtmlEncode($Language->Phrase("ExportToExcelText")) . "\" data-caption=\"" . ew_HtmlEncode($Language->Phrase("ExportToExcelText")) . "\" onclick=\"ew_Export(document.fview_purchases_detailslist,'" . ew_CurrentPage() . "','excel',false);\">" . $Language->Phrase("ExportToExcel") . "</a>";
			} else {
				$item->Body = "<a href=\"" . $this->ExportExcelUrl . "\" class=\"ewExportLink ewExcel\" title=\"" . ew_HtmlEncode($Language->Phrase("ExportToExcelText")) . "\"  data-caption=\"" . ew_HtmlEncode($Language->Phrase("ExportToExcelText")) . "\">" . $Language->Phrase("ExportToExcel") . "</a>";
			}
        ...
        if ($Security->CanExportToWord() || $Security->IsAdmin() ) {
			...
			if (MS_EXPORT_RECORD_OPTIONS=="selectedrecords") {
				$item->Body = "<a class=\"ewExportLink ewWord\" title=\"" . ew_HtmlEncode($Language->Phrase("ExportToWordText")) . "\" data-caption=\"" . ew_HtmlEncode($Language->Phrase("ExportToWordText")) . "\" onclick=\"ew_Export(document.fview_purchases_detailslist,'" . ew_CurrentPage() . "','word',false);\">" . $Language->Phrase("ExportToWord") . "</a>";
			} else {
				$item->Body = "<a href=\"" . $this->ExportWordUrl . "\" class=\"ewExportLink ewWord\" title=\"" . ew_HtmlEncode($Language->Phrase("ExportToWordText")) . "\"  data-caption=\"" . ew_HtmlEncode($Language->Phrase("ExportToWordText")) . "\">" . $Language->Phrase("ExportToWord") . "</a>";
			}
        ...
        if ($Security->CanExportToHTML() || $Security->IsAdmin() ) {
			...
			if (MS_EXPORT_RECORD_OPTIONS=="selectedrecords") {
				$item->Body = "<a class=\"ewExportLink ewHtml\" title=\"" . ew_HtmlEncode($Language->Phrase("ExportToHtmlText")) . "\" data-caption=\"" . ew_HtmlEncode($Language->Phrase("ExportToHtmlText")) . "\" onclick=\"ew_Export(document.fview_purchases_detailslist,'" . ew_CurrentPage() . "','html',false);\">" . $Language->Phrase("ExportToHtml") . "</a>";
			} else {
				$item->Body = "<a href=\"" . $this->ExportHtmlUrl . "\" class=\"ewExportLink ewHtml\" title=\"" . ew_HtmlEncode($Language->Phrase("ExportToHtmlText")) . "\"  data-caption=\"" . ew_HtmlEncode($Language->Phrase("ExportToHtmlText")) . "\">" . $Language->Phrase("ExportToHTML") . "</a>";
			}
        ...
        if ($Security->CanExportToXML() || $Security->IsAdmin() ) {
			...
			if (MS_EXPORT_RECORD_OPTIONS=="selectedrecords") {
				$item->Body = "<a class=\"ewExportLink ewXml\" title=\"" . ew_HtmlEncode($Language->Phrase("ExportToXmlText")) . "\" data-caption=\"" . ew_HtmlEncode($Language->Phrase("ExportToXmlText")) . "\" onclick=\"ew_Export(document.fview_purchases_detailslist,'" . ew_CurrentPage() . "','xml',false);\">" . $Language->Phrase("ExportToXml") . "</a>";
			} else {
				$item->Body = "<a href=\"" . $this->ExportXmlUrl . "\" class=\"ewExportLink ewXml\" title=\"" . ew_HtmlEncode($Language->Phrase("ExportToXmlText")) . "\" data-caption=\"" . ew_HtmlEncode($Language->Phrase("ExportToXmlText")) . "\">" . $Language->Phrase("ExportToXML") . "</a>";
			}
        ...
        if ($Security->CanExportToCSV() || $Security->IsAdmin() ) {
			...
			if (MS_EXPORT_RECORD_OPTIONS=="selectedrecords") {
				$item->Body = "<a class=\"ewExportLink ewCsv\" title=\"" . ew_HtmlEncode($Language->Phrase("ExportToCsvText")) . "\" data-caption=\"" . ew_HtmlEncode($Language->Phrase("ExportToCsvText")) . "\" onclick=\"ew_Export(document.fview_purchases_detailslist,'" . ew_CurrentPage() . "','csv',false);\">" . $Language->Phrase("ExportToCsv") . "</a>";
			} else {
				$item->Body = "<a href=\"" . $this->ExportCsvUrl . "\" class=\"ewExportLink ewCsv\" title=\"" . ew_HtmlEncode($Language->Phrase("ExportToCsvText")) . "\"  data-caption=\"" . ew_HtmlEncode($Language->Phrase("ExportToCsvText")) . "\">" . $Language->Phrase("ExportToCSV") . "</a>";
			}
        ...
        if ($Security->CanExportToPDF() || $Security->IsAdmin() ) {
			...
			if (MS_EXPORT_RECORD_OPTIONS=="selectedrecords") {
				$item->Body = "<a class=\"ewExportLink ewPdf\" title=\"" . ew_HtmlEncode($Language->Phrase("ExportToPDFText")) . "\" data-caption=\"" . ew_HtmlEncode($Language->Phrase("ExportToPDFText")) . "\" onclick=\"ew_Export(document.fview_purchases_detailslist,'" . ew_CurrentPage() . "','pdf',false);\">" . $Language->Phrase("ExportToPDF") . "</a>";
			} else {
				$item->Body = "<a href=\"" . $this->ExportPdfUrl . "\" class=\"ewExportLink ewPdf\" title=\"" . ew_HtmlEncode($Language->Phrase("ExportToPDFText")) . "\"  data-caption=\"" . ew_HtmlEncode($Language->Phrase("ExportToPDFText")) . "\">" . $Language->Phrase("ExportToPDF") . "</a>";
			}
		...
		if ($Security->CanExportToEmail() || $Security->IsAdmin() ) {
			...
			// $item->Body = "<button id=\"emf_view_purchases_details\" class=\"ewExportLink ewEmail\" title=\"" . $Language->Phrase("ExportToEmailText") . "\" data-caption=\"" . $Language->Phrase("ExportToEmailText") . "\" onclick=\"ew_EmailDialogShow({lnk:'emf_view_purchases_details',hdr:ewLanguage.Phrase('ExportToEmailText'),f:document.fview_purchases_detailslist,sel:false" . $url . "});\">" . $Language->Phrase("ExportToEmail") . "</button>";
			// Begin of mofidication Flexibility of Export Records Options, by Masino Sinaga, May 14, 2012

		if (MS_EXPORT_RECORD_OPTIONS=="selectedrecords") {
			$item->Body = "<a id=\"emf_view_purchases_details\" href=\"javascript:void(0);\" class=\"ewExportLink ewEmail\" title=\"" . $Language->Phrase("ExportToEmailText") . "\"  data-caption=\"" . $Language->Phrase("ExportToEmailText") . "\" onclick=\"ew_EmailDialogShow({lnk:'emf_view_purchases_details',hdr:ewLanguage.Phrase('ExportToEmailText'),f:document.fview_purchases_detailslist,sel:true});\">" . $Language->Phrase("ExportToEmail") . "</a>";
		} else {
	...
	function ExportQueryString() {
		...
		$this->AddSearchQueryString($sQry, $this->Purchasing_Quantity); // Purchasing_Quantity
		...
		$this->AddSearchQueryString($sQry, $this->Purchasing_Total_Amount); // Purchasing_Total_Amount
	...
	function SetUpMasterParms() {
		...
		if ($bValidMaster) {
			...
			if ($sMasterTblVar <> "a_stock_items") {
				if ($this->Stock_Item->QueryStringValue == "") $this->Stock_Item->setSessionValue("");
			}
		}
...
// Create page object
if (!isset($view_purchases_details_list)) $view_purchases_details_list = new cview_purchases_details_list();

// Page init
$view_purchases_details_list->Page_Init();

// Page main
$view_purchases_details_list->Page_Main();
...
// Page object
var view_purchases_details_list = new ew_Page("view_purchases_details_list");
view_purchases_details_list.PageID = "list"; // Page ID
var EW_PAGE_ID = view_purchases_details_list.PageID; // For backward compatibility

// Form object
var fview_purchases_detailslist = new ew_Form("fview_purchases_detailslist");
fview_purchases_detailslist.FormKeyCountName = '<?php echo $view_purchases_details_list->FormKeyCountName ?>';

// Form_CustomValidate event
fview_purchases_detailslist.Form_CustomValidate = 
...
fview_purchases_detailslist.ValidateRequired = true;
<?php } else { ?>
fview_purchases_detailslist.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
fview_purchases_detailslist.Lists["x_Supplier_Number"] = {"LinkField":"x_Supplier_Number","Ajax":null,"AutoFill":false,"DisplayFields":["x_Supplier_Name","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
fview_purchases_detailslist.Lists["x_Stock_Item"] = {"LinkField":"x_Stock_Number","Ajax":null,"AutoFill":false,"DisplayFields":["x_Stock_Name","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};

// Form object for search
var fview_purchases_detailslistsrch = new ew_Form("fview_purchases_detailslistsrch");

// Init search panel as collapsed
<?php if (MS_USE_TABLE_SETTING_FOR_SEARCH_PANEL_COLLAPSED) { ?>
if (fview_purchases_detailslistsrch) fview_purchases_detailslistsrch.InitSearchPanel = true;
<?php } else { ?>
...
if (fview_purchases_detailslistsrch) fview_purchases_detailslistsrch.InitSearchPanel = false;
<?php } ?>
<?php } ?>
</script>
...
<?php } ?>
<?php } ?>
<?php if ($view_purchases_details_list->TotalRecs > 0 && $view_purchases_details->getCurrentMasterTable() == "" && $view_purchases_details_list->ExportOptions->Visible()) { ?>
<?php $view_purchases_details_list->ExportOptions->Render("body") ?>
<?php } ?>
<?php if (($view_purchases_details->Export == "") || (EW_EXPORT_MASTER_RECORD && $view_purchases_details->Export == "print")) { ?>
<?php
$gsMasterReturnUrl = "a_stock_itemslist.php";
if ($view_purchases_details_list->DbMasterFilter <> "" && $view_purchases_details->getCurrentMasterTable() == "a_stock_items") {
	if ($view_purchases_details_list->MasterRecordExists) {
		if ($view_purchases_details->getCurrentMasterTable() == $view_purchases_details->TableVar) $gsMasterReturnUrl .= "?" . EW_TABLE_SHOW_MASTER . "=";
?>
<?php if ($view_purchases_details_list->ExportOptions->Visible()) { ?>
<?php $view_purchases_details_list->ExportOptions->Render("body") ?> 
<?php } ?>
...
<?php
	...
	if ($bSelectLimit) { // begin of v11.0.4
		if ($view_purchases_details_list->TotalRecs <= 0)
			$view_purchases_details_list->TotalRecs = $view_purchases_details->SelectRecordCount();
	} else {
		if (!$view_purchases_details_list->Recordset && ($view_purchases_details_list->Recordset = $view_purchases_details_list->LoadRecordset()))
			$view_purchases_details_list->TotalRecs = $view_purchases_details_list->Recordset->RecordCount();
	} // end of v11.0.4
	$view_purchases_details_list->StartRec = 1;

// Begin of mofidication Flexibility of Export Records Options, by Masino Sinaga, May 14, 2012     
    if ($view_purchases_details_list->DisplayRecs <= 0 || ($view_purchases_details->Export <> "" && $view_purchases_details->ExportAll=="allpages")) // Display all records
        $view_purchases_details_list->DisplayRecs = $view_purchases_details_list->TotalRecs;
    if (!($view_purchases_details->Export <> "" && $view_purchases_details->ExportAll=="allpages"))
        $view_purchases_details_list->SetUpStartRec(); // Set up start record position

// End of mofidication Flexibility of Export Records Options, by Masino Sinaga, May 14, 2012
	if ($bSelectLimit)
		$view_purchases_details_list->Recordset = $view_purchases_details_list->LoadRecordset($view_purchases_details_list->StartRec-1, $view_purchases_details_list->DisplayRecs);

	// Set no record found message
	if ($view_purchases_details->CurrentAction == "" && $view_purchases_details_list->TotalRecs == 0) {
		if (!$Security->CanList())
			$view_purchases_details_list->setWarningMessage($Language->Phrase("NoPermission"));
		if ($view_purchases_details_list->SearchWhere == "0=101")
			$view_purchases_details_list->setWarningMessage($Language->Phrase("EnterSearchCriteria"));
		else
			$view_purchases_details_list->setWarningMessage($Language->Phrase("NoRecord"));
	}
$view_purchases_details_list->RenderOtherOptions();
?>
<?php if ($Security->CanSearch()) { ?>
<?php if ($view_purchases_details->Export == "" && $view_purchases_details->CurrentAction == "") { ?>
<form name="fview_purchases_detailslistsrch" id="fview_purchases_detailslistsrch" class="form-inline ewForm" action="<?php echo ew_CurrentPage() ?>">
<?php $SearchPanelClass = ($view_purchases_details_list->SearchWhere <> "") ? " in" : " in"; ?>
<div id="fview_purchases_detailslistsrch_SearchPanel" class="ewSearchPanel collapse<?php echo $SearchPanelClass ?>">
<input type="hidden" name="cmd" value="search">
<input type="hidden" name="t" value="view_purchases_details">
	<div class="ewBasicSearch">
<div id="xsr_1" class="ewRow">
	<div class="ewQuickSearch input-group">
	<input type="text" name="<?php echo EW_TABLE_BASIC_SEARCH ?>" id="<?php echo EW_TABLE_BASIC_SEARCH ?>" class="form-control" value="<?php echo ew_HtmlEncode($view_purchases_details_list->BasicSearch->getKeyword()) ?>" placeholder="<?php echo ew_HtmlEncode($Language->Phrase("Search")) ?>">
	<input type="hidden" name="<?php echo EW_TABLE_BASIC_SEARCH_TYPE ?>" id="<?php echo EW_TABLE_BASIC_SEARCH_TYPE ?>" value="<?php echo ew_HtmlEncode($view_purchases_details_list->BasicSearch->getType()) ?>">
	<div class="input-group-btn">
		<button type="button" data-toggle="dropdown" class="btn btn-default"><span id="searchtype"><?php echo $view_purchases_details_list->BasicSearch->getTypeNameShort() ?></span><span class="caret"></span></button>
		<ul class="dropdown-menu pull-right" role="menu">
			<li<?php if ($view_purchases_details_list->BasicSearch->getType() == "") echo " class=\"active\""; ?>><a href="javascript:void(0);" onclick="ew_SetSearchType(this)"><?php echo $Language->Phrase("QuickSearchAuto") ?></a></li>
			<li<?php if ($view_purchases_details_list->BasicSearch->getType() == "=") echo " class=\"active\""; ?>><a href="javascript:void(0);" onclick="ew_SetSearchType(this,'=')"><?php echo $Language->Phrase("QuickSearchExact") ?></a></li>
			<li<?php if ($view_purchases_details_list->BasicSearch->getType() == "AND") echo " class=\"active\""; ?>><a href="javascript:void(0);" onclick="ew_SetSearchType(this,'AND')"><?php echo $Language->Phrase("QuickSearchAll") ?></a></li>
			<li<?php if ($view_purchases_details_list->BasicSearch->getType() == "OR") echo " class=\"active\""; ?>><a href="javascript:void(0);" onclick="ew_SetSearchType(this,'OR')"><?php echo $Language->Phrase("QuickSearchAny") ?></a></li>
		</ul>
...
<?php if ($view_purchases_details_list->TotalRecs == 0) { ?>
<div class="ewGrid">
<div class="ewGridUpperPanel" style="height:40px;">
<?php if ($view_purchases_details_list->TotalRecs == 0 && $view_purchases_details->CurrentAction == "") { // Show other options ?>
<div class="ewListOtherOptions">
<?php
	foreach ($view_purchases_details_list->OtherOptions as &$option) {
		$option->ButtonClass = "";
		$option->Render("body", "");
	}
?>
...
<div id="gmp_view_purchases_details_empty_table" class="<?php if (ew_IsResponsiveLayout()) { echo "table-responsive "; } ?>ewGridMiddlePanel">
<table id="tbl_view_purchases_detailslist" class="table ewTable">
...
<?php if ($view_purchases_details->Purchase_Number->Visible) { // Purchase_Number ?>
	...
	<?php } else { ?>
		<th data-name="Purchase_Number"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $view_purchases_details->SortUrl($view_purchases_details->Purchase_Number) ?>',1);"><div id="elh_view_purchases_details_Purchase_Number" class="view_purchases_details_Purchase_Number">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $view_purchases_details->Purchase_Number->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></span><span class="ewTableHeaderSort"><?php if ($view_purchases_details->Purchase_Number->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($view_purchases_details->Purchase_Number->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
        </div></div></th>
	<?php } ?>
<?php } ?>		
<?php if ($view_purchases_details->Supplier_Number->Visible) { // Supplier_Number ?>
	...
	<?php } else { ?>
		<th data-name="Supplier_Number"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $view_purchases_details->SortUrl($view_purchases_details->Supplier_Number) ?>',1);"><div id="elh_view_purchases_details_Supplier_Number" class="view_purchases_details_Supplier_Number">
...
<?php if ($view_purchases_details->Stock_Item->Visible) { // Stock_Item ?>
	...
	<?php } else { ?>
		<th data-name="Stock_Item"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $view_purchases_details->SortUrl($view_purchases_details->Stock_Item) ?>',1);"><div id="elh_view_purchases_details_Stock_Item" class="view_purchases_details_Stock_Item">
...
<?php if ($view_purchases_details->Purchasing_Quantity->Visible) { // Purchasing_Quantity ?>
	...
	<?php } else { ?>
		<th data-name="Purchasing_Quantity"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $view_purchases_details->SortUrl($view_purchases_details->Purchasing_Quantity) ?>',1);"><div id="elh_view_purchases_details_Purchasing_Quantity" class="view_purchases_details_Purchasing_Quantity">
...
<?php if ($view_purchases_details->Purchasing_Price->Visible) { // Purchasing_Price ?>
	...
	<?php } else { ?>
		<th data-name="Purchasing_Price"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $view_purchases_details->SortUrl($view_purchases_details->Purchasing_Price) ?>',1);"><div id="elh_view_purchases_details_Purchasing_Price" class="view_purchases_details_Purchasing_Price">
...
<?php if ($view_purchases_details->Selling_Price->Visible) { // Selling_Price ?>
	...
	<?php } else { ?>
		<th data-name="Selling_Price"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $view_purchases_details->SortUrl($view_purchases_details->Selling_Price) ?>',1);"><div id="elh_view_purchases_details_Selling_Price" class="view_purchases_details_Selling_Price">
...
<?php if ($view_purchases_details->Purchasing_Total_Amount->Visible) { // Purchasing_Total_Amount ?>
	...
	<?php } else { ?>
		<th data-name="Purchasing_Total_Amount"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $view_purchases_details->SortUrl($view_purchases_details->Purchasing_Total_Amount) ?>',1);"><div id="elh_view_purchases_details_Purchasing_Total_Amount" class="view_purchases_details_Purchasing_Total_Amount">
...
<a id="<?php echo $view_purchases_details_list->PageObjName . "_row_" . $view_purchases_details_list->RowCnt ?>"></a></td>
	<?php } ?>
	<?php if ($view_purchases_details->Supplier_Number->Visible) { // Supplier_Number ?>
...
<tfoot><!-- Table footer -->
	...
	<?php if ($view_purchases_details->Purchase_Number->Visible) { // Purchase_Number ?>
		<td data-name="Purchase_Number"><span id="elf_view_purchases_details_Purchase_Number" class="view_purchases_details_Purchase_Number">
		&nbsp;
		</span></td>
	<?php } ?>
	<?php if ($view_purchases_details->Supplier_Number->Visible) { // Supplier_Number ?>
		<td data-name="Supplier_Number"><span id="elf_view_purchases_details_Supplier_Number" class="view_purchases_details_Supplier_Number">
		&nbsp;
		</span></td>
	<?php } ?>
	<?php if ($view_purchases_details->Stock_Item->Visible) { // Stock_Item ?>
		<td data-name="Stock_Item"><span id="elf_view_purchases_details_Stock_Item" class="view_purchases_details_Stock_Item">
		&nbsp;
		</span></td>
	<?php } ?>
	<?php if ($view_purchases_details->Purchasing_Quantity->Visible) { // Purchasing_Quantity ?>
		<td data-name="Purchasing_Quantity"><span id="elf_view_purchases_details_Purchasing_Quantity" class="view_purchases_details_Purchasing_Quantity">
<span class="ewAggregate"><?php echo $Language->Phrase("TOTAL") ?></span>
<?php echo $view_purchases_details->Purchasing_Quantity->ViewValue ?>
		</span></td>
	<?php } ?>
	<?php if ($view_purchases_details->Purchasing_Price->Visible) { // Purchasing_Price ?>
		<td data-name="Purchasing_Price"><span id="elf_view_purchases_details_Purchasing_Price" class="view_purchases_details_Purchasing_Price">
		&nbsp;
		</span></td>
	<?php } ?>
	<?php if ($view_purchases_details->Selling_Price->Visible) { // Selling_Price ?>
		<td data-name="Selling_Price"><span id="elf_view_purchases_details_Selling_Price" class="view_purchases_details_Selling_Price">
		&nbsp;
		</span></td>
	<?php } ?>
	<?php if ($view_purchases_details->Purchasing_Total_Amount->Visible) { // Purchasing_Total_Amount ?>
		<td data-name="Purchasing_Total_Amount"><span id="elf_view_purchases_details_Purchasing_Total_Amount" class="view_purchases_details_Purchasing_Total_Amount">
<span class="ewAggregate"><?php echo $Language->Phrase("TOTAL") ?></span>
<?php echo $view_purchases_details->Purchasing_Total_Amount->ViewValue ?>
		</span></td>
	<?php } ?>
	</tr>
</tfoot>
...
<?php //////////////////////////// END Empty Table ?>
<?php if ($view_purchases_details_list->TotalRecs > 0 || $view_purchases_details->CurrentAction <> "") { ?>
<div class="ewGrid">
...
<?php if ($view_purchases_details->CurrentAction <> "gridadd" && $view_purchases_details->CurrentAction <> "gridedit") { ?>
...
?>
<option value="<?php echo $thisDisplayRecs; ?>"<?php if ($view_purchases_details_list->DisplayRecs == $thisValue) { ?> selected="selected"<?php } ?>><?php echo $thisDisplayRecs; ?></option>
<?php	} else { ?>
<option value="ALL"<?php if ($view_purchases_details->getRecordsPerPage() == -1) { ?> selected="selected"<?php } ?>><?php echo $Language->Phrase("AllRecords") ?></option>
<?php
		}
	}
?>
...
<?php } ?>
	<?php if (MS_PAGINATION_STYLE==1) { // link ?>
		<?php if (!isset($view_purchases_details_list->Pager)) $view_purchases_details_list->Pager = new cNumericPager($view_purchases_details_list->StartRec, $view_purchases_details_list->DisplayRecs, $view_purchases_details_list->TotalRecs, $view_purchases_details_list->RecRange) ?>
		<?php if ($view_purchases_details_list->Pager->RecordCount > 0) { ?>
				<?php if (($view_purchases_details_list->Pager->PageCount==1) && ($view_purchases_details_list->Pager->CurrentPage == 1) && (MS_SHOW_PAGENUM_IF_REC_NOT_OVER_PAGESIZE==FALSE)  ) { ?>
				<?php } else { // MS_SHOW_PAGENUM_IF_REC_NOT_OVER_PAGESIZE ?>
				<div class="ewPager">
				<div class="ewNumericPage"><ul class="pagination">
					<?php if ($view_purchases_details_list->Pager->FirstButton->Enabled) { ?>
					<?php if ($Language->Phrase("dir") == "rtl") { // begin of rtl ?>
					<li><a href="<?php echo $view_purchases_details_list->PageUrl() ?>start=<?php echo $view_purchases_details_list->Pager->FirstButton->Start ?>"><?php echo $Language->Phrase("PagerLast") ?></a></li>
					<?php } else { // else of rtl ?>
					<li><a href="<?php echo $view_purchases_details_list->PageUrl() ?>start=<?php echo $view_purchases_details_list->Pager->FirstButton->Start ?>"><?php echo $Language->Phrase("PagerFirst") ?></a></li>
					<?php } // end of rtl ?>
					<?php } ?>
					<?php if ($view_purchases_details_list->Pager->PrevButton->Enabled) { ?>
					<?php if ($Language->Phrase("dir") == "rtl") { // begin of rtl ?>
					<li><a href="<?php echo $view_purchases_details_list->PageUrl() ?>start=<?php echo $view_purchases_details_list->Pager->PrevButton->Start ?>"><?php echo $Language->Phrase("PagerNext") ?></a></li>
					<?php } else { // else of rtl { ?>
					<li><a href="<?php echo $view_purchases_details_list->PageUrl() ?>start=<?php echo $view_purchases_details_list->Pager->PrevButton->Start ?>"><?php echo $Language->Phrase("PagerPrevious") ?></a></li>
					<?php } // end of rtl { ?>
					<?php } ?>
					<?php foreach ($view_purchases_details_list->Pager->Items as $PagerItem) { ?>
						<li<?php if (!$PagerItem->Enabled) { echo " class=\" active\""; } ?>><a href="<?php if ($PagerItem->Enabled) { echo $view_purchases_details_list->PageUrl() . "start=" . $PagerItem->Start; } else { echo "#"; } ?>"><?php echo $PagerItem->Text ?></a></li>
					<?php } ?>
					<?php if ($view_purchases_details_list->Pager->NextButton->Enabled) { ?>
					<?php if ($Language->Phrase("dir") == "rtl") { // begin of rtl ?>
					<li><a href="<?php echo $view_purchases_details_list->PageUrl() ?>start=<?php echo $view_purchases_details_list->Pager->NextButton->Start ?>"><?php echo $Language->Phrase("PagerPrevious") ?></a></li>
					<?php } else { // else of rtl ?>
					<li><a href="<?php echo $view_purchases_details_list->PageUrl() ?>start=<?php echo $view_purchases_details_list->Pager->NextButton->Start ?>"><?php echo $Language->Phrase("PagerNext") ?></a></li>
					<?php } // end of rtl ?>
					<?php } ?>
					<?php if ($view_purchases_details_list->Pager->LastButton->Enabled) { ?>
					<?php if ($Language->Phrase("dir") == "rtl") { // begin of rtl ?>
					<li><a href="<?php echo $view_purchases_details_list->PageUrl() ?>start=<?php echo $view_purchases_details_list->Pager->LastButton->Start ?>"><?php echo $Language->Phrase("PagerFirst") ?></a></li>
					<?php } else { // else of rtl ?>
					<li><a href="<?php echo $view_purchases_details_list->PageUrl() ?>start=<?php echo $view_purchases_details_list->Pager->LastButton->Start ?>"><?php echo $Language->Phrase("PagerLast") ?></a></li>
					<?php } // end of rtl ?>
					<?php } ?>
				</ul></div>
				</div>
				...
				<div class="ewPager ewRec">
					<span><?php echo $Language->Phrase("Record") ?>&nbsp;<?php echo $view_purchases_details_list->Pager->FromIndex ?>&nbsp;<?php echo $Language->Phrase("To") ?>&nbsp;<?php echo $view_purchases_details_list->Pager->ToIndex ?>&nbsp;<?php echo $Language->Phrase("Of") ?>&nbsp;<?php echo $view_purchases_details_list->Pager->RecordCount ?></span>
				</div>
		<?php } ?>	
	<?php } elseif (MS_PAGINATION_STYLE==2) { // button ?>
		<?php if (!isset($view_purchases_details_list->Pager)) $view_purchases_details_list->Pager = new cPrevNextPager($view_purchases_details_list->StartRec, $view_purchases_details_list->DisplayRecs, $view_purchases_details_list->TotalRecs) ?>
		<?php if ($view_purchases_details_list->Pager->RecordCount > 0) { ?>
				<?php if (($view_purchases_details_list->Pager->PageCount==1) && ($view_purchases_details_list->Pager->CurrentPage == 1) && (MS_SHOW_PAGENUM_IF_REC_NOT_OVER_PAGESIZE==FALSE)  ) { ?>
				...
				<!--first page button-->
					<?php if ($view_purchases_details_list->Pager->FirstButton->Enabled) { ?>
					<?php if ($Language->Phrase("dir") == "rtl") { // begin of rtl ?>
					<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerFirst") ?>" href="<?php echo $view_purchases_details_list->PageUrl() ?>start=<?php echo $view_purchases_details_list->Pager->FirstButton->Start ?>"><span class="icon-last ewIcon"></span></a>
					<?php } else { // else of rtl ?>
					<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerFirst") ?>" href="<?php echo $view_purchases_details_list->PageUrl() ?>start=<?php echo $view_purchases_details_list->Pager->FirstButton->Start ?>"><span class="icon-first ewIcon"></span></a>
					<?php } // end of rtl ?>
					<?php } else { ?>
					<?php if ($Language->Phrase("dir") == "rtl") { // begin of rtl ?>
				...
				<!--previous page button-->
					<?php if ($view_purchases_details_list->Pager->PrevButton->Enabled) { ?>
					<?php if ($Language->Phrase("dir") == "rtl") { // begin of rtl ?>
					<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerPrevious") ?>" href="<?php echo $view_purchases_details_list->PageUrl() ?>start=<?php echo $view_purchases_details_list->Pager->PrevButton->Start ?>"><span class="icon-next ewIcon"></span></a>
					<?php } else { // else of rtl ?>
					<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerPrevious") ?>" href="<?php echo $view_purchases_details_list->PageUrl() ?>start=<?php echo $view_purchases_details_list->Pager->PrevButton->Start ?>"><span class="icon-prev ewIcon"></span></a>
					<?php } // end of rtl ?>
					<?php } else { ?>
					<?php if ($Language->Phrase("dir") == "rtl") { // begin of rtl ?>
				...
				<!--current page number-->
					<input class="form-control input-sm" type="text" name="<?php echo EW_TABLE_PAGE_NO ?>" value="<?php echo $view_purchases_details_list->Pager->CurrentPage ?>">
				<div class="input-group-btn">
				<!--next page button-->
					<?php if ($view_purchases_details_list->Pager->NextButton->Enabled) { ?>
					<?php if ($Language->Phrase("dir") == "rtl") { // begin of rtl ?>
					<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerNext") ?>" href="<?php echo $view_purchases_details_list->PageUrl() ?>start=<?php echo $view_purchases_details_list->Pager->NextButton->Start ?>"><span class="icon-prev ewIcon"></span></a>
					<?php } else { // else of rtl ?>
					<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerNext") ?>" href="<?php echo $view_purchases_details_list->PageUrl() ?>start=<?php echo $view_purchases_details_list->Pager->NextButton->Start ?>"><span class="icon-next ewIcon"></span></a>
					<?php } // end of rtl ?>
					<?php } else { ?>
					<?php if ($Language->Phrase("dir") == "rtl") { // begin of rtl ?>
				...
				<!--last page button-->
					<?php if ($view_purchases_details_list->Pager->LastButton->Enabled) { ?>
					<?php if ($Language->Phrase("dir") == "rtl") { // begin of rtl ?>
					<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerLast") ?>" href="<?php echo $view_purchases_details_list->PageUrl() ?>start=<?php echo $view_purchases_details_list->Pager->LastButton->Start ?>"><span class="icon-first ewIcon"></span></a>
					<?php } else { // else of rtl ?>
					<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerLast") ?>" href="<?php echo $view_purchases_details_list->PageUrl() ?>start=<?php echo $view_purchases_details_list->Pager->LastButton->Start ?>"><span class="icon-last ewIcon"></span></a>
					<?php } // end of rtl ?>
					<?php } else { ?>
					<?php if ($Language->Phrase("dir") == "rtl") { // begin of rtl ?>
				...
				<span>&nbsp;<?php echo $Language->Phrase("of") ?>&nbsp;<?php echo $view_purchases_details_list->Pager->PageCount ?></span>
				</div>
...
<input type="hidden" name="t" value="view_purchases_details">
<select name="<?php echo EW_TABLE_REC_PER_PAGE ?>" class="form-control input-sm" onchange="this.form.submit();">
<option value="1"<?php if ($view_purchases_details_list->DisplayRecs == 1) { ?> selected="selected"<?php } ?>>1</option>
<option value="3"<?php if ($view_purchases_details_list->DisplayRecs == 3) { ?> selected="selected"<?php } ?>>3</option>
<option value="5"<?php if ($view_purchases_details_list->DisplayRecs == 5) { ?> selected="selected"<?php } ?>>5</option>
<option value="10"<?php if ($view_purchases_details_list->DisplayRecs == 10) { ?> selected="selected"<?php } ?>>10</option>
<option value="20"<?php if ($view_purchases_details_list->DisplayRecs == 20) { ?> selected="selected"<?php } ?>>20</option>
<option value="50"<?php if ($view_purchases_details_list->DisplayRecs == 50) { ?> selected="selected"<?php } ?>>50</option>
<option value="100"<?php if ($view_purchases_details_list->DisplayRecs == 100) { ?> selected="selected"<?php } ?>>100</option>
</select>
</div>
...
<form name="fview_purchases_detailslist" id="fview_purchases_detailslist" class="form-inline ewForm ewListForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($view_purchases_details_list->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $view_purchases_details_list->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="view_purchases_details">
...
// Begin of mofidication Flexibility of Export Records Options, by Masino Sinaga, May 14, 2012
if ($view_purchases_details->ExportAll=="allpages" && $view_purchases_details->Export <> "") {
    $view_purchases_details_list->StopRec = $view_purchases_details_list->TotalRecs;

// End of mofidication Flexibility of Export Records Options, by Masino Sinaga, May 14, 2012
} else {

	// Set the last record to display
	if ($view_purchases_details_list->TotalRecs > $view_purchases_details_list->StartRec + $view_purchases_details_list->DisplayRecs - 1)
		$view_purchases_details_list->StopRec = $view_purchases_details_list->StartRec + $view_purchases_details_list->DisplayRecs - 1;
	else
		$view_purchases_details_list->StopRec = $view_purchases_details_list->TotalRecs;
}
$view_purchases_details_list->RecCnt = $view_purchases_details_list->StartRec - 1;
if ($view_purchases_details_list->Recordset && !$view_purchases_details_list->Recordset->EOF) {
	...
	$bSelectLimit = EW_SELECT_LIMIT;
	if (!$bSelectLimit && $view_purchases_details_list->StartRec > 1)
		$view_purchases_details_list->Recordset->Move($view_purchases_details_list->StartRec - 1);
} elseif (!$view_purchases_details->AllowAddDeleteRow && $view_purchases_details_list->StopRec == 0) {
	$view_purchases_details_list->StopRec = $view_purchases_details->GridAddRowCount;
}
...
while ($view_purchases_details_list->RecCnt < $view_purchases_details_list->StopRec) {
	$view_purchases_details_list->RecCnt++;
	if (intval($view_purchases_details_list->RecCnt) >= intval($view_purchases_details_list->StartRec)) {
		$view_purchases_details_list->RowCnt++;
		...
		$view_purchases_details->RowAttrs = array_merge($view_purchases_details->RowAttrs, array('data-rowindex'=>$view_purchases_details_list->RowCnt, 'id'=>'r' . $view_purchases_details_list->RowCnt . '_view_purchases_details', 'data-rowtype'=>$view_purchases_details->RowType));
...
$view_purchases_details_list->ListOptions->Render("body", "left", $view_purchases_details_list->RowCnt);
?>
	<?php if ($view_purchases_details->Purchase_Number->Visible) { // Purchase_Number ?>
		<td data-name="Purchase_Number"<?php echo $view_purchases_details->Purchase_Number->CellAttributes() ?>>
<span<?php echo $view_purchases_details->Purchase_Number->ViewAttributes() ?>>
...
$view_purchases_details_list->ListOptions->Render("body", "right", $view_purchases_details_list->RowCnt);
?>
	</tr>
<?php
	}
...
$view_purchases_details->RowType = EW_ROWTYPE_AGGREGATE;
$view_purchases_details->ResetAttrs();
$view_purchases_details_list->RenderRow();
?>
<?php if ($view_purchases_details_list->TotalRecs > 0 && ($view_purchases_details->CurrentAction <> "gridadd" && $view_purchases_details->CurrentAction <> "gridedit")) { ?>
<tfoot><!-- Table footer -->
	<tr class="ewTableFooter">
<?php
...
$view_purchases_details_list->ListOptions->Render("footer", "left");
?>
	<?php if ($view_purchases_details->Purchase_Number->Visible) { // Purchase_Number ?>
		<td data-name="Purchase_Number"><span id="elf_view_purchases_details_Purchase_Number" class="view_purchases_details_Purchase_Number">
		&nbsp;
...
$view_purchases_details_list->ListOptions->Render("footer", "right");
?>
	</tr>
</tfoot>	
<?php } ?>
...
<?php if ($view_purchases_details->Export == "") { ?>
<script type="text/javascript">
fview_purchases_detailslistsrch.Init();
fview_purchases_detailslist.Init();
</script>
<?php } ?>
<?php
...
<?php if (MS_USE_TABLE_SETTING_FOR_SEARCH_PANEL_COLLAPSED) { ?>
<?php if (isset($_SESSION['table_view_purchases_details_views']) && $_SESSION['table_view_purchases_details_views'] == 1) { ?>
...
<script type="text/javascript">
$(document).ready(function() { var SearchToggle = $('.ewSearchToggle'); var SearchPanel = $('.ewSearchPanel'); if(getCookie('view_purchases_details_searchpanel')=="active"){ SearchToggle.addClass(getCookie('view_purchases_details_searchpanel')); SearchPanel.addClass('in'); SearchToggle.addClass('active'); }else{ SearchPanel.removeClass('in'); SearchToggle.removeClass('active'); } SearchToggle.on('click',function(event) { event.preventDefault(); if (SearchToggle.hasClass('active')){ createCookie("view_purchases_details_searchpanel", "notactive", 1); }else{ createCookie("view_purchases_details_searchpanel", "active", 1); } }); });
</script>
...
<?php if ($view_purchases_details->Export == "") { ?>
<script type="text/javascript">
$('.ewGridSave, .ewGridInsert').attr('onclick', 'return alertifySaveGrid(this)'); function alertifySaveGrid(obj) { <?php global $Language; ?> if (fview_purchases_detailslist.Validate() == true ) { alertify.set({buttonFocus: 'cancel'});alertify.confirm("<?php echo $Language->Phrase('AlertifySaveGridConfirm'); ?>", function (e) { if (e) { $(window).unbind('beforeunload'); alertify.success("<?php echo $Language->Phrase('AlertifySaveGrid'); ?>"); $("#fview_purchases_detailslist").submit(); } else { alertify.error("<?php echo $Language->Phrase('AlertifyCancel'); ?>"); } }, "<?php echo $Language->Phrase('AlertifyConfirm'); ?>"); } return false; }
</script>
<script type="text/javascript">
$('.ewInlineUpdate').attr('onclick', 'return alertifySaveInlineEdit(this)'); function alertifySaveInlineEdit(obj) { <?php global $Language; ?> if (fview_purchases_detailslist.Validate() == true ) { alertify.set({buttonFocus: 'cancel'});alertify.confirm("<?php echo $Language->Phrase('AlertifySaveGridConfirm'); ?>", function (e) { if (e) { $(window).unbind('beforeunload'); alertify.success("<?php echo $Language->Phrase('AlertifySaveGrid'); ?>"); $("#fview_purchases_detailslist").submit(); } else { alertify.error("<?php echo $Language->Phrase('AlertifyCancel'); ?>"); } }, "<?php echo $Language->Phrase('AlertifyConfirm'); ?>"); } return false; }
</script>
<script type="text/javascript">
$('.ewInlineInsert').attr('onclick', 'return alertifySaveInlineInsert(this)'); function alertifySaveInlineInsert(obj) { <?php global $Language; ?> if (fview_purchases_detailslist.Validate() == true ) { alertify.set({buttonFocus: 'cancel'});alertify.confirm("<?php echo $Language->Phrase('AlertifySaveGridConfirm'); ?>", function (e) { if (e) { $(window).unbind('beforeunload'); alertify.success("<?php echo $Language->Phrase('AlertifySaveGrid'); ?>"); $("#fview_purchases_detailslist").submit(); } else { alertify.error("<?php echo $Language->Phrase('AlertifyCancel'); ?>"); } }, "<?php echo $Language->Phrase('AlertifyConfirm'); ?>"); } return false; }
</script>
<?php } ?>
<?php if ($view_purchases_details->CurrentAction == "" || $view_purchases_details->Export == "") { // Change && become || in order to add scroll table in Grid, by Masino Sinaga, August 3, 2014 ?>
<script type="text/javascript">
...
jQuery(function(){ew_ScrollableTable("gmp_view_purchases_details", "<?php echo MS_SCROLL_TABLE_WIDTH; ?>px", "<?php echo MS_SCROLL_TABLE_HEIGHT; ?>px");});
jQuery(function(){ew_ScrollableTable("gmp_view_purchases_details_empty_table", "<?php echo MS_SCROLL_TABLE_WIDTH; ?>px", "<?php echo MS_SCROLL_TABLE_HEIGHT; ?>px");});
...
jQuery(function(){ew_ScrollableTable("gmp_view_purchases_details", "100%", "<?php echo MS_SCROLL_TABLE_HEIGHT; ?>px");});
jQuery(function(){ew_ScrollableTable("gmp_view_purchases_details_empty_table", "100%", "<?php echo MS_SCROLL_TABLE_HEIGHT; ?>px");});
...
<?php
$view_purchases_details_list->Page_Terminate();
?>
