...
$a_sales_list = NULL; // Initialize page object first

class ca_sales_list extends ca_sales {
	...
	// Page object name
	var $PageObjName = 'a_sales_list';

	// Grid form hidden field names
	var $FormName = 'fa_saleslist';
	...
	function __construct() {
		...
		$this->AddUrl = "a_salesadd.php?" . EW_TABLE_SHOW_DETAIL . "=";
		$this->InlineAddUrl = $this->PageUrl() . "a=add";
		$this->GridAddUrl = $this->PageUrl() . "a=gridadd";
		$this->GridEditUrl = $this->PageUrl() . "a=gridedit";
		$this->MultiDeleteUrl = "a_salesdelete.php";
		$this->MultiUpdateUrl = "a_salesupdate.php";
...
// End of modification Customize Navigation/Pager Panel, by Masino Sinaga, May 2, 2012
	...
	function Page_Main() {
		...
		// Load master record
		if ($this->CurrentMode <> "add" && $this->GetMasterFilter() <> "" && $this->getCurrentMasterTable() == "a_customers") {
			global $a_customers;
			$rsmaster = $a_customers->LoadRs($this->DbMasterFilter);
			$this->MasterRecordExists = ($rsmaster && !$rsmaster->EOF);
			if (!$this->MasterRecordExists) {
				$this->setFailureMessage($Language->Phrase("NoRecord")); // Set no record found
				$this->Page_Terminate("a_customerslist.php"); // Return to master page
			} else {
				$a_customers->LoadListRowValues($rsmaster);
				$a_customers->RowType = EW_ROWTYPE_MASTER; // Master row
				$a_customers->RenderListRow();
				$rsmaster->Close();
			}
		}
	...
	function AdvancedSearchWhere($Default = FALSE) {
		...
		$this->BuildSearchSql($sWhere, $this->Tax_Amount, $Default, FALSE); // Tax_Amount
	...
	function SetUpSortOrder() {
		...
		if (@$_GET["order"] <> "") {
			...
			$this->UpdateSort($this->Tax_Amount); // Tax_Amount
	...
	function SetupListOptions() {
		...
		$item = &$this->ListOptions->Add("detail_a_sales_detail");
		$item->CssStyle = "white-space: nowrap;";
		$item->Visible = $Security->AllowList(CurrentProjectID() . 'a_sales_detail') && !$this->ShowMultipleDetails;
		$item->OnLeft = TRUE;
	...
	function RenderListOptions() {
		...
		// "detail_a_sales_detail"
		$oListOpt = &$this->ListOptions->Items["detail_a_sales_detail"];
		if ($Security->AllowList(CurrentProjectID() . 'a_sales_detail')) {
			$body = $Language->Phrase("DetailLink") . $Language->TablePhrase("a_sales_detail", "TblCaption");
			...
			if ( $this->a_sales_detail_Count > 0 && MS_SHOW_DETAILCOUNT_GREATER_THAN_ZERO_ONLY == TRUE ) {
				if (MS_USE_BADGE_FOR_DETAILCOUNT) {
					$body .= "&nbsp;<i class='badge'>".$this->a_sales_detail_Count."</i>"; // we cannot use <span class='badge'></span> here, not sure why? strange, huh?
				} else {
					$body .= "&nbsp;" . str_replace("%c", $this->a_sales_detail_Count, $Language->Phrase("DetailCount"));
				}
			...
			$links = "";
			if ($GLOBALS["a_sales_detail_grid"]->DetailView && $Security->CanView() && $Security->AllowView(CurrentProjectID() . 'a_sales_detail')) {
			...
			if ($GLOBALS["a_sales_detail_grid"]->DetailEdit && $Security->CanEdit() && $Security->AllowEdit(CurrentProjectID() . 'a_sales_detail')) {
	...
	function SetupOtherOptions() {
		...
		$DetailTableLink = "";
		$item = &$option->Add("detailadd_a_sales_detail");
		$item->Body = "<a class=\"ewDetailAddGroup ewDetailAdd\" title=\"" . ew_HtmlTitle($Language->Phrase("AddMasterDetailLink")) . "\" data-caption=\"" . ew_HtmlTitle($Language->Phrase("AddMasterDetailLink")) . "\" href=\"" . ew_HtmlEncode($this->GetAddUrl() . "?" . EW_TABLE_SHOW_DETAIL . "=a_sales_detail") . "\">" . $Language->Phrase("Add") . "&nbsp;" . $this->TableCaption() . "/" . $GLOBALS["a_sales_detail"]->TableCaption() . "</a>";
		$item->Visible = ($GLOBALS["a_sales_detail"]->DetailAdd && $Security->AllowAdd(CurrentProjectID() . 'a_sales_detail') && $Security->CanAdd());
		if ($item->Visible) {
		...
		$item->Body = "<a class=\"ewAction ewMultiDelete\" title=\"" . ew_HtmlTitle($Language->Phrase("DeleteSelectedLink")) . "\" data-caption=\"" . ew_HtmlTitle($Language->Phrase("DeleteSelectedLink")) . "\" href=\"\" onclick=\"ew_SubmitSelected(document.fa_saleslist, '" . $this->MultiDeleteUrl . "', ewLanguage.Phrase('DeleteMultiConfirmMsg'));return false;\">" . $Language->Phrase("DeleteSelectedLink") . "</a>";
		$item->Visible = ($Security->CanDelete());
	...
	function RenderOtherOptions() {
		...
		$options = &$this->OtherOptions;
			...
			foreach ($this->CustomActions as $action => $name) {
				...
				$item->Body = "<a class=\"ewAction ewCustomAction\" href=\"\" onclick=\"ew_SubmitSelected(document.fa_saleslist, '" . ew_CurrentUrl() . "', null, '" . $action . "');return false;\">" . $name . "</a>";
			}
	...
	function SetupSearchOptions() {
		...
		if ($this->CurrentAction == "edit" || $this->CurrentAction == "copy") {
			...
			$item->Body = "<button type=\"button\" class=\"btn btn-default ewSearchToggle" . $SearchToggleClass . "\" title=\"" . $Language->Phrase("SearchPanel") . "\" data-caption=\"" . $Language->Phrase("SearchPanel") . "\" data-toggle=\"button\" data-form=\"fa_saleslistsrch\">" . $Language->Phrase("SearchBtn") . "</button>";
			$item->Visible = TRUE;
		}
		...
		$item->Body = "<a class=\"btn btn-default ewAdvancedSearch\" title=\"" . $Language->Phrase("AdvancedSearch") . "\" data-caption=\"" . $Language->Phrase("AdvancedSearch") . "\" href=\"a_salessrch.php\">" . $Language->Phrase("AdvancedSearchBtn") . "</a>";
		$item->Visible = TRUE;
	...
	function RenderListOptionsExt() {
		...
		$links = "";
		$btngrps = "";
		$sSqlWrk = "`Sales_Number`='" . ew_AdjustSql($this->Sales_Number->CurrentValue) . "'";

		// Column "detail_a_sales_detail"
		$link = "";
		$option = &$this->ListOptions->Items["detail_a_sales_detail"];
		$url = "a_sales_detailpreview.php?t=a_sales&f=" . ew_Encrypt($sSqlWrk);
		$btngrp = "<div data-table=\"a_sales_detail\" data-url=\"" . $url . "\" class=\"btn-group\">";
		if ($Security->AllowList(CurrentProjectID() . 'a_sales_detail')) {			
			$label = $Language->TablePhrase("a_sales_detail", "TblCaption");

			// $label .= "&nbsp;" . ew_JsEncode2(str_replace("%c", $this->a_sales_detail_Count, $Language->Phrase("DetailCount")));
			if ( $this->a_sales_detail_Count > 0 && @MS_SHOW_DETAILCOUNT_GREATER_THAN_ZERO_ONLY == TRUE ) {
				if (@MS_USE_BADGE_FOR_DETAILCOUNT) {
					$label .= "&nbsp; <span class='badge badge-info'>".$this->a_sales_detail_Count."</span>"; 
				} else {
					$label .= "&nbsp;" . ew_JsEncode2(str_replace("%c", $this->a_sales_detail_Count, $Language->Phrase("DetailCount")));
				}
			} elseif ( $this->a_sales_detail_Count >= 0 && @MS_SHOW_DETAILCOUNT_GREATER_THAN_ZERO_ONLY == FALSE ) {
			...
			$link = "<li><a href=\"#\" data-toggle=\"tab\" data-table=\"a_sales_detail\" data-url=\"" . $url . "\">" . $label . "</a></li>";			
			$links .= $link;
			$detaillnk = ew_JsEncode3("a_sales_detaillist.php?" . EW_TABLE_SHOW_MASTER . "=a_sales&fk_Sales_Number=" . urlencode(strval($this->Sales_Number->CurrentValue)) . "");
			$btngrp .= "<button type=\"button\" class=\"btn btn-default btn-sm\" title=\"" . $Language->TablePhrase("a_sales_detail", "TblCaption") . "\" onclick=\"window.location='" . $detaillnk . "'\">" . $Language->Phrase("MasterDetailListLink") . "</button>";
		}
		if ($GLOBALS["a_sales_detail_grid"]->DetailView && $Security->CanView() && $Security->AllowView(CurrentProjectID() . 'a_sales_detail'))
			$btngrp .= "<button type=\"button\" class=\"btn btn-default btn-sm\" title=\"" . ew_HtmlTitle($Language->Phrase("MasterDetailViewLink")) . "\" onclick=\"window.location='" . $this->GetViewUrl(EW_TABLE_SHOW_DETAIL . "=a_sales_detail") . "'\">" . $Language->Phrase("MasterDetailViewLink") . "</button>";
		if ($GLOBALS["a_sales_detail_grid"]->DetailEdit && $Security->CanEdit() && $Security->AllowEdit(CurrentProjectID() . 'a_sales_detail'))
			$btngrp .= "<button type=\"button\" class=\"btn btn-default btn-sm\" title=\"" . ew_HtmlTitle($Language->Phrase("MasterDetailEditLink")) . "\" onclick=\"window.location='" . $this->GetEditUrl(EW_TABLE_SHOW_DETAIL . "=a_sales_detail") . "'\">" . $Language->Phrase("MasterDetailEditLink") . "</button>";
		$btngrp .= "</div>";
		if ($link <> "") {
			$btngrps .= $btngrp;
	...
	function LoadSearchValues() {
		...
		// Tax_Amount
		$this->Tax_Amount->AdvancedSearch->SearchValue = ew_StripSlashes(@$_GET["x_Tax_Amount"]);
		if ($this->Tax_Amount->AdvancedSearch->SearchValue <> "") $this->Command = "search";
		$this->Tax_Amount->AdvancedSearch->SearchOperator = @$_GET["z_Tax_Amount"];
	...
	function LoadRowValues(&$rs) {
		...
		$sDetailFilter = str_replace("@Sales_Number@", ew_AdjustSql($this->Sales_Number->DbValue), $sDetailFilter);
	...
	function RenderRow() {
		...
		if ($this->RowType == EW_ROWTYPE_VIEW) { // View row
			...
			$this->Discount_Amount->Total = 0; // Initialize total
			$this->Tax_Amount->Total = 0; // Initialize total
...
// Begin of modification Permission Access for Export To Feature, by Masino Sinaga, May 5, 2012
        ...
        if ($Security->CanExportToPrint() || $Security->IsAdmin() ) {
			...
			if (MS_EXPORT_RECORD_OPTIONS=="selectedrecords") {
				$item->Body = "<a class=\"ewExportLink ewPrint\" title=\"" . ew_HtmlEncode($Language->Phrase("PrinterFriendlyText")) . "\" data-caption=\"" . ew_HtmlEncode($Language->Phrase("PrinterFriendlyText")) . "\" onclick=\"ew_Export(document.fa_saleslist,'" . ew_CurrentPage() . "','print',false);\">" . $Language->Phrase("PrinterFriendly") . "</a>";
			} else {
				$item->Body = "<a href=\"" . $this->ExportPrintUrl . "\" class=\"ewExportLink ewPrint\" title=\"" . ew_HtmlEncode($Language->Phrase("PrinterFriendlyText")) . "\"  data-caption=\"" . ew_HtmlEncode($Language->Phrase("PrinterFriendlyText")) . "\">" . $Language->Phrase("PrinterFriendly") . "</a>";
			}
        ...
        if ($Security->CanExportToExcel() || $Security->IsAdmin() ) {
			...
			if (MS_EXPORT_RECORD_OPTIONS=="selectedrecords") {
				$item->Body = "<a class=\"ewExportLink ewExcel\" title=\"" . ew_HtmlEncode($Language->Phrase("ExportToExcelText")) . "\" data-caption=\"" . ew_HtmlEncode($Language->Phrase("ExportToExcelText")) . "\" onclick=\"ew_Export(document.fa_saleslist,'" . ew_CurrentPage() . "','excel',false);\">" . $Language->Phrase("ExportToExcel") . "</a>";
			} else {
				$item->Body = "<a href=\"" . $this->ExportExcelUrl . "\" class=\"ewExportLink ewExcel\" title=\"" . ew_HtmlEncode($Language->Phrase("ExportToExcelText")) . "\"  data-caption=\"" . ew_HtmlEncode($Language->Phrase("ExportToExcelText")) . "\">" . $Language->Phrase("ExportToExcel") . "</a>";
			}
        ...
        if ($Security->CanExportToWord() || $Security->IsAdmin() ) {
			...
			if (MS_EXPORT_RECORD_OPTIONS=="selectedrecords") {
				$item->Body = "<a class=\"ewExportLink ewWord\" title=\"" . ew_HtmlEncode($Language->Phrase("ExportToWordText")) . "\" data-caption=\"" . ew_HtmlEncode($Language->Phrase("ExportToWordText")) . "\" onclick=\"ew_Export(document.fa_saleslist,'" . ew_CurrentPage() . "','word',false);\">" . $Language->Phrase("ExportToWord") . "</a>";
			} else {
				$item->Body = "<a href=\"" . $this->ExportWordUrl . "\" class=\"ewExportLink ewWord\" title=\"" . ew_HtmlEncode($Language->Phrase("ExportToWordText")) . "\"  data-caption=\"" . ew_HtmlEncode($Language->Phrase("ExportToWordText")) . "\">" . $Language->Phrase("ExportToWord") . "</a>";
			}
        ...
        if ($Security->CanExportToHTML() || $Security->IsAdmin() ) {
			...
			if (MS_EXPORT_RECORD_OPTIONS=="selectedrecords") {
				$item->Body = "<a class=\"ewExportLink ewHtml\" title=\"" . ew_HtmlEncode($Language->Phrase("ExportToHtmlText")) . "\" data-caption=\"" . ew_HtmlEncode($Language->Phrase("ExportToHtmlText")) . "\" onclick=\"ew_Export(document.fa_saleslist,'" . ew_CurrentPage() . "','html',false);\">" . $Language->Phrase("ExportToHtml") . "</a>";
			} else {
				$item->Body = "<a href=\"" . $this->ExportHtmlUrl . "\" class=\"ewExportLink ewHtml\" title=\"" . ew_HtmlEncode($Language->Phrase("ExportToHtmlText")) . "\"  data-caption=\"" . ew_HtmlEncode($Language->Phrase("ExportToHtmlText")) . "\">" . $Language->Phrase("ExportToHTML") . "</a>";
			}
        ...
        if ($Security->CanExportToXML() || $Security->IsAdmin() ) {
			...
			if (MS_EXPORT_RECORD_OPTIONS=="selectedrecords") {
				$item->Body = "<a class=\"ewExportLink ewXml\" title=\"" . ew_HtmlEncode($Language->Phrase("ExportToXmlText")) . "\" data-caption=\"" . ew_HtmlEncode($Language->Phrase("ExportToXmlText")) . "\" onclick=\"ew_Export(document.fa_saleslist,'" . ew_CurrentPage() . "','xml',false);\">" . $Language->Phrase("ExportToXml") . "</a>";
			} else {
				$item->Body = "<a href=\"" . $this->ExportXmlUrl . "\" class=\"ewExportLink ewXml\" title=\"" . ew_HtmlEncode($Language->Phrase("ExportToXmlText")) . "\" data-caption=\"" . ew_HtmlEncode($Language->Phrase("ExportToXmlText")) . "\">" . $Language->Phrase("ExportToXML") . "</a>";
			}
        ...
        if ($Security->CanExportToCSV() || $Security->IsAdmin() ) {
			...
			if (MS_EXPORT_RECORD_OPTIONS=="selectedrecords") {
				$item->Body = "<a class=\"ewExportLink ewCsv\" title=\"" . ew_HtmlEncode($Language->Phrase("ExportToCsvText")) . "\" data-caption=\"" . ew_HtmlEncode($Language->Phrase("ExportToCsvText")) . "\" onclick=\"ew_Export(document.fa_saleslist,'" . ew_CurrentPage() . "','csv',false);\">" . $Language->Phrase("ExportToCsv") . "</a>";
			} else {
				$item->Body = "<a href=\"" . $this->ExportCsvUrl . "\" class=\"ewExportLink ewCsv\" title=\"" . ew_HtmlEncode($Language->Phrase("ExportToCsvText")) . "\"  data-caption=\"" . ew_HtmlEncode($Language->Phrase("ExportToCsvText")) . "\">" . $Language->Phrase("ExportToCSV") . "</a>";
			}
        ...
        if ($Security->CanExportToPDF() || $Security->IsAdmin() ) {
			...
			if (MS_EXPORT_RECORD_OPTIONS=="selectedrecords") {
				$item->Body = "<a class=\"ewExportLink ewPdf\" title=\"" . ew_HtmlEncode($Language->Phrase("ExportToPDFText")) . "\" data-caption=\"" . ew_HtmlEncode($Language->Phrase("ExportToPDFText")) . "\" onclick=\"ew_Export(document.fa_saleslist,'" . ew_CurrentPage() . "','pdf',false);\">" . $Language->Phrase("ExportToPDF") . "</a>";
			} else {
				$item->Body = "<a href=\"" . $this->ExportPdfUrl . "\" class=\"ewExportLink ewPdf\" title=\"" . ew_HtmlEncode($Language->Phrase("ExportToPDFText")) . "\"  data-caption=\"" . ew_HtmlEncode($Language->Phrase("ExportToPDFText")) . "\">" . $Language->Phrase("ExportToPDF") . "</a>";
			}
		...
		if ($Security->CanExportToEmail() || $Security->IsAdmin() ) {
			...
			// $item->Body = "<button id=\"emf_a_sales\" class=\"ewExportLink ewEmail\" title=\"" . $Language->Phrase("ExportToEmailText") . "\" data-caption=\"" . $Language->Phrase("ExportToEmailText") . "\" onclick=\"ew_EmailDialogShow({lnk:'emf_a_sales',hdr:ewLanguage.Phrase('ExportToEmailText'),f:document.fa_saleslist,sel:false" . $url . "});\">" . $Language->Phrase("ExportToEmail") . "</button>";
			// Begin of mofidication Flexibility of Export Records Options, by Masino Sinaga, May 14, 2012

		if (MS_EXPORT_RECORD_OPTIONS=="selectedrecords") {
			$item->Body = "<a id=\"emf_a_sales\" href=\"javascript:void(0);\" class=\"ewExportLink ewEmail\" title=\"" . $Language->Phrase("ExportToEmailText") . "\"  data-caption=\"" . $Language->Phrase("ExportToEmailText") . "\" onclick=\"ew_EmailDialogShow({lnk:'emf_a_sales',hdr:ewLanguage.Phrase('ExportToEmailText'),f:document.fa_saleslist,sel:true});\">" . $Language->Phrase("ExportToEmail") . "</a>";
		} else {
			$item->Body = "<a id=\"emf_a_sales\" href=\"javascript:void(0);\" class=\"ewExportLink ewEmail\" title=\"" . $Language->Phrase("ExportToEmailText") . "\"  data-caption=\"" . $Language->Phrase("ExportToEmailText") . "\" onclick=\"ew_EmailDialogShow({lnk:'emf_a_sales',hdr:ewLanguage.Phrase('ExportToEmailText'),f:document.fa_saleslist,sel:false});\">" . $Language->Phrase("ExportToEmail") . "</a>";
		}
	...
	function ExportData() {
		...
		if (EW_EXPORT_MASTER_RECORD && $this->GetMasterFilter() <> "" && $this->getCurrentMasterTable() == "a_customers") {
			global $a_customers;
			if (!isset($a_customers)) $a_customers = new ca_customers;
			$rsmaster = $a_customers->LoadRs($this->DbMasterFilter); // Load master record
			if ($rsmaster && !$rsmaster->EOF) {
				if ($this->Export <> "csv" || EW_EXPORT_MASTER_RECORD_FOR_CSV) {
					$Doc->Table = &$a_customers; // v11.0.4
					$a_customers->ExportDocument($Doc, $rsmaster, 1, 1);
					$Doc->ExportEmptyRow();
	...
	function ExportQueryString() {
		...
		$this->AddSearchQueryString($sQry, $this->Tax_Amount); // Tax_Amount
	...
	function SetUpMasterParms() {
		...
		if ($bValidMaster) {
			...
			if ($sMasterTblVar <> "a_customers") {
				if ($this->Customer_ID->QueryStringValue == "") $this->Customer_ID->setSessionValue("");
			}
		}
...
// Create page object
if (!isset($a_sales_list)) $a_sales_list = new ca_sales_list();

// Page init
$a_sales_list->Page_Init();

// Page main
$a_sales_list->Page_Main();
...
// Page object
var a_sales_list = new ew_Page("a_sales_list");
a_sales_list.PageID = "list"; // Page ID
var EW_PAGE_ID = a_sales_list.PageID; // For backward compatibility

// Form object
var fa_saleslist = new ew_Form("fa_saleslist");
fa_saleslist.FormKeyCountName = '<?php echo $a_sales_list->FormKeyCountName ?>';

// Form_CustomValidate event
fa_saleslist.Form_CustomValidate = 
...
fa_saleslist.ValidateRequired = true;
<?php } else { ?>
fa_saleslist.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
fa_saleslist.Lists["x_Customer_ID"] = {"LinkField":"x_Customer_Number","Ajax":null,"AutoFill":false,"DisplayFields":["x_Customer_Name","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};

// Form object for search
var fa_saleslistsrch = new ew_Form("fa_saleslistsrch");

// Init search panel as collapsed
<?php if (MS_USE_TABLE_SETTING_FOR_SEARCH_PANEL_COLLAPSED) { ?>
if (fa_saleslistsrch) fa_saleslistsrch.InitSearchPanel = true;
<?php } else { ?>
...
if (fa_saleslistsrch) fa_saleslistsrch.InitSearchPanel = false;
<?php } ?>
<?php } ?>
</script>
...
<?php } ?>
<?php } ?>
<?php if ($a_sales_list->TotalRecs > 0 && $a_sales->getCurrentMasterTable() == "" && $a_sales_list->ExportOptions->Visible()) { ?>
<?php $a_sales_list->ExportOptions->Render("body") ?>
<?php } ?>
<?php if (($a_sales->Export == "") || (EW_EXPORT_MASTER_RECORD && $a_sales->Export == "print")) { ?>
<?php
$gsMasterReturnUrl = "a_customerslist.php";
if ($a_sales_list->DbMasterFilter <> "" && $a_sales->getCurrentMasterTable() == "a_customers") {
	if ($a_sales_list->MasterRecordExists) {
		if ($a_sales->getCurrentMasterTable() == $a_sales->TableVar) $gsMasterReturnUrl .= "?" . EW_TABLE_SHOW_MASTER . "=";
?>
<?php if ($a_sales_list->ExportOptions->Visible()) { ?>
<?php $a_sales_list->ExportOptions->Render("body") ?> 
<?php } ?>
<?php if ($a_sales_list->SearchOptions->Visible()) { ?>
<?php $a_sales_list->SearchOptions->Render("body") ?>
<?php } ?>
<?php if ($a_sales->Export == "") { ?>
...
<?php } ?>
<?php include_once "a_customersmaster.php" ?>
<?php
	}
}
?>
...
<?php
	...
	if ($bSelectLimit) { // begin of v11.0.4
		if ($a_sales_list->TotalRecs <= 0)
			$a_sales_list->TotalRecs = $a_sales->SelectRecordCount();
	} else {
		if (!$a_sales_list->Recordset && ($a_sales_list->Recordset = $a_sales_list->LoadRecordset()))
			$a_sales_list->TotalRecs = $a_sales_list->Recordset->RecordCount();
	} // end of v11.0.4
	$a_sales_list->StartRec = 1;

// Begin of mofidication Flexibility of Export Records Options, by Masino Sinaga, May 14, 2012     
    if ($a_sales_list->DisplayRecs <= 0 || ($a_sales->Export <> "" && $a_sales->ExportAll=="allpages")) // Display all records
        $a_sales_list->DisplayRecs = $a_sales_list->TotalRecs;
    if (!($a_sales->Export <> "" && $a_sales->ExportAll=="allpages"))
        $a_sales_list->SetUpStartRec(); // Set up start record position

// End of mofidication Flexibility of Export Records Options, by Masino Sinaga, May 14, 2012
	if ($bSelectLimit)
		$a_sales_list->Recordset = $a_sales_list->LoadRecordset($a_sales_list->StartRec-1, $a_sales_list->DisplayRecs);

	// Set no record found message
	if ($a_sales->CurrentAction == "" && $a_sales_list->TotalRecs == 0) {
		if (!$Security->CanList())
			$a_sales_list->setWarningMessage($Language->Phrase("NoPermission"));
		if ($a_sales_list->SearchWhere == "0=101")
			$a_sales_list->setWarningMessage($Language->Phrase("EnterSearchCriteria"));
		else
			$a_sales_list->setWarningMessage($Language->Phrase("NoRecord"));
	}
$a_sales_list->RenderOtherOptions();
?>
<?php if ($Security->CanSearch()) { ?>
<?php if ($a_sales->Export == "" && $a_sales->CurrentAction == "") { ?>
<form name="fa_saleslistsrch" id="fa_saleslistsrch" class="form-inline ewForm" action="<?php echo ew_CurrentPage() ?>">
<?php $SearchPanelClass = ($a_sales_list->SearchWhere <> "") ? " in" : " in"; ?>
<div id="fa_saleslistsrch_SearchPanel" class="ewSearchPanel collapse<?php echo $SearchPanelClass ?>">
<input type="hidden" name="cmd" value="search">
<input type="hidden" name="t" value="a_sales">
	<div class="ewBasicSearch">
<div id="xsr_1" class="ewRow">
	<div class="ewQuickSearch input-group">
	<input type="text" name="<?php echo EW_TABLE_BASIC_SEARCH ?>" id="<?php echo EW_TABLE_BASIC_SEARCH ?>" class="form-control" value="<?php echo ew_HtmlEncode($a_sales_list->BasicSearch->getKeyword()) ?>" placeholder="<?php echo ew_HtmlEncode($Language->Phrase("Search")) ?>">
	<input type="hidden" name="<?php echo EW_TABLE_BASIC_SEARCH_TYPE ?>" id="<?php echo EW_TABLE_BASIC_SEARCH_TYPE ?>" value="<?php echo ew_HtmlEncode($a_sales_list->BasicSearch->getType()) ?>">
	<div class="input-group-btn">
		<button type="button" data-toggle="dropdown" class="btn btn-default"><span id="searchtype"><?php echo $a_sales_list->BasicSearch->getTypeNameShort() ?></span><span class="caret"></span></button>
		<ul class="dropdown-menu pull-right" role="menu">
			<li<?php if ($a_sales_list->BasicSearch->getType() == "") echo " class=\"active\""; ?>><a href="javascript:void(0);" onclick="ew_SetSearchType(this)"><?php echo $Language->Phrase("QuickSearchAuto") ?></a></li>
			<li<?php if ($a_sales_list->BasicSearch->getType() == "=") echo " class=\"active\""; ?>><a href="javascript:void(0);" onclick="ew_SetSearchType(this,'=')"><?php echo $Language->Phrase("QuickSearchExact") ?></a></li>
			<li<?php if ($a_sales_list->BasicSearch->getType() == "AND") echo " class=\"active\""; ?>><a href="javascript:void(0);" onclick="ew_SetSearchType(this,'AND')"><?php echo $Language->Phrase("QuickSearchAll") ?></a></li>
			<li<?php if ($a_sales_list->BasicSearch->getType() == "OR") echo " class=\"active\""; ?>><a href="javascript:void(0);" onclick="ew_SetSearchType(this,'OR')"><?php echo $Language->Phrase("QuickSearchAny") ?></a></li>
		</ul>
...
<?php } ?>
<?php } ?>
<?php $a_sales_list->ShowPageHeader(); ?>
<?php
$a_sales_list->ShowMessage();
?>
...
<?php if ($a_sales_list->TotalRecs == 0) { ?>
<div class="ewGrid">
<div class="ewGridUpperPanel" style="height:40px;">
<?php if ($a_sales_list->TotalRecs == 0 && $a_sales->CurrentAction == "") { // Show other options ?>
<div class="ewListOtherOptions">
<?php
	foreach ($a_sales_list->OtherOptions as &$option) {
		$option->ButtonClass = "";
		$option->Render("body", "");
	}
?>
...
<div id="gmp_a_sales_empty_table" class="<?php if (ew_IsResponsiveLayout()) { echo "table-responsive "; } ?>ewGridMiddlePanel">
<table id="tbl_a_saleslist" class="table ewTable">
...
<?php if ($a_sales->Sales_Number->Visible) { // Sales_Number ?>
	...
	<?php } else { ?>
		<th data-name="Sales_Number"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $a_sales->SortUrl($a_sales->Sales_Number) ?>',1);"><div id="elh_a_sales_Sales_Number" class="a_sales_Sales_Number">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $a_sales->Sales_Number->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></span><span class="ewTableHeaderSort"><?php if ($a_sales->Sales_Number->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($a_sales->Sales_Number->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
        </div></div></th>
	<?php } ?>
<?php } ?>		
<?php if ($a_sales->Sales_Date->Visible) { // Sales_Date ?>
	...
	<?php } else { ?>
		<th data-name="Sales_Date"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $a_sales->SortUrl($a_sales->Sales_Date) ?>',1);"><div id="elh_a_sales_Sales_Date" class="a_sales_Sales_Date">
...
<?php if ($a_sales->Customer_ID->Visible) { // Customer_ID ?>
	...
	<?php } else { ?>
		<th data-name="Customer_ID"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $a_sales->SortUrl($a_sales->Customer_ID) ?>',1);"><div id="elh_a_sales_Customer_ID" class="a_sales_Customer_ID">
...
<?php if ($a_sales->Total_Amount->Visible) { // Total_Amount ?>
	<?php if ($a_sales->SortUrl($a_sales->Total_Amount) == "") { ?>
		<th data-name="Total_Amount"><div id="elh_a_sales_Total_Amount" class="a_sales_Total_Amount"><div class="ewTableHeaderCaption"><?php echo $a_sales->Total_Amount->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="Total_Amount"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $a_sales->SortUrl($a_sales->Total_Amount) ?>',1);"><div id="elh_a_sales_Total_Amount" class="a_sales_Total_Amount">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $a_sales->Total_Amount->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($a_sales->Total_Amount->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($a_sales->Total_Amount->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
        </div></div></th>
	<?php } ?>
<?php } ?>		
<?php if ($a_sales->Discount_Amount->Visible) { // Discount_Amount ?>
	<?php if ($a_sales->SortUrl($a_sales->Discount_Amount) == "") { ?>
		<th data-name="Discount_Amount"><div id="elh_a_sales_Discount_Amount" class="a_sales_Discount_Amount"><div class="ewTableHeaderCaption"><?php echo $a_sales->Discount_Amount->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="Discount_Amount"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $a_sales->SortUrl($a_sales->Discount_Amount) ?>',1);"><div id="elh_a_sales_Discount_Amount" class="a_sales_Discount_Amount">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $a_sales->Discount_Amount->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($a_sales->Discount_Amount->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($a_sales->Discount_Amount->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
        </div></div></th>
	<?php } ?>
<?php } ?>		
<?php if ($a_sales->Tax_Amount->Visible) { // Tax_Amount ?>
	<?php if ($a_sales->SortUrl($a_sales->Tax_Amount) == "") { ?>
		<th data-name="Tax_Amount"><div id="elh_a_sales_Tax_Amount" class="a_sales_Tax_Amount"><div class="ewTableHeaderCaption"><?php echo $a_sales->Tax_Amount->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="Tax_Amount"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $a_sales->SortUrl($a_sales->Tax_Amount) ?>',1);"><div id="elh_a_sales_Tax_Amount" class="a_sales_Tax_Amount">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $a_sales->Tax_Amount->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($a_sales->Tax_Amount->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($a_sales->Tax_Amount->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
        </div></div></th>
	<?php } ?>
<?php } ?>		
<?php if ($a_sales->Final_Total_Amount->Visible) { // Final_Total_Amount ?>
	<?php if ($a_sales->SortUrl($a_sales->Final_Total_Amount) == "") { ?>
		<th data-name="Final_Total_Amount"><div id="elh_a_sales_Final_Total_Amount" class="a_sales_Final_Total_Amount"><div class="ewTableHeaderCaption"><?php echo $a_sales->Final_Total_Amount->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="Final_Total_Amount"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $a_sales->SortUrl($a_sales->Final_Total_Amount) ?>',1);"><div id="elh_a_sales_Final_Total_Amount" class="a_sales_Final_Total_Amount">
...
<?php if ($a_sales->Total_Payment->Visible) { // Total_Payment ?>
	...
	<?php } else { ?>
		<th data-name="Total_Payment"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $a_sales->SortUrl($a_sales->Total_Payment) ?>',1);"><div id="elh_a_sales_Total_Payment" class="a_sales_Total_Payment">
...
<?php if ($a_sales->Total_Balance->Visible) { // Total_Balance ?>
	...
	<?php } else { ?>
		<th data-name="Total_Balance"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $a_sales->SortUrl($a_sales->Total_Balance) ?>',1);"><div id="elh_a_sales_Total_Balance" class="a_sales_Total_Balance">
...
<a id="<?php echo $a_sales_list->PageObjName . "_row_" . $a_sales_list->RowCnt ?>"></a></td>
	<?php } ?>
	<?php if ($a_sales->Sales_Date->Visible) { // Sales_Date ?>
...
<tfoot><!-- Table footer -->
	...
	<?php if ($a_sales->Sales_Number->Visible) { // Sales_Number ?>
		<td data-name="Sales_Number"><span id="elf_a_sales_Sales_Number" class="a_sales_Sales_Number">
		&nbsp;
		</span></td>
	<?php } ?>
	<?php if ($a_sales->Sales_Date->Visible) { // Sales_Date ?>
		<td data-name="Sales_Date"><span id="elf_a_sales_Sales_Date" class="a_sales_Sales_Date">
		&nbsp;
		</span></td>
	<?php } ?>
	<?php if ($a_sales->Customer_ID->Visible) { // Customer_ID ?>
		<td data-name="Customer_ID"><span id="elf_a_sales_Customer_ID" class="a_sales_Customer_ID">
		&nbsp;
		</span></td>
	<?php } ?>
	<?php if ($a_sales->Total_Amount->Visible) { // Total_Amount ?>
		<td data-name="Total_Amount"><span id="elf_a_sales_Total_Amount" class="a_sales_Total_Amount">
<span class="ewAggregate"><?php echo $Language->Phrase("TOTAL") ?></span>
<?php echo $a_sales->Total_Amount->ViewValue ?>
		</span></td>
	<?php } ?>
	<?php if ($a_sales->Discount_Amount->Visible) { // Discount_Amount ?>
		<td data-name="Discount_Amount"><span id="elf_a_sales_Discount_Amount" class="a_sales_Discount_Amount">
<span class="ewAggregate"><?php echo $Language->Phrase("TOTAL") ?></span>
<?php echo $a_sales->Discount_Amount->ViewValue ?>
		</span></td>
	<?php } ?>
	<?php if ($a_sales->Tax_Amount->Visible) { // Tax_Amount ?>
		<td data-name="Tax_Amount"><span id="elf_a_sales_Tax_Amount" class="a_sales_Tax_Amount">
<span class="ewAggregate"><?php echo $Language->Phrase("TOTAL") ?></span>
<?php echo $a_sales->Tax_Amount->ViewValue ?>
		</span></td>
	<?php } ?>
	<?php if ($a_sales->Final_Total_Amount->Visible) { // Final_Total_Amount ?>
		<td data-name="Final_Total_Amount"><span id="elf_a_sales_Final_Total_Amount" class="a_sales_Final_Total_Amount">
<span class="ewAggregate"><?php echo $Language->Phrase("TOTAL") ?></span>
<?php echo $a_sales->Final_Total_Amount->ViewValue ?>
		</span></td>
	<?php } ?>
	<?php if ($a_sales->Total_Payment->Visible) { // Total_Payment ?>
		<td data-name="Total_Payment"><span id="elf_a_sales_Total_Payment" class="a_sales_Total_Payment">
<span class="ewAggregate"><?php echo $Language->Phrase("TOTAL") ?></span>
<?php echo $a_sales->Total_Payment->ViewValue ?>
		</span></td>
	<?php } ?>
	<?php if ($a_sales->Total_Balance->Visible) { // Total_Balance ?>
		<td data-name="Total_Balance"><span id="elf_a_sales_Total_Balance" class="a_sales_Total_Balance">
<span class="ewAggregate"><?php echo $Language->Phrase("TOTAL") ?></span>
...
<?php //////////////////////////// END Empty Table ?>
<?php if ($a_sales_list->TotalRecs > 0 || $a_sales->CurrentAction <> "") { ?>
<div class="ewGrid">
...
<?php if ($a_sales->CurrentAction <> "gridadd" && $a_sales->CurrentAction <> "gridedit") { ?>
...
?>
<option value="<?php echo $thisDisplayRecs; ?>"<?php if ($a_sales_list->DisplayRecs == $thisValue) { ?> selected="selected"<?php } ?>><?php echo $thisDisplayRecs; ?></option>
<?php	} else { ?>
<option value="ALL"<?php if ($a_sales->getRecordsPerPage() == -1) { ?> selected="selected"<?php } ?>><?php echo $Language->Phrase("AllRecords") ?></option>
<?php
		}
	}
?>
...
<?php } ?>
	<?php if (MS_PAGINATION_STYLE==1) { // link ?>
		<?php if (!isset($a_sales_list->Pager)) $a_sales_list->Pager = new cNumericPager($a_sales_list->StartRec, $a_sales_list->DisplayRecs, $a_sales_list->TotalRecs, $a_sales_list->RecRange) ?>
		<?php if ($a_sales_list->Pager->RecordCount > 0) { ?>
				<?php if (($a_sales_list->Pager->PageCount==1) && ($a_sales_list->Pager->CurrentPage == 1) && (MS_SHOW_PAGENUM_IF_REC_NOT_OVER_PAGESIZE==FALSE)  ) { ?>
				<?php } else { // MS_SHOW_PAGENUM_IF_REC_NOT_OVER_PAGESIZE ?>
				<div class="ewPager">
				<div class="ewNumericPage"><ul class="pagination">
					<?php if ($a_sales_list->Pager->FirstButton->Enabled) { ?>
					<?php if ($Language->Phrase("dir") == "rtl") { // begin of rtl ?>
					<li><a href="<?php echo $a_sales_list->PageUrl() ?>start=<?php echo $a_sales_list->Pager->FirstButton->Start ?>"><?php echo $Language->Phrase("PagerLast") ?></a></li>
					<?php } else { // else of rtl ?>
					<li><a href="<?php echo $a_sales_list->PageUrl() ?>start=<?php echo $a_sales_list->Pager->FirstButton->Start ?>"><?php echo $Language->Phrase("PagerFirst") ?></a></li>
					<?php } // end of rtl ?>
					<?php } ?>
					<?php if ($a_sales_list->Pager->PrevButton->Enabled) { ?>
					<?php if ($Language->Phrase("dir") == "rtl") { // begin of rtl ?>
					<li><a href="<?php echo $a_sales_list->PageUrl() ?>start=<?php echo $a_sales_list->Pager->PrevButton->Start ?>"><?php echo $Language->Phrase("PagerNext") ?></a></li>
					<?php } else { // else of rtl { ?>
					<li><a href="<?php echo $a_sales_list->PageUrl() ?>start=<?php echo $a_sales_list->Pager->PrevButton->Start ?>"><?php echo $Language->Phrase("PagerPrevious") ?></a></li>
					<?php } // end of rtl { ?>
					<?php } ?>
					<?php foreach ($a_sales_list->Pager->Items as $PagerItem) { ?>
						<li<?php if (!$PagerItem->Enabled) { echo " class=\" active\""; } ?>><a href="<?php if ($PagerItem->Enabled) { echo $a_sales_list->PageUrl() . "start=" . $PagerItem->Start; } else { echo "#"; } ?>"><?php echo $PagerItem->Text ?></a></li>
					<?php } ?>
					<?php if ($a_sales_list->Pager->NextButton->Enabled) { ?>
					<?php if ($Language->Phrase("dir") == "rtl") { // begin of rtl ?>
					<li><a href="<?php echo $a_sales_list->PageUrl() ?>start=<?php echo $a_sales_list->Pager->NextButton->Start ?>"><?php echo $Language->Phrase("PagerPrevious") ?></a></li>
					<?php } else { // else of rtl ?>
					<li><a href="<?php echo $a_sales_list->PageUrl() ?>start=<?php echo $a_sales_list->Pager->NextButton->Start ?>"><?php echo $Language->Phrase("PagerNext") ?></a></li>
					<?php } // end of rtl ?>
					<?php } ?>
					<?php if ($a_sales_list->Pager->LastButton->Enabled) { ?>
					<?php if ($Language->Phrase("dir") == "rtl") { // begin of rtl ?>
					<li><a href="<?php echo $a_sales_list->PageUrl() ?>start=<?php echo $a_sales_list->Pager->LastButton->Start ?>"><?php echo $Language->Phrase("PagerFirst") ?></a></li>
					<?php } else { // else of rtl ?>
					<li><a href="<?php echo $a_sales_list->PageUrl() ?>start=<?php echo $a_sales_list->Pager->LastButton->Start ?>"><?php echo $Language->Phrase("PagerLast") ?></a></li>
					<?php } // end of rtl ?>
					<?php } ?>
				</ul></div>
				</div>
				...
				<div class="ewPager ewRec">
					<span><?php echo $Language->Phrase("Record") ?>&nbsp;<?php echo $a_sales_list->Pager->FromIndex ?>&nbsp;<?php echo $Language->Phrase("To") ?>&nbsp;<?php echo $a_sales_list->Pager->ToIndex ?>&nbsp;<?php echo $Language->Phrase("Of") ?>&nbsp;<?php echo $a_sales_list->Pager->RecordCount ?></span>
				</div>
		<?php } ?>	
	<?php } elseif (MS_PAGINATION_STYLE==2) { // button ?>
		<?php if (!isset($a_sales_list->Pager)) $a_sales_list->Pager = new cPrevNextPager($a_sales_list->StartRec, $a_sales_list->DisplayRecs, $a_sales_list->TotalRecs) ?>
		<?php if ($a_sales_list->Pager->RecordCount > 0) { ?>
				<?php if (($a_sales_list->Pager->PageCount==1) && ($a_sales_list->Pager->CurrentPage == 1) && (MS_SHOW_PAGENUM_IF_REC_NOT_OVER_PAGESIZE==FALSE)  ) { ?>
				...
				<!--first page button-->
					<?php if ($a_sales_list->Pager->FirstButton->Enabled) { ?>
					<?php if ($Language->Phrase("dir") == "rtl") { // begin of rtl ?>
					<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerFirst") ?>" href="<?php echo $a_sales_list->PageUrl() ?>start=<?php echo $a_sales_list->Pager->FirstButton->Start ?>"><span class="icon-last ewIcon"></span></a>
					<?php } else { // else of rtl ?>
					<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerFirst") ?>" href="<?php echo $a_sales_list->PageUrl() ?>start=<?php echo $a_sales_list->Pager->FirstButton->Start ?>"><span class="icon-first ewIcon"></span></a>
					<?php } // end of rtl ?>
					<?php } else { ?>
					<?php if ($Language->Phrase("dir") == "rtl") { // begin of rtl ?>
				...
				<!--previous page button-->
					<?php if ($a_sales_list->Pager->PrevButton->Enabled) { ?>
					<?php if ($Language->Phrase("dir") == "rtl") { // begin of rtl ?>
					<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerPrevious") ?>" href="<?php echo $a_sales_list->PageUrl() ?>start=<?php echo $a_sales_list->Pager->PrevButton->Start ?>"><span class="icon-next ewIcon"></span></a>
					<?php } else { // else of rtl ?>
					<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerPrevious") ?>" href="<?php echo $a_sales_list->PageUrl() ?>start=<?php echo $a_sales_list->Pager->PrevButton->Start ?>"><span class="icon-prev ewIcon"></span></a>
					<?php } // end of rtl ?>
					<?php } else { ?>
					<?php if ($Language->Phrase("dir") == "rtl") { // begin of rtl ?>
				...
				<!--current page number-->
					<input class="form-control input-sm" type="text" name="<?php echo EW_TABLE_PAGE_NO ?>" value="<?php echo $a_sales_list->Pager->CurrentPage ?>">
				<div class="input-group-btn">
				<!--next page button-->
					<?php if ($a_sales_list->Pager->NextButton->Enabled) { ?>
					<?php if ($Language->Phrase("dir") == "rtl") { // begin of rtl ?>
					<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerNext") ?>" href="<?php echo $a_sales_list->PageUrl() ?>start=<?php echo $a_sales_list->Pager->NextButton->Start ?>"><span class="icon-prev ewIcon"></span></a>
					<?php } else { // else of rtl ?>
					<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerNext") ?>" href="<?php echo $a_sales_list->PageUrl() ?>start=<?php echo $a_sales_list->Pager->NextButton->Start ?>"><span class="icon-next ewIcon"></span></a>
					<?php } // end of rtl ?>
					<?php } else { ?>
					<?php if ($Language->Phrase("dir") == "rtl") { // begin of rtl ?>
				...
				<!--last page button-->
					<?php if ($a_sales_list->Pager->LastButton->Enabled) { ?>
					<?php if ($Language->Phrase("dir") == "rtl") { // begin of rtl ?>
					<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerLast") ?>" href="<?php echo $a_sales_list->PageUrl() ?>start=<?php echo $a_sales_list->Pager->LastButton->Start ?>"><span class="icon-first ewIcon"></span></a>
					<?php } else { // else of rtl ?>
					<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerLast") ?>" href="<?php echo $a_sales_list->PageUrl() ?>start=<?php echo $a_sales_list->Pager->LastButton->Start ?>"><span class="icon-last ewIcon"></span></a>
					<?php } // end of rtl ?>
					<?php } else { ?>
					<?php if ($Language->Phrase("dir") == "rtl") { // begin of rtl ?>
				...
				<span>&nbsp;<?php echo $Language->Phrase("of") ?>&nbsp;<?php echo $a_sales_list->Pager->PageCount ?></span>
				</div>
...
<input type="hidden" name="t" value="a_sales">
<select name="<?php echo EW_TABLE_REC_PER_PAGE ?>" class="form-control input-sm" onchange="this.form.submit();">
<option value="1"<?php if ($a_sales_list->DisplayRecs == 1) { ?> selected="selected"<?php } ?>>1</option>
<option value="3"<?php if ($a_sales_list->DisplayRecs == 3) { ?> selected="selected"<?php } ?>>3</option>
<option value="5"<?php if ($a_sales_list->DisplayRecs == 5) { ?> selected="selected"<?php } ?>>5</option>
<option value="10"<?php if ($a_sales_list->DisplayRecs == 10) { ?> selected="selected"<?php } ?>>10</option>
<option value="20"<?php if ($a_sales_list->DisplayRecs == 20) { ?> selected="selected"<?php } ?>>20</option>
<option value="50"<?php if ($a_sales_list->DisplayRecs == 50) { ?> selected="selected"<?php } ?>>50</option>
<option value="100"<?php if ($a_sales_list->DisplayRecs == 100) { ?> selected="selected"<?php } ?>>100</option>
</select>
</div>
...
<form name="fa_saleslist" id="fa_saleslist" class="form-inline ewForm ewListForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($a_sales_list->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $a_sales_list->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="a_sales">
...
// Render list options
$a_sales_list->RenderListOptions();

// Render list options (header, left)
$a_sales_list->ListOptions->Render("header", "left");
?>
<?php if ($a_sales->Sales_Number->Visible) { // Sales_Number ?>
	<?php if ($a_sales->SortUrl($a_sales->Sales_Number) == "") { ?>
		<th data-name="Sales_Number"><div id="elh_a_sales_Sales_Number" class="a_sales_Sales_Number"><div class="ewTableHeaderCaption"><?php echo $a_sales->Sales_Number->FldCaption() ?></div></div></th>
...
$a_sales_list->ListOptions->Render("header", "right");
?>
	</tr>
</thead>
<tbody>
...
// Begin of mofidication Flexibility of Export Records Options, by Masino Sinaga, May 14, 2012
if ($a_sales->ExportAll=="allpages" && $a_sales->Export <> "") {
    $a_sales_list->StopRec = $a_sales_list->TotalRecs;

// End of mofidication Flexibility of Export Records Options, by Masino Sinaga, May 14, 2012
} else {

	// Set the last record to display
	if ($a_sales_list->TotalRecs > $a_sales_list->StartRec + $a_sales_list->DisplayRecs - 1)
		$a_sales_list->StopRec = $a_sales_list->StartRec + $a_sales_list->DisplayRecs - 1;
	else
		$a_sales_list->StopRec = $a_sales_list->TotalRecs;
}
$a_sales_list->RecCnt = $a_sales_list->StartRec - 1;
if ($a_sales_list->Recordset && !$a_sales_list->Recordset->EOF) {
	$a_sales_list->Recordset->MoveFirst();
	$bSelectLimit = EW_SELECT_LIMIT;
	if (!$bSelectLimit && $a_sales_list->StartRec > 1)
		$a_sales_list->Recordset->Move($a_sales_list->StartRec - 1);
} elseif (!$a_sales->AllowAddDeleteRow && $a_sales_list->StopRec == 0) {
	$a_sales_list->StopRec = $a_sales->GridAddRowCount;
}

// Initialize aggregate
$a_sales->RowType = EW_ROWTYPE_AGGREGATEINIT;
$a_sales->ResetAttrs();
$a_sales_list->RenderRow();
while ($a_sales_list->RecCnt < $a_sales_list->StopRec) {
	$a_sales_list->RecCnt++;
	if (intval($a_sales_list->RecCnt) >= intval($a_sales_list->StartRec)) {
		$a_sales_list->RowCnt++;

		// Set up key count
		$a_sales_list->KeyCount = $a_sales_list->RowIndex;
		...
		if ($a_sales->CurrentAction == "gridadd") {
			...
			$a_sales_list->LoadRowValues($a_sales_list->Recordset); // Load row values
		}
		$a_sales->RowType = EW_ROWTYPE_VIEW; // Render view

		// Set up row id / data-rowindex
		$a_sales->RowAttrs = array_merge($a_sales->RowAttrs, array('data-rowindex'=>$a_sales_list->RowCnt, 'id'=>'r' . $a_sales_list->RowCnt . '_a_sales', 'data-rowtype'=>$a_sales->RowType));
...
$a_sales_list->ListOptions->Render("body", "left", $a_sales_list->RowCnt);
?>
	<?php if ($a_sales->Sales_Number->Visible) { // Sales_Number ?>
		<td data-name="Sales_Number"<?php echo $a_sales->Sales_Number->CellAttributes() ?>>
<span<?php echo $a_sales->Sales_Number->ViewAttributes() ?>>
...
$a_sales_list->ListOptions->Render("body", "right", $a_sales_list->RowCnt);
?>
	</tr>
<?php
	}
	if ($a_sales->CurrentAction <> "gridadd")
		$a_sales_list->Recordset->MoveNext();
}
?>
</tbody>
<?php
...
$a_sales->RowType = EW_ROWTYPE_AGGREGATE;
$a_sales->ResetAttrs();
$a_sales_list->RenderRow();
?>
<?php if ($a_sales_list->TotalRecs > 0 && ($a_sales->CurrentAction <> "gridadd" && $a_sales->CurrentAction <> "gridedit")) { ?>
<tfoot><!-- Table footer -->
	<tr class="ewTableFooter">
<?php
...
$a_sales_list->ListOptions->Render("footer", "left");
?>
	<?php if ($a_sales->Sales_Number->Visible) { // Sales_Number ?>
		<td data-name="Sales_Number"><span id="elf_a_sales_Sales_Number" class="a_sales_Sales_Number">
		&nbsp;
...
$a_sales_list->ListOptions->Render("footer", "right");
?>
	</tr>
</tfoot>	
<?php } ?>
...
// Close recordset
if ($a_sales_list->Recordset)
	$a_sales_list->Recordset->Close();
?>
<?php // Begin of modification Customize Navigation/Pager Panel, by Masino Sinaga, May 2, 2012 ?>
...
<?php if ($a_sales->Export == "") { ?>
<script type="text/javascript">
fa_saleslistsrch.Init();
fa_saleslist.Init();
</script>
<?php } ?>
<?php
$a_sales_list->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<?php if ($a_sales->Export == "") { ?>
...
<?php if (MS_USE_TABLE_SETTING_FOR_SEARCH_PANEL_COLLAPSED) { ?>
<?php if (isset($_SESSION['table_a_sales_views']) && $_SESSION['table_a_sales_views'] == 1) { ?>
...
<script type="text/javascript">
$(document).ready(function() { var SearchToggle = $('.ewSearchToggle'); var SearchPanel = $('.ewSearchPanel'); if(getCookie('a_sales_searchpanel')=="active"){ SearchToggle.addClass(getCookie('a_sales_searchpanel')); SearchPanel.addClass('in'); SearchToggle.addClass('active'); }else{ SearchPanel.removeClass('in'); SearchToggle.removeClass('active'); } SearchToggle.on('click',function(event) { event.preventDefault(); if (SearchToggle.hasClass('active')){ createCookie("a_sales_searchpanel", "notactive", 1); }else{ createCookie("a_sales_searchpanel", "active", 1); } }); });
</script>
...
<?php if ($a_sales->Export == "") { ?>
<script type="text/javascript">
$('.ewGridSave, .ewGridInsert').attr('onclick', 'return alertifySaveGrid(this)'); function alertifySaveGrid(obj) { <?php global $Language; ?> if (fa_saleslist.Validate() == true ) { alertify.set({buttonFocus: 'cancel'});alertify.confirm("<?php echo $Language->Phrase('AlertifySaveGridConfirm'); ?>", function (e) { if (e) { $(window).unbind('beforeunload'); alertify.success("<?php echo $Language->Phrase('AlertifySaveGrid'); ?>"); $("#fa_saleslist").submit(); } else { alertify.error("<?php echo $Language->Phrase('AlertifyCancel'); ?>"); } }, "<?php echo $Language->Phrase('AlertifyConfirm'); ?>"); } return false; }
</script>
<script type="text/javascript">
$('.ewInlineUpdate').attr('onclick', 'return alertifySaveInlineEdit(this)'); function alertifySaveInlineEdit(obj) { <?php global $Language; ?> if (fa_saleslist.Validate() == true ) { alertify.set({buttonFocus: 'cancel'});alertify.confirm("<?php echo $Language->Phrase('AlertifySaveGridConfirm'); ?>", function (e) { if (e) { $(window).unbind('beforeunload'); alertify.success("<?php echo $Language->Phrase('AlertifySaveGrid'); ?>"); $("#fa_saleslist").submit(); } else { alertify.error("<?php echo $Language->Phrase('AlertifyCancel'); ?>"); } }, "<?php echo $Language->Phrase('AlertifyConfirm'); ?>"); } return false; }
</script>
<script type="text/javascript">
$('.ewInlineInsert').attr('onclick', 'return alertifySaveInlineInsert(this)'); function alertifySaveInlineInsert(obj) { <?php global $Language; ?> if (fa_saleslist.Validate() == true ) { alertify.set({buttonFocus: 'cancel'});alertify.confirm("<?php echo $Language->Phrase('AlertifySaveGridConfirm'); ?>", function (e) { if (e) { $(window).unbind('beforeunload'); alertify.success("<?php echo $Language->Phrase('AlertifySaveGrid'); ?>"); $("#fa_saleslist").submit(); } else { alertify.error("<?php echo $Language->Phrase('AlertifyCancel'); ?>"); } }, "<?php echo $Language->Phrase('AlertifyConfirm'); ?>"); } return false; }
</script>
<?php } ?>
<?php if ($a_sales->CurrentAction == "" || $a_sales->Export == "") { // Change && become || in order to add scroll table in Grid, by Masino Sinaga, August 3, 2014 ?>
<script type="text/javascript">
...
jQuery(function(){ew_ScrollableTable("gmp_a_sales", "<?php echo MS_SCROLL_TABLE_WIDTH; ?>px", "<?php echo MS_SCROLL_TABLE_HEIGHT; ?>px");});
jQuery(function(){ew_ScrollableTable("gmp_a_sales_empty_table", "<?php echo MS_SCROLL_TABLE_WIDTH; ?>px", "<?php echo MS_SCROLL_TABLE_HEIGHT; ?>px");});
...
jQuery(function(){ew_ScrollableTable("gmp_a_sales", "100%", "<?php echo MS_SCROLL_TABLE_HEIGHT; ?>px");});
jQuery(function(){ew_ScrollableTable("gmp_a_sales_empty_table", "100%", "<?php echo MS_SCROLL_TABLE_HEIGHT; ?>px");});
