	...
	function Page_Main() {
		...
		if ( isset($_GET["PageTitleParent"]) && isset($_GET["PageTitle"]) ) {
			if ( ($PageTitleParent != "") && ($PageTitle != "") ) {	
			  ...
			  if ( $value==0 ) {
				if (EW_IS_MYSQL) {
				  $rsb = $conn->Execute("CALL ".MS_BREADCRUMB_LINKS_ADD_SP."('".$PageTitleParent."','".$PageTitle."','".$PageURL."')");
				} else {
				  $rsb = $conn->Execute("".MS_BREADCRUMB_LINKS_ADD_SP."('".$PageTitleParent."','".$PageTitle."','".$PageURL."')");
				}
