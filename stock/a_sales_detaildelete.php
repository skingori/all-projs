...
$a_sales_detail_delete = NULL; // Initialize page object first

class ca_sales_detail_delete extends ca_sales_detail {
	...
	// Page object name
	var $PageObjName = 'a_sales_detail_delete';
...
// End of modification Disable/Enable Registration Page, by Masino Sinaga, May 14, 2012
	...
	function Page_Main() {
		...
		if ($sFilter == "")
			$this->Page_Terminate("a_sales_detaillist.php"); // Prevent SQL injection, return to list

		// Set up filter (SQL WHHERE clause) and get return SQL
		// SQL constructor in a_sales_detail class, a_sales_detailinfo.php
...
// Create page object
if (!isset($a_sales_detail_delete)) $a_sales_detail_delete = new ca_sales_detail_delete();

// Page init
$a_sales_detail_delete->Page_Init();

// Page main
$a_sales_detail_delete->Page_Main();
...
$a_sales_detail_delete->Page_Render();
?>
<?php include_once "header.php" ?>
<script type="text/javascript">

// Page object
var a_sales_detail_delete = new ew_Page("a_sales_detail_delete");
a_sales_detail_delete.PageID = "delete"; // Page ID
var EW_PAGE_ID = a_sales_detail_delete.PageID; // For backward compatibility

// Form object
var fa_sales_detaildelete = new ew_Form("fa_sales_detaildelete");

// Form_CustomValidate event
fa_sales_detaildelete.Form_CustomValidate = 
...
fa_sales_detaildelete.ValidateRequired = true;
<?php } else { ?>
fa_sales_detaildelete.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
fa_sales_detaildelete.Lists["x_Supplier_Number"] = {"LinkField":"x_Supplier_Number","Ajax":null,"AutoFill":false,"DisplayFields":["x_Supplier_Name","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
fa_sales_detaildelete.Lists["x_Stock_Item"] = {"LinkField":"x_Stock_Number","Ajax":null,"AutoFill":false,"DisplayFields":["x_Stock_Name","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
...
if ($a_sales_detail_delete->Recordset = $a_sales_detail_delete->LoadRecordset())
	$a_sales_detail_deleteTotalRecs = $a_sales_detail_delete->Recordset->RecordCount(); // Get record count
if ($a_sales_detail_deleteTotalRecs <= 0) { // No record found, exit
	if ($a_sales_detail_delete->Recordset)
		$a_sales_detail_delete->Recordset->Close();
	$a_sales_detail_delete->Page_Terminate("a_sales_detaillist.php"); // Return to list
}
?>
<div class="ewToolbar">
...
<?php $a_sales_detail_delete->ShowPageHeader(); ?>
<?php
$a_sales_detail_delete->ShowMessage();
?>
<form name="fa_sales_detaildelete" id="fa_sales_detaildelete" class="form-inline ewForm ewDeleteForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($a_sales_detail_delete->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $a_sales_detail_delete->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="a_sales_detail">
<input type="hidden" name="a_delete" id="a_delete" value="D">
<?php foreach ($a_sales_detail_delete->RecKeys as $key) { ?>
...
<?php if ($a_sales_detail->Sales_Number->Visible) { // Sales_Number ?>
		<th><span id="elh_a_sales_detail_Sales_Number" class="a_sales_detail_Sales_Number"><?php echo $a_sales_detail->Sales_Number->FldCaption() ?></span></th>
<?php } ?>
<?php if ($a_sales_detail->Supplier_Number->Visible) { // Supplier_Number ?>
		<th><span id="elh_a_sales_detail_Supplier_Number" class="a_sales_detail_Supplier_Number"><?php echo $a_sales_detail->Supplier_Number->FldCaption() ?></span></th>
<?php } ?>
<?php if ($a_sales_detail->Stock_Item->Visible) { // Stock_Item ?>
		<th><span id="elh_a_sales_detail_Stock_Item" class="a_sales_detail_Stock_Item"><?php echo $a_sales_detail->Stock_Item->FldCaption() ?></span></th>
<?php } ?>
<?php if ($a_sales_detail->Sales_Quantity->Visible) { // Sales_Quantity ?>
		<th><span id="elh_a_sales_detail_Sales_Quantity" class="a_sales_detail_Sales_Quantity"><?php echo $a_sales_detail->Sales_Quantity->FldCaption() ?></span></th>
<?php } ?>
<?php if ($a_sales_detail->Purchasing_Price->Visible) { // Purchasing_Price ?>
		<th><span id="elh_a_sales_detail_Purchasing_Price" class="a_sales_detail_Purchasing_Price"><?php echo $a_sales_detail->Purchasing_Price->FldCaption() ?></span></th>
<?php } ?>
<?php if ($a_sales_detail->Sales_Price->Visible) { // Sales_Price ?>
		<th><span id="elh_a_sales_detail_Sales_Price" class="a_sales_detail_Sales_Price"><?php echo $a_sales_detail->Sales_Price->FldCaption() ?></span></th>
<?php } ?>
<?php if ($a_sales_detail->Sales_Total_Amount->Visible) { // Sales_Total_Amount ?>
		<th><span id="elh_a_sales_detail_Sales_Total_Amount" class="a_sales_detail_Sales_Total_Amount"><?php echo $a_sales_detail->Sales_Total_Amount->FldCaption() ?></span></th>
<?php } ?>
	</tr>
	</thead>
	<tbody>
<?php
$a_sales_detail_delete->RecCnt = 0;
$i = 0;
while (!$a_sales_detail_delete->Recordset->EOF) {
	$a_sales_detail_delete->RecCnt++;
	$a_sales_detail_delete->RowCnt++;

	// Set row properties
	$a_sales_detail->ResetAttrs();
	$a_sales_detail->RowType = EW_ROWTYPE_VIEW; // View

	// Get the field contents
	$a_sales_detail_delete->LoadRowValues($a_sales_detail_delete->Recordset);

	// Render row
	$a_sales_detail_delete->RenderRow();
?>
	<tr<?php echo $a_sales_detail->RowAttributes() ?>>
<?php if ($a_sales_detail->Sales_Number->Visible) { // Sales_Number ?>
		<td<?php echo $a_sales_detail->Sales_Number->CellAttributes() ?>>
<span id="el<?php echo $a_sales_detail_delete->RowCnt ?>_a_sales_detail_Sales_Number" class="form-group a_sales_detail_Sales_Number">
...
<span id="el<?php echo $a_sales_detail_delete->RowCnt ?>_a_sales_detail_Supplier_Number" class="form-group a_sales_detail_Supplier_Number">
...
<span id="el<?php echo $a_sales_detail_delete->RowCnt ?>_a_sales_detail_Stock_Item" class="form-group a_sales_detail_Stock_Item">
...
<span id="el<?php echo $a_sales_detail_delete->RowCnt ?>_a_sales_detail_Sales_Quantity" class="form-group a_sales_detail_Sales_Quantity">
...
<span id="el<?php echo $a_sales_detail_delete->RowCnt ?>_a_sales_detail_Purchasing_Price" class="form-group a_sales_detail_Purchasing_Price">
...
<span id="el<?php echo $a_sales_detail_delete->RowCnt ?>_a_sales_detail_Sales_Price" class="form-group a_sales_detail_Sales_Price">
...
<span id="el<?php echo $a_sales_detail_delete->RowCnt ?>_a_sales_detail_Sales_Total_Amount" class="form-group a_sales_detail_Sales_Total_Amount">
...
<?php
	$a_sales_detail_delete->Recordset->MoveNext();
}
$a_sales_detail_delete->Recordset->Close();
?>
</tbody>
...
<script type="text/javascript">
fa_sales_detaildelete.Init();
</script>
<?php
$a_sales_detail_delete->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<script type="text/javascript">
$('#btnAction').attr('onclick', 'return alertifyDelete(this)'); function alertifyDelete(obj) { <?php global $Language; ?> if (fa_sales_detaildelete.Validate() == true ) { alertify.set({buttonFocus: 'cancel'});alertify.confirm("<?php echo  $Language->Phrase('AlertifyDeleteConfirm'); ?>", function (e) { if (e) {	$(window).unbind('beforeunload'); alertify.success("<?php echo $Language->Phrase('AlertifyDelete'); ?>"); $("#fa_sales_detaildelete").submit(); } else { alertify.error("<?php echo $Language->Phrase('AlertifyCancel'); ?>"); } }, "<?php echo $Language->Phrase('AlertifyConfirm'); ?>"); } return false; }
</script>
<?php } ?>
<?php include_once "footer.php" ?>
<?php
$a_sales_detail_delete->Page_Terminate();
?>
