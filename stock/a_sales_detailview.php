...
$a_sales_detail_view = NULL; // Initialize page object first

class ca_sales_detail_view extends ca_sales_detail {
	...
	// Page object name
	var $PageObjName = 'a_sales_detail_view';
...
// End of modification Disable/Enable Registration Page, by Masino Sinaga, May 14, 2012
	...
	function Page_Main() {
		...
		if ($this->IsPageRequest()) { // Validate request
			...
			switch ($this->CurrentAction) {
				case "I": // Get a record to display
					...
					if (!$bMatchRecord) {
						if ($this->getSuccessMessage() == "" && $this->getFailureMessage() == "")
							$this->setFailureMessage($Language->Phrase("NoRecord")); // Set no record message
						$sReturnUrl = "a_sales_detaillist.php"; // No matching record, return to list
					} else {
			...
			$sReturnUrl = "a_sales_detaillist.php"; // Not page request, return to list
		}
		if ($sReturnUrl <> "")
...
// Begin of modification Permission Access for Export To Feature, by Masino Sinaga, May 5, 2012
	...
	function SetupBreadcrumb() {
		...
		$Breadcrumb->Add("list", $this->TableVar, "a_sales_detaillist.php", "", $this->TableVar, TRUE);
		$PageId = "view";
...
// Create page object
if (!isset($a_sales_detail_view)) $a_sales_detail_view = new ca_sales_detail_view();

// Page init
$a_sales_detail_view->Page_Init();

// Page main
$a_sales_detail_view->Page_Main();
...
$a_sales_detail_view->Page_Render();
?>
...
// Page object
var a_sales_detail_view = new ew_Page("a_sales_detail_view");
a_sales_detail_view.PageID = "view"; // Page ID
var EW_PAGE_ID = a_sales_detail_view.PageID; // For backward compatibility

// Form object
var fa_sales_detailview = new ew_Form("fa_sales_detailview");

// Form_CustomValidate event
fa_sales_detailview.Form_CustomValidate = 
...
fa_sales_detailview.ValidateRequired = true;
<?php } else { ?>
fa_sales_detailview.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
fa_sales_detailview.Lists["x_Supplier_Number"] = {"LinkField":"x_Supplier_Number","Ajax":null,"AutoFill":false,"DisplayFields":["x_Supplier_Name","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
fa_sales_detailview.Lists["x_Stock_Item"] = {"LinkField":"x_Stock_Number","Ajax":null,"AutoFill":false,"DisplayFields":["x_Stock_Name","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
...
<?php } ?>
<?php } ?>
<?php $a_sales_detail_view->ExportOptions->Render("body") ?>
<?php
	foreach ($a_sales_detail_view->OtherOptions as &$option)
		$option->Render("body");
?>
<?php if ($a_sales_detail->Export == "") { ?>
...
<?php } ?>
<?php $a_sales_detail_view->ShowPageHeader(); ?>
<?php
$a_sales_detail_view->ShowMessage();
?>
...
<form name="ewPagerForm" class="form-inline ewForm ewPagerForm" action="<?php echo ew_CurrentPage() ?>">
	<?php if (MS_PAGINATION_STYLE==1) { // link ?>
		<?php if (!isset($a_sales_detail_view->Pager)) $a_sales_detail_view->Pager = new cNumericPager($a_sales_detail_view->StartRec, $a_sales_detail_view->DisplayRecs, $a_sales_detail_view->TotalRecs, $a_sales_detail_view->RecRange) ?>
		<?php if ($a_sales_detail_view->Pager->RecordCount > 0) { ?>
				<?php if (($a_sales_detail_view->Pager->PageCount==1) && ($a_sales_detail_view->Pager->CurrentPage == 1) && (MS_SHOW_PAGENUM_IF_REC_NOT_OVER_PAGESIZE==FALSE)  ) { ?>
				<div class="ewPager ewRec">
					<span><?php echo $Language->Phrase("Record") ?>&nbsp;<?php echo $a_sales_detail_view->Pager->FromIndex ?>&nbsp;<?php echo $Language->Phrase("To") ?>&nbsp;<?php echo $a_sales_detail_view->Pager->ToIndex ?>&nbsp;<?php echo $Language->Phrase("Of") ?>&nbsp;<?php echo $a_sales_detail_view->Pager->RecordCount ?></span>
				</div>
				<?php } else { // MS_SHOW_PAGENUM_IF_REC_NOT_OVER_PAGESIZE ?>
				<div class="ewPager">
				<div class="ewNumericPage"><ul class="pagination">
					<?php if ($a_sales_detail_view->Pager->FirstButton->Enabled) { ?>
					<?php if ($Language->Phrase("dir") == "rtl") { // begin of rtl ?>
					<li><a href="<?php echo $a_sales_detail_view->PageUrl() ?>start=<?php echo $a_sales_detail_view->Pager->FirstButton->Start ?>"><?php echo $Language->Phrase("PagerLast") ?></a></li>
					<?php } else { // else of rtl ?>
					<li><a href="<?php echo $a_sales_detail_view->PageUrl() ?>start=<?php echo $a_sales_detail_view->Pager->FirstButton->Start ?>"><?php echo $Language->Phrase("PagerFirst") ?></a></li>
					<?php } // end of rtl ?>
					<?php } ?>
					<?php if ($a_sales_detail_view->Pager->PrevButton->Enabled) { ?>
					<?php if ($Language->Phrase("dir") == "rtl") { // begin of rtl ?>
					<li><a href="<?php echo $a_sales_detail_view->PageUrl() ?>start=<?php echo $a_sales_detail_view->Pager->PrevButton->Start ?>"><?php echo $Language->Phrase("PagerNext") ?></a></li>
					<?php } else { // else of rtl { ?>
					<li><a href="<?php echo $a_sales_detail_view->PageUrl() ?>start=<?php echo $a_sales_detail_view->Pager->PrevButton->Start ?>"><?php echo $Language->Phrase("PagerPrevious") ?></a></li>
					<?php } // end of rtl { ?>
					<?php } ?>
					<?php foreach ($a_sales_detail_view->Pager->Items as $PagerItem) { ?>
						<li<?php if (!$PagerItem->Enabled) { echo " class=\" active\""; } ?>><a href="<?php if ($PagerItem->Enabled) { echo $a_sales_detail_view->PageUrl() . "start=" . $PagerItem->Start; } else { echo "#"; } ?>"><?php echo $PagerItem->Text ?></a></li>
					<?php } ?>
					<?php if ($a_sales_detail_view->Pager->NextButton->Enabled) { ?>
					<?php if ($Language->Phrase("dir") == "rtl") { // begin of rtl ?>
					<li><a href="<?php echo $a_sales_detail_view->PageUrl() ?>start=<?php echo $a_sales_detail_view->Pager->NextButton->Start ?>"><?php echo $Language->Phrase("PagerPrevious") ?></a></li>
					<?php } else { // else of rtl ?>
					<li><a href="<?php echo $a_sales_detail_view->PageUrl() ?>start=<?php echo $a_sales_detail_view->Pager->NextButton->Start ?>"><?php echo $Language->Phrase("PagerNext") ?></a></li>
					<?php } // end of rtl ?>
					<?php } ?>
					<?php if ($a_sales_detail_view->Pager->LastButton->Enabled) { ?>
					<?php if ($Language->Phrase("dir") == "rtl") { // begin of rtl ?>
					<li><a href="<?php echo $a_sales_detail_view->PageUrl() ?>start=<?php echo $a_sales_detail_view->Pager->LastButton->Start ?>"><?php echo $Language->Phrase("PagerFirst") ?></a></li>
					<?php } else { // else of rtl ?>
					<li><a href="<?php echo $a_sales_detail_view->PageUrl() ?>start=<?php echo $a_sales_detail_view->Pager->LastButton->Start ?>"><?php echo $Language->Phrase("PagerLast") ?></a></li>
					<?php } // end of rtl ?>
					<?php } ?>
				</ul></div>
				</div>
	...
	<?php } elseif (MS_PAGINATION_STYLE==2) { // button ?>
		<?php if (!isset($a_sales_detail_view->Pager)) $a_sales_detail_view->Pager = new cPrevNextPager($a_sales_detail_view->StartRec, $a_sales_detail_view->DisplayRecs, $a_sales_detail_view->TotalRecs) ?>
		<?php if ($a_sales_detail_view->Pager->RecordCount > 0) { ?>
				<?php if (($a_sales_detail_view->Pager->PageCount==1) && ($a_sales_detail_view->Pager->CurrentPage == 1) && (MS_SHOW_PAGENUM_IF_REC_NOT_OVER_PAGESIZE==FALSE)  ) { ?>
						<div class="ewPager ewRec">
							<span><?php echo $Language->Phrase("Record") ?>&nbsp;<?php echo $a_sales_detail_view->Pager->FromIndex ?>&nbsp;<?php echo $Language->Phrase("To") ?>&nbsp;<?php echo $a_sales_detail_view->Pager->ToIndex ?>&nbsp;<?php echo $Language->Phrase("Of") ?>&nbsp;<?php echo $a_sales_detail_view->Pager->RecordCount ?></span>
				...
				<!--first page button-->
					<?php if ($a_sales_detail_view->Pager->FirstButton->Enabled) { ?>
					<?php if ($Language->Phrase("dir") == "rtl") { // begin of rtl ?>
					<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerFirst") ?>" href="<?php echo $a_sales_detail_view->PageUrl() ?>start=<?php echo $a_sales_detail_view->Pager->FirstButton->Start ?>"><span class="icon-last ewIcon"></span></a>
					<?php } else { // else of rtl ?>
					<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerFirst") ?>" href="<?php echo $a_sales_detail_view->PageUrl() ?>start=<?php echo $a_sales_detail_view->Pager->FirstButton->Start ?>"><span class="icon-first ewIcon"></span></a>
					<?php } // end of rtl ?>
					<?php } else { ?>
					<?php if ($Language->Phrase("dir") == "rtl") { // begin of rtl ?>
				...
				<!--previous page button-->
					<?php if ($a_sales_detail_view->Pager->PrevButton->Enabled) { ?>
					<?php if ($Language->Phrase("dir") == "rtl") { // begin of rtl ?>
					<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerPrevious") ?>" href="<?php echo $a_sales_detail_view->PageUrl() ?>start=<?php echo $a_sales_detail_view->Pager->PrevButton->Start ?>"><span class="icon-next ewIcon"></span></a>
					<?php } else { // else of rtl ?>
					<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerPrevious") ?>" href="<?php echo $a_sales_detail_view->PageUrl() ?>start=<?php echo $a_sales_detail_view->Pager->PrevButton->Start ?>"><span class="icon-prev ewIcon"></span></a>
					<?php } // end of rtl ?>
					<?php } else { ?>
					<?php if ($Language->Phrase("dir") == "rtl") { // begin of rtl ?>
				...
				<!--current page number-->
					<input class="form-control input-sm" type="text" name="<?php echo EW_TABLE_PAGE_NO ?>" value="<?php echo $a_sales_detail_view->Pager->CurrentPage ?>">
				<div class="input-group-btn">
				<!--next page button-->
					<?php if ($a_sales_detail_view->Pager->NextButton->Enabled) { ?>
					<?php if ($Language->Phrase("dir") == "rtl") { // begin of rtl ?>
					<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerNext") ?>" href="<?php echo $a_sales_detail_view->PageUrl() ?>start=<?php echo $a_sales_detail_view->Pager->NextButton->Start ?>"><span class="icon-prev ewIcon"></span></a>
					<?php } else { // else of rtl ?>
					<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerNext") ?>" href="<?php echo $a_sales_detail_view->PageUrl() ?>start=<?php echo $a_sales_detail_view->Pager->NextButton->Start ?>"><span class="icon-next ewIcon"></span></a>
					<?php } // end of rtl ?>
					<?php } else { ?>
					<?php if ($Language->Phrase("dir") == "rtl") { // begin of rtl ?>
				...
				<!--last page button-->
					<?php if ($a_sales_detail_view->Pager->LastButton->Enabled) { ?>
					<?php if ($Language->Phrase("dir") == "rtl") { // begin of rtl ?>
					<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerLast") ?>" href="<?php echo $a_sales_detail_view->PageUrl() ?>start=<?php echo $a_sales_detail_view->Pager->LastButton->Start ?>"><span class="icon-first ewIcon"></span></a>
					<?php } else { // else of rtl ?>
					<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerLast") ?>" href="<?php echo $a_sales_detail_view->PageUrl() ?>start=<?php echo $a_sales_detail_view->Pager->LastButton->Start ?>"><span class="icon-last ewIcon"></span></a>
					<?php } // end of rtl ?>
					<?php } else { ?>
					<?php if ($Language->Phrase("dir") == "rtl") { // begin of rtl ?>
				...
				<span>&nbsp;<?php echo $Language->Phrase("of") ?>&nbsp;<?php echo $a_sales_detail_view->Pager->PageCount ?></span>
				</div>
...
<form name="fa_sales_detailview" id="fa_sales_detailview" class="form-inline ewForm ewViewForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($a_sales_detail_view->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $a_sales_detail_view->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="a_sales_detail">
<table class="table table-bordered table-striped ewViewTable">
<?php if ($a_sales_detail->Sales_ID->Visible) { // Sales_ID ?>
	<tr id="r_Sales_ID">
		<td><span id="elh_a_sales_detail_Sales_ID"><?php echo $a_sales_detail->Sales_ID->FldCaption() ?></span></td>
		<td<?php echo $a_sales_detail->Sales_ID->CellAttributes() ?>>
<span id="el_a_sales_detail_Sales_ID" class="form-group">
<span<?php echo $a_sales_detail->Sales_ID->ViewAttributes() ?>>
<?php echo $a_sales_detail->Sales_ID->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($a_sales_detail->Sales_Number->Visible) { // Sales_Number ?>
	<tr id="r_Sales_Number">
		<td><span id="elh_a_sales_detail_Sales_Number"><?php echo $a_sales_detail->Sales_Number->FldCaption() ?></span></td>
		<td<?php echo $a_sales_detail->Sales_Number->CellAttributes() ?>>
<span id="el_a_sales_detail_Sales_Number" class="form-group">
<span<?php echo $a_sales_detail->Sales_Number->ViewAttributes() ?>>
<?php echo $a_sales_detail->Sales_Number->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($a_sales_detail->Supplier_Number->Visible) { // Supplier_Number ?>
	<tr id="r_Supplier_Number">
		<td><span id="elh_a_sales_detail_Supplier_Number"><?php echo $a_sales_detail->Supplier_Number->FldCaption() ?></span></td>
		<td<?php echo $a_sales_detail->Supplier_Number->CellAttributes() ?>>
<span id="el_a_sales_detail_Supplier_Number" class="form-group">
<span<?php echo $a_sales_detail->Supplier_Number->ViewAttributes() ?>>
<?php echo $a_sales_detail->Supplier_Number->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($a_sales_detail->Stock_Item->Visible) { // Stock_Item ?>
	<tr id="r_Stock_Item">
		<td><span id="elh_a_sales_detail_Stock_Item"><?php echo $a_sales_detail->Stock_Item->FldCaption() ?></span></td>
		<td<?php echo $a_sales_detail->Stock_Item->CellAttributes() ?>>
<span id="el_a_sales_detail_Stock_Item" class="form-group">
<span<?php echo $a_sales_detail->Stock_Item->ViewAttributes() ?>>
<?php echo $a_sales_detail->Stock_Item->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($a_sales_detail->Sales_Quantity->Visible) { // Sales_Quantity ?>
	<tr id="r_Sales_Quantity">
		<td><span id="elh_a_sales_detail_Sales_Quantity"><?php echo $a_sales_detail->Sales_Quantity->FldCaption() ?></span></td>
		<td<?php echo $a_sales_detail->Sales_Quantity->CellAttributes() ?>>
<span id="el_a_sales_detail_Sales_Quantity" class="form-group">
<span<?php echo $a_sales_detail->Sales_Quantity->ViewAttributes() ?>>
<?php echo $a_sales_detail->Sales_Quantity->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($a_sales_detail->Purchasing_Price->Visible) { // Purchasing_Price ?>
	<tr id="r_Purchasing_Price">
		<td><span id="elh_a_sales_detail_Purchasing_Price"><?php echo $a_sales_detail->Purchasing_Price->FldCaption() ?></span></td>
		<td<?php echo $a_sales_detail->Purchasing_Price->CellAttributes() ?>>
<span id="el_a_sales_detail_Purchasing_Price" class="form-group">
<span<?php echo $a_sales_detail->Purchasing_Price->ViewAttributes() ?>>
<?php echo $a_sales_detail->Purchasing_Price->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($a_sales_detail->Sales_Price->Visible) { // Sales_Price ?>
	<tr id="r_Sales_Price">
		<td><span id="elh_a_sales_detail_Sales_Price"><?php echo $a_sales_detail->Sales_Price->FldCaption() ?></span></td>
		<td<?php echo $a_sales_detail->Sales_Price->CellAttributes() ?>>
<span id="el_a_sales_detail_Sales_Price" class="form-group">
<span<?php echo $a_sales_detail->Sales_Price->ViewAttributes() ?>>
<?php echo $a_sales_detail->Sales_Price->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($a_sales_detail->Sales_Total_Amount->Visible) { // Sales_Total_Amount ?>
	<tr id="r_Sales_Total_Amount">
		<td><span id="elh_a_sales_detail_Sales_Total_Amount"><?php echo $a_sales_detail->Sales_Total_Amount->FldCaption() ?></span></td>
		<td<?php echo $a_sales_detail->Sales_Total_Amount->CellAttributes() ?>>
<span id="el_a_sales_detail_Sales_Total_Amount" class="form-group">
<span<?php echo $a_sales_detail->Sales_Total_Amount->ViewAttributes() ?>>
<?php echo $a_sales_detail->Sales_Total_Amount->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
...
<script type="text/javascript">
fa_sales_detailview.Init();
</script>
<?php
$a_sales_detail_view->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<?php if ($a_sales_detail->Export == "") { ?>
...
<?php
$a_sales_detail_view->Page_Terminate();
?>
