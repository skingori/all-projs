...
// End of modification Disable/Enable Registration Page, by Masino Sinaga, May 14, 2012
		...
		switch ($this->CurrentAction) {
			...
			case "A": // Add new record
				...
				if ($this->AddRow($this->OldRecordset)) { // Add successful
					...
					if (ew_GetPageName($sReturnUrl) == "helpview.php")
	...
	function RenderRow() {
		...
		if ($this->RowType == EW_ROWTYPE_VIEW) { // View row
			...
			// Call Lookup selecting
			$this->Lookup_Selecting($this->Updated_By, $sWhereWrk);
			if ($sWhereWrk <> "") $sSqlWrk .= " WHERE " . $sWhereWrk;
				$rswrk = $conn->Execute($sSqlWrk);
				if ($rswrk && !$rswrk->EOF) { // Lookup values found
					$this->Updated_By->ViewValue = $rswrk->fields('DispFld');
					$this->Updated_By->ViewValue .= ew_ValueSeparator(1,$this->Updated_By) . $rswrk->fields('Disp2Fld');
					$rswrk->Close();
				} else {
	...
	function SetupBreadcrumb() {
		...
		$Breadcrumb->Add("list", $this->TableVar, "helplist.php", "", $this->TableVar, TRUE);
...
// Page object
var help_add = new ew_Page("help_add");
help_add.PageID = "add"; // Page ID
var EW_PAGE_ID = help_add.PageID; // For backward compatibility

// Form object
var fhelpadd = new ew_Form("fhelpadd");
...
fhelpadd.Lists["x__Language"] = {"LinkField":"x_Language_Code","Ajax":null,"AutoFill":false,"DisplayFields":["x_Language_Name","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
fhelpadd.Lists["x_Category"] = {"LinkField":"x_Category_ID","Ajax":null,"AutoFill":false,"DisplayFields":["x_Category_Description","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
fhelpadd.Lists["x_Updated_By"] = {"LinkField":"x_Username","Ajax":null,"AutoFill":false,"DisplayFields":["x_First_Name","x_Last_Name","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
...
?>
<form name="fhelpadd" id="fhelpadd" class="form-horizontal ewForm ewAddForm" action="<?php echo ew_CurrentPage() ?>" method="post">
...
<input type="text" data-field="x_Help_ID" name="x_Help_ID" id="x_Help_ID" size="30" placeholder="<?php echo ew_HtmlEncode($help->Help_ID->PlaceHolder) ?>" value="<?php echo $help->Help_ID->EditValue ?>"<?php echo $help->Help_ID->EditAttributes() ?>>
</span>
...
<select data-field="x__Language" id="x__Language" name="x__Language"<?php echo $help->_Language->EditAttributes() ?>>
<?php
if (is_array($help->_Language->EditValue)) {
	...
	for ($rowcntwrk = 0; $rowcntwrk < $rowswrk; $rowcntwrk++) {
		$selwrk = (strval($help->_Language->CurrentValue) == strval($arwrk[$rowcntwrk][0])) ? " selected=\"selected\"" : "";
...
<script type="text/javascript">
fhelpadd.Lists["x__Language"].Options = <?php echo (is_array($help->_Language->EditValue)) ? ew_ArrayToJson($help->_Language->EditValue, 1) : "[]" ?>;
</script>
</span>
...
<input type="text" data-field="x_Topic" name="x_Topic" id="x_Topic" size="30" maxlength="255" placeholder="<?php echo ew_HtmlEncode($help->Topic->PlaceHolder) ?>" value="<?php echo $help->Topic->EditValue ?>"<?php echo $help->Topic->EditAttributes() ?>>
</span>
...
<textarea data-field="x_Description" name="x_Description" id="x_Description" cols="50" rows="6" placeholder="<?php echo ew_HtmlEncode($help->Description->PlaceHolder) ?>"<?php echo $help->Description->EditAttributes() ?>><?php echo $help->Description->EditValue ?></textarea>
</span>
...
<?php } else { ?>
<span id="el_help_Category">
<select data-field="x_Category" id="x_Category" name="x_Category"<?php echo $help->Category->EditAttributes() ?>>
<?php
if (is_array($help->Category->EditValue)) {
	...
	$emptywrk = TRUE;
	for ($rowcntwrk = 0; $rowcntwrk < $rowswrk; $rowcntwrk++) {
		$selwrk = (strval($help->Category->CurrentValue) == strval($arwrk[$rowcntwrk][0])) ? " selected=\"selected\"" : "";
		if ($selwrk <> "") $emptywrk = FALSE;
?>
<option value="<?php echo ew_HtmlEncode($arwrk[$rowcntwrk][0]) ?>"<?php echo $selwrk ?>>
<?php echo $arwrk[$rowcntwrk][1] ?>
...
<script type="text/javascript">
fhelpadd.Lists["x_Category"].Options = <?php echo (is_array($help->Category->EditValue)) ? ew_ArrayToJson($help->Category->EditValue, 1) : "[]" ?>;
</script>
</span>
<?php } ?>
...
<input type="text" data-field="x_Order" name="x_Order" id="x_Order" size="30" placeholder="<?php echo ew_HtmlEncode($help->Order->PlaceHolder) ?>" value="<?php echo $help->Order->EditValue ?>"<?php echo $help->Order->EditAttributes() ?>>
</span>
...
<input type="text" data-field="x_Display_in_Page" name="x_Display_in_Page" id="x_Display_in_Page" size="30" maxlength="100" placeholder="<?php echo ew_HtmlEncode($help->Display_in_Page->PlaceHolder) ?>" value="<?php echo $help->Display_in_Page->EditValue ?>"<?php echo $help->Display_in_Page->EditAttributes() ?>>
</span>
...
<?php if ($help->Export == "") { ?>
<script type="text/javascript">
$('#btnAction').attr('onclick', 'return alertifyAdd(this)'); function alertifyAdd(obj) { <?php global $Language; ?> if (fhelpadd.Validate() == true ) { alertify.set({buttonFocus: 'cancel'});alertify.confirm("<?php echo $Language->Phrase('AlertifyAddConfirm'); ?>", function (e) { if (e) { $(window).unbind('beforeunload'); alertify.success("<?php echo $Language->Phrase('AlertifyAdd'); ?>"); $("#fhelpadd").submit(); } else { alertify.error("<?php echo $Language->Phrase('AlertifyCancel'); ?>"); } }, "<?php echo $Language->Phrase('AlertifyConfirm'); ?>"); } return false; }
</script>
<?php } ?>
