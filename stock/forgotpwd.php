...
// End of modification Disable/Enable Registration Page, by Masino Sinaga, May 14, 2012
	...
	function Page_Main() {
		...
		// Handle email activation
			...
			if ($this->Username <> "") {
				if ($this->Username <> ew_Decrypt($this->ActivateCode) || strtolower($this->Action) <> "confirm") { // Email activation
					if ($this->getFailureMessage() == "")
						$this->setFailureMessage($Language->Phrase("ActivateFailed")); // Set activate failed message
					$this->Page_Terminate("login.php"); // Go to login page
				}
		...
		if ($this->Action <> "") {
			...
			if (@$_GET["email"] <> "") {
				...
				$sFilter = str_replace("%u", ew_AdjustSql($this->Username), EW_USER_NAME_FILTER);
			} else {
				$sFilter = str_replace("%e", ew_AdjustSql($this->Email), EW_USER_EMAIL_FILTER);
			}
...
<?php } ?>
	<div class="form-group">
		<div class="col-sm-offset-4 col-sm-8">
			...
			<button class="btn btn-default ewButton" name="btnreset" id="btnreset" type="reset"><?php echo $Language->Phrase("Reset") ?></button>
		</div>
	</div>
</div>
<?php if (MS_FORGOTPWD_WINDOW_TYPE=="default" || MS_FORGOTPWD_WINDOW_TYPE=="") { ?>
