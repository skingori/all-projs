...
// Begin of modification Displaying Breadcrumbs in All Pages, by Masino Sinaga, May 25, 2012   
    ...
    function BreadcrumbPhrase($Id) {
        if (is_object($this->Phrases)) {
            ...
            return ew_ConvertFromUtf8(@$this->Phrases['ew-language']['breadcrumb']['phrase'][($Id)]['attr']['value']);
        }
    }
			...
			if ($this->RenderItem($item)) {
				...
				if ($item->GroupTitle && (!$this->IsRoot || !$this->MenuRootGroupTitleAsSubMenu)) { // Group title
					...
					if (!is_null($item->SubMenu)) {
						foreach ($item->SubMenu->ItemData as $subitem) {
							...
							if (!$subitem->IsCustomUrl && ew_CurrentPage() == ew_GetPageName($subitem->Url) || $subitem->IsCustomUrl && $cururl == $subitem->Url) {
								...
								// $subitem->Url = "javascript:void(0);";  // <-- No need this, removed by Masino Sinaga, July 22, 2014
							}
					...
					if (!$item->IsCustomUrl && ew_CurrentPage() == ew_GetPageName($item->Url) || $item->IsCustomUrl && $cururl == $item->Url) {
						...
						// $item->Url = "javascript:void(0);"; // <-- No need this, removed by Masino Sinaga, July 22, 2014
					}
...
class cMenuItem {
	...
	function Render($aclass = "", $liclass = "", $mobile = FALSE) {
		...
		if ($mobile) {
			...
			if ($url == "") $url = "#";
			$attrs = array("class" => $aclass, "rel" => ($url != "#") ? "external" : "", "href" => $url, "target" => $this->Target, "onclick" => $this->OnClick); // added "onclick" attribute, by Masino Sinaga, July 22, 2014
		} else {
			if ($url == "") $url = "#";
			...
			$attrs = array("class" => $aclass, "href" => $url, "target" => $this->Target, "onclick" => $this->OnClick); // added "onclick" attribute, by Masino Sinaga, July 22, 2014
		}
		$text = $this->Text;
...
function Menu_Rendering(&$Menu) {
  ...
  if (IsLoggedIn()) {  
	...
	$active_sign = "&nbsp;<i class='glyphicon glyphicon-check'></i> ";
	$active_fontname = CheckActiveFontName(); 
	switch ($active_fontname) {
		case "arial":
			$Menu->FindItem(36)->Text = $Language->MenuPhrase("36", "MenuText").$active_sign;
			break;
		case "calibri":
			$Menu->FindItem(37)->Text = $Language->MenuPhrase("37", "MenuText").$active_sign;
			break;
		case "century":
			$Menu->FindItem(38)->Text = $Language->MenuPhrase("38", "MenuText").$active_sign;
			break;
		case "centurygothic":
			$Menu->FindItem(39)->Text = $Language->MenuPhrase("39", "MenuText").$active_sign;
			break;
		case "comicsansms":
			$Menu->FindItem(40)->Text = $Language->MenuPhrase("40", "MenuText").$active_sign;
			break;
		case "couriernew":               
			$Menu->FindItem(41)->Text = $Language->MenuPhrase("41", "MenuText").$active_sign;   
			break;
		case "futuranormal":
			$Menu->FindItem(42)->Text = $Language->MenuPhrase("42", "MenuText").$active_sign;
			break;
		case "lucidasans":               
			$Menu->FindItem(43)->Text = $Language->MenuPhrase("43", "MenuText").$active_sign;
			break;
		case "lucidasanstypewriter":
			$Menu->FindItem(44)->Text = $Language->MenuPhrase("44", "MenuText").$active_sign;
			break;
		case "msgothic":               
			$Menu->FindItem(45)->Text = $Language->MenuPhrase("45", "MenuText").$active_sign;
			break;
		case "mssansserif":
			$Menu->FindItem(46)->Text = $Language->MenuPhrase("46", "MenuText").$active_sign;
			break;
		case "tahoma":               
			$Menu->FindItem(47)->Text = $Language->MenuPhrase("47", "MenuText").$active_sign;
			break;
		case "timesnewroman":
			$Menu->FindItem(48)->Text = $Language->MenuPhrase("48", "MenuText").$active_sign;
			break;
		case "verdana":               
			$Menu->FindItem(49)->Text = $Language->MenuPhrase("49", "MenuText").$active_sign;
			break;
	}
	$active_fontsize = CheckActiveFontSize(); 
	switch ($active_fontsize) {
		case "11px":
			$Menu->FindItem(51)->Text = $Language->MenuPhrase("51", "MenuText").$active_sign;
			break;
		case "12px":
			$Menu->FindItem(52)->Text = $Language->MenuPhrase("52", "MenuText").$active_sign;
			break;
		case "13px":
			$Menu->FindItem(54)->Text = $Language->MenuPhrase("54", "MenuText").$active_sign;
			break;
		case "14px":
			$Menu->FindItem(55)->Text = $Language->MenuPhrase("55", "MenuText").$active_sign;
			break;
	}
	$active_theme = CheckActiveTheme();
	switch ($active_theme) {
		case "theme-black.css":
			$Menu->FindItem(57)->Text = $Language->MenuPhrase("57", "MenuText").$active_sign;
			break;
		case "theme-blue.css":
			$Menu->FindItem(58)->Text = $Language->MenuPhrase("58", "MenuText").$active_sign;
			break;
		case "theme-dark.css":
			$Menu->FindItem(59)->Text = $Language->MenuPhrase("59", "MenuText").$active_sign;
			break;
		case "theme-darkglass.css":
			$Menu->FindItem(60)->Text = $Language->MenuPhrase("60", "MenuText").$active_sign;
			break;
		case "theme-default.css":
			$Menu->FindItem(61)->Text = $Language->MenuPhrase("61", "MenuText").$active_sign;
			break;
		case "theme-glass.css":
			$Menu->FindItem(62)->Text = $Language->MenuPhrase("62", "MenuText").$active_sign;
			break;                             
		case "theme-gray.css":
			$Menu->FindItem(63)->Text = $Language->MenuPhrase("63", "MenuText").$active_sign;
			break;
		case "theme-green.css":
			$Menu->FindItem(64)->Text = $Language->MenuPhrase("64", "MenuText").$active_sign;
			break;
		case "theme-maroon.css":
			$Menu->FindItem(65)->Text = $Language->MenuPhrase("65", "MenuText").$active_sign;
			break;
		case "theme-olive.css":
			$Menu->FindItem(66)->Text = $Language->MenuPhrase("66", "MenuText").$active_sign;
			break;
		case "theme-professional.css":
			$Menu->FindItem(67)->Text = $Language->MenuPhrase("67", "MenuText").$active_sign;
			break;
		case "theme-purple.css":
			$Menu->FindItem(68)->Text = $Language->MenuPhrase("68", "MenuText").$active_sign;
			break;
		case "theme-red.css":
			$Menu->FindItem(69)->Text = $Language->MenuPhrase("69", "MenuText").$active_sign;
			break;
		case "theme-sand.css":
			$Menu->FindItem(70)->Text = $Language->MenuPhrase("70", "MenuText").$active_sign;
			break;           
		case "theme-silver.css":
			$Menu->FindItem(71)->Text = $Language->MenuPhrase("71", "MenuText").$active_sign;
			break;
		case "theme-white.css":
			$Menu->FindItem(72)->Text = $Language->MenuPhrase("72", "MenuText").$active_sign;
			break;
		default: // theme-default.css
			$Menu->FindItem(61)->Text = $Language->MenuPhrase("73", "MenuText").$active_sign;
			break;
	}
	$active_menu_position = CheckActiveMenuPosition();
	if ($active_menu_position == "Y") {
		$Menu->FindItem(74)->Text = $Language->MenuPhrase("74", "MenuText").$active_sign;
	} else {
		$Menu->FindItem(75)->Text = $Language->MenuPhrase("75", "MenuText").$active_sign;
	}
	$active_table_width_style = CheckActiveTableWidthStyle();
	if ($active_table_width_style == "1") {
		$Menu->FindItem(77)->Text = $Language->MenuPhrase("77", "MenuText").$active_sign;
	} elseif ($active_table_width_style == "2") {
		$Menu->FindItem(78)->Text = $Language->MenuPhrase("78", "MenuText").$active_sign;
	} else {
		$Menu->FindItem(79)->Text = $Language->MenuPhrase("79", "MenuText").$active_sign;
	}                                           
  }
}
...
function MenuItem_Adding(&$Item) {
	...
	global $Language;
	if ($Item->Url == 'changepwd.php') {
		$Item->Text = '<i class="glyphicon glyphicon-refresh"></i> ' . $Language->Phrase("ChangePwd");
	} else if ($Item->Url == 'logout.php') {
	...
	if ($Item->Url == "breadcrumblinksaddsp.php") { 
	   return Security()->IsAdmin();
	}
	if ($Item->Url == "breadcrumblinkschecksp.php") { 
	   return Security()->IsAdmin();
	}                                     
	if ($Item->Url == "breadcrumblinksdeletesp.php") { 
	   return Security()->IsAdmin();
	}                                      
	if ($Item->Url == "breadcrumblinksmovesp.php") { 
	   return Security()->IsAdmin();
	}
	if ($Item->Url == "menuhorizontal.php") {
	   return IsLoggedIn();    
	}
	if ($Item->Url == "menuvertical.php") {
	   return IsLoggedIn();    
	}                                                      
	if (!IsLoggedIn()) {
		if ($Item->Text == "<i class='glyphicon glyphicon-picture'></i> Themes/Colors" || $Item->Text == "<i class='glyphicon glyphicon-picture'></i> Tema/Warna" 
			|| $Item->Text == "<i class='glyphicon glyphicon-list'></i> Menu Positions" || $Item->Text == "<i class='glyphicon glyphicon-list'></i> Posisi Menu" 
			|| $Item->Text == "<i class='glyphicon glyphicon-refresh'></i> Change My Password" || $Item->Text == "<i class='glyphicon glyphicon-refresh'></i> Ganti Kata Sandi") {
		   return FALSE;    
		}    
	}    
	return TRUE;
...
// moved from phpfn.php since v11.0.4
