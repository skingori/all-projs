...
$a_sales_edit = NULL; // Initialize page object first

class ca_sales_edit extends ca_sales {
...
// End of modification Disable/Enable Registration Page, by Masino Sinaga, May 14, 2012
	...
	function ValidateForm() {
		...
		if (in_array("a_sales_detail", $DetailTblVar) && $GLOBALS["a_sales_detail"]->DetailEdit) {
	...
	function EditRow() {
		...
		if ($this->Sales_Number->CurrentValue <> "") { // Check field with unique index
			$sFilterChk = "(`Sales_Number` = '" . ew_AdjustSql($this->Sales_Number->CurrentValue) . "')";
		...
		if ($rs->EOF) {
			...
			// Sales_Date
			$this->Sales_Date->SetDbValueDef($rsnew, ew_UnFormatDateTime($this->Sales_Date->CurrentValue, 9), ew_CurrentDate(), $this->Sales_Date->ReadOnly);
			...
			// Discount_Type
			$this->Discount_Type->SetDbValueDef($rsnew, $this->Discount_Type->CurrentValue, NULL, $this->Discount_Type->ReadOnly);

			// Discount_Percentage
			$this->Discount_Percentage->SetDbValueDef($rsnew, $this->Discount_Percentage->CurrentValue, NULL, $this->Discount_Percentage->ReadOnly);

			// Discount_Amount
			$this->Discount_Amount->SetDbValueDef($rsnew, $this->Discount_Amount->CurrentValue, NULL, $this->Discount_Amount->ReadOnly);

			// Tax_Percentage
			$this->Tax_Percentage->SetDbValueDef($rsnew, $this->Tax_Percentage->CurrentValue, NULL, $this->Tax_Percentage->ReadOnly);

			// Tax_Amount
			$this->Tax_Amount->SetDbValueDef($rsnew, $this->Tax_Amount->CurrentValue, NULL, $this->Tax_Amount->ReadOnly);

			// Tax_Description
			$this->Tax_Description->SetDbValueDef($rsnew, $this->Tax_Description->CurrentValue, NULL, $this->Tax_Description->ReadOnly);

			// Final_Total_Amount
			$this->Final_Total_Amount->SetDbValueDef($rsnew, $this->Final_Total_Amount->CurrentValue, NULL, $this->Final_Total_Amount->ReadOnly);
			...
			if ($bUpdateRow) {
				...
				if ($EditRow) {
					...
					if (in_array("a_sales_detail", $DetailTblVar) && $GLOBALS["a_sales_detail"]->DetailEdit) {
						if (!isset($GLOBALS["a_sales_detail_grid"])) $GLOBALS["a_sales_detail_grid"] = new ca_sales_detail_grid(); // Get detail page object
						$EditRow = $GLOBALS["a_sales_detail_grid"]->GridUpdate();
					}
				}
	...
	function SetUpDetailParms() {
		...
		if ($sDetailTblVar <> "") {
			...
			if (in_array("a_sales_detail", $DetailTblVar)) {
				...
				if ($GLOBALS["a_sales_detail_grid"]->DetailEdit) {
					$GLOBALS["a_sales_detail_grid"]->CurrentMode = "edit";
					$GLOBALS["a_sales_detail_grid"]->CurrentAction = "gridedit";
...
// Create page object
if (!isset($a_sales_edit)) $a_sales_edit = new ca_sales_edit();

// Page init
$a_sales_edit->Page_Init();

// Page main
$a_sales_edit->Page_Main();
...
$a_sales_edit->Page_Render();
?>
<?php include_once "header.php" ?>
<script type="text/javascript">

// Page object
var a_sales_edit = new ew_Page("a_sales_edit");
a_sales_edit.PageID = "edit"; // Page ID
var EW_PAGE_ID = a_sales_edit.PageID; // For backward compatibility

// Form object
var fa_salesedit = new ew_Form("fa_salesedit");

// Validate form
fa_salesedit.Validate = function() {
...
fa_salesedit.Form_CustomValidate = 
...
fa_salesedit.ValidateRequired = true;
<?php } else { ?>
fa_salesedit.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
fa_salesedit.Lists["x_Customer_ID"] = {"LinkField":"x_Customer_Number","Ajax":null,"AutoFill":false,"DisplayFields":["x_Customer_Name","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
...
<?php $a_sales_edit->ShowPageHeader(); ?>
<?php
$a_sales_edit->ShowMessage();
?>
...
<form name="ewPagerForm" class="form-horizontal ewForm ewPagerForm" action="<?php echo ew_CurrentPage() ?>">
	<?php if (MS_PAGINATION_STYLE==1) { // link ?>
		<?php if (!isset($a_sales_edit->Pager)) $a_sales_edit->Pager = new cNumericPager($a_sales_edit->StartRec, $a_sales_edit->DisplayRecs, $a_sales_edit->TotalRecs, $a_sales_edit->RecRange) ?>
		<?php if ($a_sales_edit->Pager->RecordCount > 0) { ?>
				<?php if (($a_sales_edit->Pager->PageCount==1) && ($a_sales_edit->Pager->CurrentPage == 1) && (MS_SHOW_PAGENUM_IF_REC_NOT_OVER_PAGESIZE==FALSE)  ) { ?>
				<?php } else { // MS_SHOW_PAGENUM_IF_REC_NOT_OVER_PAGESIZE ?>
				<div class="ewPager">
				<div class="ewNumericPage"><ul class="pagination">
					<?php if ($a_sales_edit->Pager->FirstButton->Enabled) { ?>
					<?php if ($Language->Phrase("dir") == "rtl") { // begin of rtl ?>
					<li><a href="<?php echo $a_sales_edit->PageUrl() ?>start=<?php echo $a_sales_edit->Pager->FirstButton->Start ?>"><?php echo $Language->Phrase("PagerLast") ?></a></li>
					<?php } else { // else of rtl ?>
					<li><a href="<?php echo $a_sales_edit->PageUrl() ?>start=<?php echo $a_sales_edit->Pager->FirstButton->Start ?>"><?php echo $Language->Phrase("PagerFirst") ?></a></li>
					<?php } // end of rtl ?>
					<?php } ?>
					<?php if ($a_sales_edit->Pager->PrevButton->Enabled) { ?>
					<?php if ($Language->Phrase("dir") == "rtl") { // begin of rtl ?>
					<li><a href="<?php echo $a_sales_edit->PageUrl() ?>start=<?php echo $a_sales_edit->Pager->PrevButton->Start ?>"><?php echo $Language->Phrase("PagerNext") ?></a></li>
					<?php } else { // else of rtl { ?>
					<li><a href="<?php echo $a_sales_edit->PageUrl() ?>start=<?php echo $a_sales_edit->Pager->PrevButton->Start ?>"><?php echo $Language->Phrase("PagerPrevious") ?></a></li>
					<?php } // end of rtl { ?>
					<?php } ?>
					<?php foreach ($a_sales_edit->Pager->Items as $PagerItem) { ?>
						<li<?php if (!$PagerItem->Enabled) { echo " class=\" active\""; } ?>><a href="<?php if ($PagerItem->Enabled) { echo $a_sales_edit->PageUrl() . "start=" . $PagerItem->Start; } else { echo "#"; } ?>"><?php echo $PagerItem->Text ?></a></li>
					<?php } ?>
					<?php if ($a_sales_edit->Pager->NextButton->Enabled) { ?>
					<?php if ($Language->Phrase("dir") == "rtl") { // begin of rtl ?>
					<li><a href="<?php echo $a_sales_edit->PageUrl() ?>start=<?php echo $a_sales_edit->Pager->NextButton->Start ?>"><?php echo $Language->Phrase("PagerPrevious") ?></a></li>
					<?php } else { // else of rtl ?>
					<li><a href="<?php echo $a_sales_edit->PageUrl() ?>start=<?php echo $a_sales_edit->Pager->NextButton->Start ?>"><?php echo $Language->Phrase("PagerNext") ?></a></li>
					<?php } // end of rtl ?>
					<?php } ?>
					<?php if ($a_sales_edit->Pager->LastButton->Enabled) { ?>
					<?php if ($Language->Phrase("dir") == "rtl") { // begin of rtl ?>
					<li><a href="<?php echo $a_sales_edit->PageUrl() ?>start=<?php echo $a_sales_edit->Pager->LastButton->Start ?>"><?php echo $Language->Phrase("PagerFirst") ?></a></li>
					<?php } else { // else of rtl ?>
					<li><a href="<?php echo $a_sales_edit->PageUrl() ?>start=<?php echo $a_sales_edit->Pager->LastButton->Start ?>"><?php echo $Language->Phrase("PagerLast") ?></a></li>
					<?php } // end of rtl ?>
					<?php } ?>
				</ul></div>
				</div>
	...
	<?php } elseif (MS_PAGINATION_STYLE==2) { // button ?>
		<?php if (!isset($a_sales_edit->Pager)) $a_sales_edit->Pager = new cPrevNextPager($a_sales_edit->StartRec, $a_sales_edit->DisplayRecs, $a_sales_edit->TotalRecs) ?>
		<?php if ($a_sales_edit->Pager->RecordCount > 0) { ?>
				<?php if (($a_sales_edit->Pager->PageCount==1) && ($a_sales_edit->Pager->CurrentPage == 1) && (MS_SHOW_PAGENUM_IF_REC_NOT_OVER_PAGESIZE==FALSE)  ) { ?>
				...
				<!--first page button-->
					<?php if ($a_sales_edit->Pager->FirstButton->Enabled) { ?>
					<?php if ($Language->Phrase("dir") == "rtl") { // begin of rtl ?>
					<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerFirst") ?>" href="<?php echo $a_sales_edit->PageUrl() ?>start=<?php echo $a_sales_edit->Pager->FirstButton->Start ?>"><span class="icon-last ewIcon"></span></a>
					<?php } else { // else of rtl ?>
					<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerFirst") ?>" href="<?php echo $a_sales_edit->PageUrl() ?>start=<?php echo $a_sales_edit->Pager->FirstButton->Start ?>"><span class="icon-first ewIcon"></span></a>
					<?php } // end of rtl ?>
					<?php } else { ?>
					<?php if ($Language->Phrase("dir") == "rtl") { // begin of rtl ?>
				...
				<!--previous page button-->
					<?php if ($a_sales_edit->Pager->PrevButton->Enabled) { ?>
					<?php if ($Language->Phrase("dir") == "rtl") { // begin of rtl ?>
					<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerPrevious") ?>" href="<?php echo $a_sales_edit->PageUrl() ?>start=<?php echo $a_sales_edit->Pager->PrevButton->Start ?>"><span class="icon-next ewIcon"></span></a>
					<?php } else { // else of rtl ?>
					<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerPrevious") ?>" href="<?php echo $a_sales_edit->PageUrl() ?>start=<?php echo $a_sales_edit->Pager->PrevButton->Start ?>"><span class="icon-prev ewIcon"></span></a>
					<?php } // end of rtl ?>
					<?php } else { ?>
					<?php if ($Language->Phrase("dir") == "rtl") { // begin of rtl ?>
				...
				<!--current page number-->
					<input class="form-control input-sm" type="text" name="<?php echo EW_TABLE_PAGE_NO ?>" value="<?php echo $a_sales_edit->Pager->CurrentPage ?>">
				<div class="input-group-btn">
				<!--next page button-->
					<?php if ($a_sales_edit->Pager->NextButton->Enabled) { ?>
					<?php if ($Language->Phrase("dir") == "rtl") { // begin of rtl ?>
					<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerNext") ?>" href="<?php echo $a_sales_edit->PageUrl() ?>start=<?php echo $a_sales_edit->Pager->NextButton->Start ?>"><span class="icon-prev ewIcon"></span></a>
					<?php } else { // else of rtl ?>
					<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerNext") ?>" href="<?php echo $a_sales_edit->PageUrl() ?>start=<?php echo $a_sales_edit->Pager->NextButton->Start ?>"><span class="icon-next ewIcon"></span></a>
					<?php } // end of rtl ?>
					<?php } else { ?>
					<?php if ($Language->Phrase("dir") == "rtl") { // begin of rtl ?>
				...
				<!--last page button-->
					<?php if ($a_sales_edit->Pager->LastButton->Enabled) { ?>
					<?php if ($Language->Phrase("dir") == "rtl") { // begin of rtl ?>
					<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerLast") ?>" href="<?php echo $a_sales_edit->PageUrl() ?>start=<?php echo $a_sales_edit->Pager->LastButton->Start ?>"><span class="icon-first ewIcon"></span></a>
					<?php } else { // else of rtl ?>
					<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerLast") ?>" href="<?php echo $a_sales_edit->PageUrl() ?>start=<?php echo $a_sales_edit->Pager->LastButton->Start ?>"><span class="icon-last ewIcon"></span></a>
					<?php } // end of rtl ?>
					<?php } else { ?>
					<?php if ($Language->Phrase("dir") == "rtl") { // begin of rtl ?>
				...
				<span>&nbsp;<?php echo $Language->Phrase("of") ?>&nbsp;<?php echo $a_sales_edit->Pager->PageCount ?></span>
				</div>
...
<form name="fa_salesedit" id="fa_salesedit" class="form-horizontal ewForm ewEditForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($a_sales_edit->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $a_sales_edit->Token ?>">
<?php } ?>
...
<?php if ($a_sales->Sales_ID->Visible) { // Sales_ID ?>
	<div id="r_Sales_ID" class="form-group">
		<label id="elh_a_sales_Sales_ID" class="col-sm-4 control-label ewLabel"><?php echo $a_sales->Sales_ID->FldCaption() ?></label>
		<div class="col-sm-8"><div<?php echo $a_sales->Sales_ID->CellAttributes() ?>>
<span id="el_a_sales_Sales_ID">
<span<?php echo $a_sales->Sales_ID->ViewAttributes() ?>>
<p class="form-control-static"><?php echo $a_sales->Sales_ID->EditValue ?></p></span>
</span>
<input type="hidden" data-field="x_Sales_ID" name="x_Sales_ID" id="x_Sales_ID" value="<?php echo ew_HtmlEncode($a_sales->Sales_ID->CurrentValue) ?>">
<?php echo $a_sales->Sales_ID->CustomMsg ?></div></div>
	</div>
<?php } ?>
...
<script type="text/javascript">
ew_CreateCalendar("fa_salesedit", "x_Sales_Date", "%Y/%m/%d %H:%M:%S");
</script>
<?php } ?>
</span>
...
if (is_array($a_sales->Customer_ID->EditValue)) {
	...
	for ($rowcntwrk = 0; $rowcntwrk < $rowswrk; $rowcntwrk++) {
		$selwrk = (strval($a_sales->Customer_ID->CurrentValue) == strval($arwrk[$rowcntwrk][0])) ? " selected=\"selected\"" : "";
...
<script type="text/javascript">
fa_salesedit.Lists["x_Customer_ID"].Options = <?php echo (is_array($a_sales->Customer_ID->EditValue)) ? ew_ArrayToJson($a_sales->Customer_ID->EditValue, 1) : "[]" ?>;
</script>
</span>
<?php } ?>
...
if (is_array($arwrk)) {
	...
	$emptywrk = TRUE;
	for ($rowcntwrk = 0; $rowcntwrk < $rowswrk; $rowcntwrk++) {
		$selwrk = (strval($a_sales->Discount_Type->CurrentValue) == strval($arwrk[$rowcntwrk][0])) ? " checked=\"checked\"" : "";
		if ($selwrk <> "") $emptywrk = FALSE;
...
<input type="hidden" data-field="x_Date_Added" name="x_Date_Added" id="x_Date_Added" value="<?php echo ew_HtmlEncode($a_sales->Date_Added->CurrentValue) ?>">
</span>
<span id="el_a_sales_Added_By">
<input type="hidden" data-field="x_Added_By" name="x_Added_By" id="x_Added_By" value="<?php echo ew_HtmlEncode($a_sales->Added_By->CurrentValue) ?>">
</span>
<?php
	if (in_array("a_sales_detail", explode(",", $a_sales->getCurrentDetailTable())) && $a_sales_detail->DetailEdit) {
?>
...
function GetBalanceTotal() { 
	...
	var final_total_balance = final_total_amount - total_payment; 
	$("#x_Total_Balance").val(total_balance);
	if (total_balance < 0) {
		$("#x_Total_Payment").val(final_total_amount);
		$("#x_Total_Balance").val("0");
	}
	if (total_payment >= final_total_amount) {
		$("#x_Total_Payment").val(final_total_amount);
		$("#x_Total_Balance").val("0");
	} else {
		$("#x_Total_Balance").val(final_total_balance);
	}
}
...
<script type="text/javascript">
$(document).ready(function(){$("#fa_salesedit:first *:input[type!=hidden]:first").focus(),$("input").keydown(function(i){if(13==i.which){var e=$(this).closest("form").find(":input:visible:enabled"),n=e.index(this);n==e.length-1||(e.eq(e.index(this)+1).focus(),i.preventDefault())}else 113==i.which&&$("#btnAction").click()}),$("select").keydown(function(i){if(13==i.which){var e=$(this).closest("form").find(":input:visible:enabled"),n=e.index(this);n==e.length-1||(e.eq(e.index(this)+1).focus(),i.preventDefault())}else 113==i.which&&$("#btnAction").click()}),$("radio").keydown(function(i){if(13==i.which){var e=$(this).closest("form").find(":input:visible:enabled"),n=e.index(this);n==e.length-1||(e.eq(e.index(this)+1).focus(),i.preventDefault())}else 113==i.which&&$("#btnAction").click()})});
</script>
<?php } ?>
<?php if ($a_sales->Export == "") { ?>
<script type="text/javascript">
$('#btnAction').attr('onclick', 'return alertifyEdit(this)'); function alertifyEdit(obj) { <?php global $Language; ?> if (fa_salesedit.Validate() == true ) { alertify.set({buttonFocus: 'cancel'});alertify.confirm("<?php echo $Language->Phrase('AlertifyEditConfirm'); ?>", function (e) {	if (e) { $(window).unbind('beforeunload'); alertify.success("<?php echo $Language->Phrase('AlertifyEdit'); ?>"); $("#fa_salesedit").submit(); } else {	alertify.error("<?php echo $Language->Phrase('AlertifyCancel'); ?>"); }	}, "<?php echo $Language->Phrase('AlertifyConfirm'); ?>");	} return false; }
</script>
<?php } ?>
<?php include_once "footer.php" ?>
<?php
$a_sales_edit->Page_Terminate();
?>
