...
$stats_counterlog = NULL;

//
// Table class for stats_counterlog
//
class cstats_counterlog extends cTable {
	var $IP_Address;
	var $Hostname;
	var $First_Visit;
	var $Last_Visit;
	...
	function __construct() {
		...
		$this->TableVar = 'stats_counterlog';
		$this->TableName = 'stats_counterlog';
...
// End of mofidication Flexibility of Export Records Options, by Masino Sinaga, May 14, 2012
		...
		// IP_Address
		$this->IP_Address = new cField('stats_counterlog', 'stats_counterlog', 'x_IP_Address', 'IP_Address', '`IP_Address`', '`IP_Address`', 200, -1, FALSE, '`IP_Address`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['IP_Address'] = &$this->IP_Address;

		// Hostname
		$this->Hostname = new cField('stats_counterlog', 'stats_counterlog', 'x_Hostname', 'Hostname', '`Hostname`', '`Hostname`', 200, -1, FALSE, '`Hostname`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['Hostname'] = &$this->Hostname;

		// First_Visit
		$this->First_Visit = new cField('stats_counterlog', 'stats_counterlog', 'x_First_Visit', 'First_Visit', '`First_Visit`', 'DATE_FORMAT(`First_Visit`, \'%Y/%m/%d %H:%i:%s\')', 135, 9, FALSE, '`First_Visit`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->First_Visit->FldDefaultErrMsg = str_replace("%s", "/", $Language->Phrase("IncorrectDateYMD"));
		$this->fields['First_Visit'] = &$this->First_Visit;

		// Last_Visit
		$this->Last_Visit = new cField('stats_counterlog', 'stats_counterlog', 'x_Last_Visit', 'Last_Visit', '`Last_Visit`', 'DATE_FORMAT(`Last_Visit`, \'%Y/%m/%d %H:%i:%s\')', 135, 9, FALSE, '`Last_Visit`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->Last_Visit->FldDefaultErrMsg = str_replace("%s", "/", $Language->Phrase("IncorrectDateYMD"));
		$this->fields['Last_Visit'] = &$this->Last_Visit;

		// Counter
		$this->Counter = new cField('stats_counterlog', 'stats_counterlog', 'x_Counter', 'Counter', '`Counter`', '`Counter`', 3, -1, FALSE, '`Counter`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->Counter->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`stats_counterlog`";
	}
	...
	// Update Table
	var $UpdateTable = "`stats_counterlog`";
	...
	function DeleteSQL(&$rs, $where = "") {
		...
		if ($rs) {
			if (array_key_exists('IP_Address', $rs))
				ew_AddFilter($where, ew_QuotedName('IP_Address') . '=' . ew_QuotedValue($rs['IP_Address'], $this->IP_Address->FldDataType));
		}
		$filter = $this->CurrentFilter;
		ew_AddFilter($filter, $where);
		if ($filter <> "")
	...
	function KeyFilter() {
		...
		$sKeyFilter = str_replace("@IP_Address@", ew_AdjustSql($this->IP_Address->CurrentValue), $sKeyFilter); // Replace key value
		return $sKeyFilter;
	}
	...
	function getReturnUrl() {
		...
		if (@$_SESSION[$name] <> "") {
			...
			return "stats_counterloglist.php";
		}
	}
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			return $this->KeyUrl("stats_counterlogview.php", $this->UrlParm($parm));
		else
			return $this->KeyUrl("stats_counterlogview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	}

	// Add URL
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			return "stats_counterlogadd.php?" . $this->UrlParm($parm);
		else
			return "stats_counterlogadd.php";
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		return $this->KeyUrl("stats_counterlogedit.php", $this->UrlParm($parm));
	}
	...
	function GetCopyUrl($parm = "") {
		return $this->KeyUrl("stats_counterlogadd.php", $this->UrlParm($parm));
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("stats_counterlogdelete.php", $this->UrlParm());
	}
	...
	function KeyUrl($url, $parm = "") {
		$sUrl = $url . "?";
		if ($parm <> "") $sUrl .= $parm . "&";
		if (!is_null($this->IP_Address->CurrentValue)) {
			$sUrl .= "IP_Address=" . urlencode($this->IP_Address->CurrentValue);
		} else {
	...
	function GetRecordKeys() {
		...
		if (isset($_POST["key_m"])) {
			...
			$arKeys[] = @$_GET["IP_Address"]; // IP_Address
	...
	function GetKeyFilter() {
		...
		foreach ($arKeys as $key) {
			...
			$this->IP_Address->CurrentValue = $key;
		...
		if (!$Doc->ExportCustom) {
			...
			if ($Doc->Horizontal) { // Horizontal format, write header
				...
				if ($ExportPageType == "view") {
					...
					if ($this->First_Visit->Exportable) $Doc->ExportCaption($this->First_Visit);
					if ($this->Last_Visit->Exportable) $Doc->ExportCaption($this->Last_Visit);
					if ($this->Counter->Exportable) $Doc->ExportCaption($this->Counter);
				} else {
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if (!$Doc->ExportCustom) {
					...
					if ($ExportPageType == "view") {
						...
						if ($this->First_Visit->Exportable) $Doc->ExportField($this->First_Visit);
						if ($this->Last_Visit->Exportable) $Doc->ExportField($this->Last_Visit);
						if ($this->Counter->Exportable) $Doc->ExportField($this->Counter);
					} else {
