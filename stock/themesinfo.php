...
//
// Table class for themes
//
class cthemes extends cTable {
	var $Theme_ID;
	var $Theme_Name;
	var $_Default;
...
// End of mofidication Flexibility of Export Records Options, by Masino Sinaga, May 14, 2012
		...
		// Theme_ID
		$this->Theme_ID = new cField('themes', 'themes', 'x_Theme_ID', 'Theme_ID', '`Theme_ID`', '`Theme_ID`', 200, -1, FALSE, '`Theme_ID`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['Theme_ID'] = &$this->Theme_ID;

		// Theme_Name
		$this->Theme_Name = new cField('themes', 'themes', 'x_Theme_Name', 'Theme_Name', '`Theme_Name`', '`Theme_Name`', 200, -1, FALSE, '`Theme_Name`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['Theme_Name'] = &$this->Theme_Name;

		// Default
		$this->_Default = new cField('themes', 'themes', 'x__Default', 'Default', '`Default`', '`Default`', 202, -1, FALSE, '`Default`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`themes`";
	}
	...
	function DeleteSQL(&$rs, $where = "") {
		...
		if ($rs) {
			if (array_key_exists('Theme_ID', $rs))
				ew_AddFilter($where, ew_QuotedName('Theme_ID') . '=' . ew_QuotedValue($rs['Theme_ID'], $this->Theme_ID->FldDataType));
		}
		$filter = $this->CurrentFilter;
		ew_AddFilter($filter, $where);
		if ($filter <> "")
	...
	function KeyFilter() {
		...
		$sKeyFilter = str_replace("@Theme_ID@", ew_AdjustSql($this->Theme_ID->CurrentValue), $sKeyFilter); // Replace key value
		return $sKeyFilter;
	}
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			return $this->KeyUrl("themesview.php", $this->UrlParm($parm));
		else
			return $this->KeyUrl("themesview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	}

	// Add URL
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			return "themesadd.php?" . $this->UrlParm($parm);
		else
			return "themesadd.php";
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		return $this->KeyUrl("themesedit.php", $this->UrlParm($parm));
	}
	...
	function GetCopyUrl($parm = "") {
		return $this->KeyUrl("themesadd.php", $this->UrlParm($parm));
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("themesdelete.php", $this->UrlParm());
	}
	...
	function KeyUrl($url, $parm = "") {
		$sUrl = $url . "?";
		if ($parm <> "") $sUrl .= $parm . "&";
		if (!is_null($this->Theme_ID->CurrentValue)) {
			$sUrl .= "Theme_ID=" . urlencode($this->Theme_ID->CurrentValue);
		} else {
	...
	function GetKeyFilter() {
		...
		foreach ($arKeys as $key) {
			...
			$this->Theme_ID->CurrentValue = $key;
   ...
   // Common render codes
		...
		if (!$Doc->ExportCustom) {
			...
			if ($Doc->Horizontal) { // Horizontal format, write header
				...
				if ($ExportPageType == "view") {
					if ($this->Theme_ID->Exportable) $Doc->ExportCaption($this->Theme_ID);
					if ($this->Theme_Name->Exportable) $Doc->ExportCaption($this->Theme_Name);
		...
		while (!$Recordset->EOF && $RecCnt < $StopRec) {
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if (!$Doc->ExportCustom) {
					...
					if ($ExportPageType == "view") {
						if ($this->Theme_ID->Exportable) $Doc->ExportField($this->Theme_ID);
						if ($this->Theme_Name->Exportable) $Doc->ExportField($this->Theme_Name);
	...
	function Row_Inserting($rsold, &$rsnew) {
		...
		if ($rsnew["Default"] == 'Y') {
		   ew_Execute("UPDATE `".MS_THEMES_TABLE."` SET `Default` = 'N'");
		   ew_Execute("UPDATE `".MS_SETTINGS_TABLE."` SET `Default_Theme` = '".$rsnew["Theme_ID"]."' 
		   				WHERE `Option_Default` = 'Y'");
		}
		return TRUE;
	}
	...
	function Row_Updating($rsold, &$rsnew) {
		...
		if ($rsnew["Default"] == 'Y') {
		   ew_Execute("UPDATE `".MS_THEMES_TABLE."` SET `Default` = 'N'");
		   ew_Execute("UPDATE `".MS_SETTINGS_TABLE."` SET `Default_Theme` = '".$rsold["Theme_ID"]."' 
		   				WHERE `Option_Default` = 'Y'");
		}
		return TRUE;
	}
