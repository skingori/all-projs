...
$themes_delete = NULL; // Initialize page object first

class cthemes_delete extends cthemes {
...
// End of modification Disable/Enable Registration Page, by Masino Sinaga, May 14, 2012
	...
	function Page_Main() {
		...
		if ($sFilter == "")
			$this->Page_Terminate("themeslist.php"); // Prevent SQL injection, return to list

		// Set up filter (SQL WHHERE clause) and get return SQL
		// SQL constructor in themes class, themesinfo.php
...
// Create page object
if (!isset($themes_delete)) $themes_delete = new cthemes_delete();

// Page init
$themes_delete->Page_Init();

// Page main
$themes_delete->Page_Main();
...
// Form object
var fthemesdelete = new ew_Form("fthemesdelete");

// Form_CustomValidate event
fthemesdelete.Form_CustomValidate = 
...
fthemesdelete.ValidateRequired = true;
<?php } else { ?>
fthemesdelete.ValidateRequired = false; 
<?php } ?>
...
if ($themes_delete->Recordset = $themes_delete->LoadRecordset())
	$themes_deleteTotalRecs = $themes_delete->Recordset->RecordCount(); // Get record count
if ($themes_deleteTotalRecs <= 0) { // No record found, exit
	if ($themes_delete->Recordset)
		$themes_delete->Recordset->Close();
	$themes_delete->Page_Terminate("themeslist.php"); // Return to list
}
?>
<div class="ewToolbar">
...
<?php $themes_delete->ShowPageHeader(); ?>
<?php
$themes_delete->ShowMessage();
?>
<form name="fthemesdelete" id="fthemesdelete" class="form-inline ewForm ewDeleteForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($themes_delete->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $themes_delete->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="themes">
<input type="hidden" name="a_delete" id="a_delete" value="D">
<?php foreach ($themes_delete->RecKeys as $key) { ?>
...
<?php if ($themes->Theme_ID->Visible) { // Theme_ID ?>
		<th><span id="elh_themes_Theme_ID" class="themes_Theme_ID"><?php echo $themes->Theme_ID->FldCaption() ?></span></th>
<?php } ?>
<?php if ($themes->Theme_Name->Visible) { // Theme_Name ?>
		<th><span id="elh_themes_Theme_Name" class="themes_Theme_Name"><?php echo $themes->Theme_Name->FldCaption() ?></span></th>
<?php } ?>
<?php if ($themes->_Default->Visible) { // Default ?>
		<th><span id="elh_themes__Default" class="themes__Default"><?php echo $themes->_Default->FldCaption() ?></span></th>
<?php } ?>
	</tr>
	</thead>
	<tbody>
...
$i = 0;
while (!$themes_delete->Recordset->EOF) {
	$themes_delete->RecCnt++;
	$themes_delete->RowCnt++;
	...
	$themes_delete->LoadRowValues($themes_delete->Recordset);

	// Render row
	$themes_delete->RenderRow();
?>
	<tr<?php echo $themes->RowAttributes() ?>>
<?php if ($themes->Theme_ID->Visible) { // Theme_ID ?>
		<td<?php echo $themes->Theme_ID->CellAttributes() ?>>
<span id="el<?php echo $themes_delete->RowCnt ?>_themes_Theme_ID" class="form-group themes_Theme_ID">
...
<span id="el<?php echo $themes_delete->RowCnt ?>_themes_Theme_Name" class="form-group themes_Theme_Name">
...
<span id="el<?php echo $themes_delete->RowCnt ?>_themes__Default" class="form-group themes__Default">
