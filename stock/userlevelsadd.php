...
// End of modification Disable/Enable Registration Page, by Masino Sinaga, May 14, 2012
	...
	function AddRow($rsold = NULL) {
		...
		} elseif (intval($this->User_Level_ID->CurrentValue) < -1) {
			$this->setFailureMessage($Language->Phrase("UserLevelIDIncorrect"));
		} elseif (intval($this->User_Level_ID->CurrentValue) == 0 && strtolower(trim($this->User_Level_Name->CurrentValue)) <> "default") {
			$this->setFailureMessage($Language->Phrase("UserLevelDefaultName"));
		} elseif (intval($this->User_Level_ID->CurrentValue) == -1 && strtolower(trim($this->User_Level_Name->CurrentValue)) <> "administrator") {
			$this->setFailureMessage($Language->Phrase("UserLevelAdministratorName"));
		} elseif (intval($this->User_Level_ID->CurrentValue) > 0 && (strtolower(trim($this->User_Level_Name->CurrentValue)) == "administrator" || strtolower(trim($this->User_Level_Name->CurrentValue)) == "default")) {
...
<input type="text" data-field="x_User_Level_ID" name="x_User_Level_ID" id="x_User_Level_ID" size="30" placeholder="<?php echo ew_HtmlEncode($userlevels->User_Level_ID->PlaceHolder) ?>" value="<?php echo $userlevels->User_Level_ID->EditValue ?>"<?php echo $userlevels->User_Level_ID->EditAttributes() ?>>
</span>
...
<script type="text/javascript">
$('#btnAction').attr('onclick', 'return alertifyAdd(this)'); function alertifyAdd(obj) { <?php global $Language; ?> if (fuserlevelsadd.Validate() == true ) { alertify.set({buttonFocus: 'cancel'});alertify.confirm("<?php echo $Language->Phrase('AlertifyAddConfirm'); ?>", function (e) { if (e) { $(window).unbind('beforeunload'); alertify.success("<?php echo $Language->Phrase('AlertifyAdd'); ?>"); $("#fuserlevelsadd").submit(); } else { alertify.error("<?php echo $Language->Phrase('AlertifyCancel'); ?>"); } }, "<?php echo $Language->Phrase('AlertifyConfirm'); ?>"); } return false; }
</script>
<?php } ?>
