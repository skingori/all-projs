...
//
// Table class for breadcrumblinks
//
class cbreadcrumblinks extends cTable {
	var $Page_Title;
	var $Page_URL;
	var $Lft;
	var $Rgt;
	...
	function __construct() {
		...
		$this->TableVar = 'breadcrumblinks';
		$this->TableName = 'breadcrumblinks';
...
// End of mofidication Flexibility of Export Records Options, by Masino Sinaga, May 14, 2012
		...
		// Page_Title
		$this->Page_Title = new cField('breadcrumblinks', 'breadcrumblinks', 'x_Page_Title', 'Page_Title', '`Page_Title`', '`Page_Title`', 200, -1, FALSE, '`Page_Title`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['Page_Title'] = &$this->Page_Title;

		// Page_URL
		$this->Page_URL = new cField('breadcrumblinks', 'breadcrumblinks', 'x_Page_URL', 'Page_URL', '`Page_URL`', '`Page_URL`', 200, -1, FALSE, '`Page_URL`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['Page_URL'] = &$this->Page_URL;

		// Lft
		$this->Lft = new cField('breadcrumblinks', 'breadcrumblinks', 'x_Lft', 'Lft', '`Lft`', '`Lft`', 3, -1, FALSE, '`Lft`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->Lft->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['Lft'] = &$this->Lft;

		// Rgt
		$this->Rgt = new cField('breadcrumblinks', 'breadcrumblinks', 'x_Rgt', 'Rgt', '`Rgt`', '`Rgt`', 3, -1, FALSE, '`Rgt`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`breadcrumblinks`";
	}
	...
	// Update Table
	var $UpdateTable = "`breadcrumblinks`";
	...
	function DeleteSQL(&$rs, $where = "") {
		...
		if ($rs) {
			if (array_key_exists('Page_Title', $rs))
				ew_AddFilter($where, ew_QuotedName('Page_Title') . '=' . ew_QuotedValue($rs['Page_Title'], $this->Page_Title->FldDataType));
		}
		$filter = $this->CurrentFilter;
		ew_AddFilter($filter, $where);
		if ($filter <> "")
	...
	function KeyFilter() {
		...
		$sKeyFilter = str_replace("@Page_Title@", ew_AdjustSql($this->Page_Title->CurrentValue), $sKeyFilter); // Replace key value
		return $sKeyFilter;
	}
	...
	function getReturnUrl() {
		...
		if (@$_SESSION[$name] <> "") {
			...
			return "breadcrumblinkslist.php";
		}
	}
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			return $this->KeyUrl("breadcrumblinksview.php", $this->UrlParm($parm));
		else
			return $this->KeyUrl("breadcrumblinksview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	}

	// Add URL
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			return "breadcrumblinksadd.php?" . $this->UrlParm($parm);
		else
			return "breadcrumblinksadd.php";
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		return $this->KeyUrl("breadcrumblinksedit.php", $this->UrlParm($parm));
	}
	...
	function GetCopyUrl($parm = "") {
		return $this->KeyUrl("breadcrumblinksadd.php", $this->UrlParm($parm));
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("breadcrumblinksdelete.php", $this->UrlParm());
	}
	...
	function KeyUrl($url, $parm = "") {
		$sUrl = $url . "?";
		if ($parm <> "") $sUrl .= $parm . "&";
		if (!is_null($this->Page_Title->CurrentValue)) {
			$sUrl .= "Page_Title=" . urlencode($this->Page_Title->CurrentValue);
		} else {
	...
	function GetRecordKeys() {
		...
		if (isset($_POST["key_m"])) {
			...
			$arKeys[] = @$_GET["Page_Title"]; // Page_Title
	...
	function GetKeyFilter() {
		...
		foreach ($arKeys as $key) {
			...
			$this->Page_Title->CurrentValue = $key;
		...
		if (!$Doc->ExportCustom) {
			...
			if ($Doc->Horizontal) { // Horizontal format, write header
				...
				if ($ExportPageType == "view") {
					...
					if ($this->Page_URL->Exportable) $Doc->ExportCaption($this->Page_URL);
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if (!$Doc->ExportCustom) {
					...
					if ($ExportPageType == "view") {
						...
						if ($this->Page_URL->Exportable) $Doc->ExportField($this->Page_URL);
