...
// End of mofidication Flexibility of Export Records Options, by Masino Sinaga, May 14, 2012
		...
		// Year
		$this->Year = new cField('stats_hour', 'stats_hour', 'x_Year', 'Year', '`Year`', '`Year`', 2, -1, FALSE, '`Year`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->Year->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['Year'] = &$this->Year;

		// Month
		$this->Month = new cField('stats_hour', 'stats_hour', 'x_Month', 'Month', '`Month`', '`Month`', 16, -1, FALSE, '`Month`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->Month->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['Month'] = &$this->Month;

		// Date
		$this->Date = new cField('stats_hour', 'stats_hour', 'x_Date', 'Date', '`Date`', '`Date`', 16, -1, FALSE, '`Date`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->Date->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['Date'] = &$this->Date;

		// Hour
		$this->Hour = new cField('stats_hour', 'stats_hour', 'x_Hour', 'Hour', '`Hour`', '`Hour`', 16, -1, FALSE, '`Hour`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->Hour->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['Hour'] = &$this->Hour;

		// Hits
		$this->Hits = new cField('stats_hour', 'stats_hour', 'x_Hits', 'Hits', '`Hits`', '`Hits`', 3, -1, FALSE, '`Hits`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
	...
	function DeleteSQL(&$rs, $where = "") {
		...
		if ($rs) {
			if (array_key_exists('Year', $rs))
				ew_AddFilter($where, ew_QuotedName('Year') . '=' . ew_QuotedValue($rs['Year'], $this->Year->FldDataType));
			if (array_key_exists('Month', $rs))
				ew_AddFilter($where, ew_QuotedName('Month') . '=' . ew_QuotedValue($rs['Month'], $this->Month->FldDataType));
			if (array_key_exists('Date', $rs))
				ew_AddFilter($where, ew_QuotedName('Date') . '=' . ew_QuotedValue($rs['Date'], $this->Date->FldDataType));
			if (array_key_exists('Hour', $rs))
				ew_AddFilter($where, ew_QuotedName('Hour') . '=' . ew_QuotedValue($rs['Hour'], $this->Hour->FldDataType));
		}
		$filter = $this->CurrentFilter;
		ew_AddFilter($filter, $where);
		if ($filter <> "")
	...
	function KeyFilter() {
		...
		if (!is_numeric($this->Hour->CurrentValue))
			$sKeyFilter = "0=1"; // Invalid key
		$sKeyFilter = str_replace("@Hour@", ew_AdjustSql($this->Hour->CurrentValue), $sKeyFilter); // Replace key value
		return $sKeyFilter;
	}
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			return $this->KeyUrl("stats_hourview.php", $this->UrlParm($parm));
		else
			return $this->KeyUrl("stats_hourview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	}

	// Add URL
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			return "stats_houradd.php?" . $this->UrlParm($parm);
		else
			return "stats_houradd.php";
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		return $this->KeyUrl("stats_houredit.php", $this->UrlParm($parm));
	}
	...
	function GetCopyUrl($parm = "") {
		return $this->KeyUrl("stats_houradd.php", $this->UrlParm($parm));
	}
	...
	function GetRecordKeys() {
		...
		if (isset($_POST["key_m"])) {
			...
			$arKey[] = @$_GET["Year"]; // Year
			$arKey[] = @$_GET["Month"]; // Month
			$arKey[] = @$_GET["Date"]; // Date
			$arKey[] = @$_GET["Hour"]; // Hour
			$arKeys[] = $arKey;
