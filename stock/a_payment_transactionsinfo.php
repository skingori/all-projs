...
// End of mofidication Flexibility of Export Records Options, by Masino Sinaga, May 14, 2012
		...
		// Payment_ID
		$this->Payment_ID = new cField('a_payment_transactions', 'a_payment_transactions', 'x_Payment_ID', 'Payment_ID', '`Payment_ID`', '`Payment_ID`', 3, -1, FALSE, '`Payment_ID`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->Payment_ID->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['Payment_ID'] = &$this->Payment_ID;

		// Ref_ID
		$this->Ref_ID = new cField('a_payment_transactions', 'a_payment_transactions', 'x_Ref_ID', 'Ref_ID', '`Ref_ID`', '`Ref_ID`', 200, -1, FALSE, '`Ref_ID`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['Ref_ID'] = &$this->Ref_ID;

		// Type
		$this->Type = new cField('a_payment_transactions', 'a_payment_transactions', 'x_Type', 'Type', '`Type`', '`Type`', 202, -1, FALSE, '`Type`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['Type'] = &$this->Type;

		// Customer
		$this->Customer = new cField('a_payment_transactions', 'a_payment_transactions', 'x_Customer', 'Customer', '`Customer`', '`Customer`', 200, -1, FALSE, '`Customer`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['Customer'] = &$this->Customer;

		// Supplier
		$this->Supplier = new cField('a_payment_transactions', 'a_payment_transactions', 'x_Supplier', 'Supplier', '`Supplier`', '`Supplier`', 200, -1, FALSE, '`Supplier`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['Supplier'] = &$this->Supplier;

		// Sub_Total
		$this->Sub_Total = new cField('a_payment_transactions', 'a_payment_transactions', 'x_Sub_Total', 'Sub_Total', '`Sub_Total`', '`Sub_Total`', 5, -1, FALSE, '`Sub_Total`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['Sub_Total'] = &$this->Sub_Total;

		// Payment
		$this->Payment = new cField('a_payment_transactions', 'a_payment_transactions', 'x_Payment', 'Payment', '`Payment`', '`Payment`', 5, -1, FALSE, '`Payment`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		...
		// Balance
		$this->Balance = new cField('a_payment_transactions', 'a_payment_transactions', 'x_Balance', 'Balance', '`Balance`', '`Balance`', 5, -1, FALSE, '`Balance`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['Balance'] = &$this->Balance;

		// Due_Date
		$this->Due_Date = new cField('a_payment_transactions', 'a_payment_transactions', 'x_Due_Date', 'Due_Date', '`Due_Date`', 'DATE_FORMAT(`Due_Date`, \'%Y/%m/%d %H:%i:%s\')', 133, 5, FALSE, '`Due_Date`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->Due_Date->FldDefaultErrMsg = str_replace("%s", "/", $Language->Phrase("IncorrectDateYMD"));
		$this->fields['Due_Date'] = &$this->Due_Date;

		// Date_Transaction
		$this->Date_Transaction = new cField('a_payment_transactions', 'a_payment_transactions', 'x_Date_Transaction', 'Date_Transaction', '`Date_Transaction`', 'DATE_FORMAT(`Date_Transaction`, \'%Y/%m/%d %H:%i:%s\')', 133, 5, FALSE, '`Date_Transaction`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		...
		// Date_Added
		$this->Date_Added = new cField('a_payment_transactions', 'a_payment_transactions', 'x_Date_Added', 'Date_Added', '`Date_Added`', 'DATE_FORMAT(`Date_Added`, \'%Y/%m/%d %H:%i:%s\')', 135, -1, FALSE, '`Date_Added`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['Date_Added'] = &$this->Date_Added;

		// Added_By
		$this->Added_By = new cField('a_payment_transactions', 'a_payment_transactions', 'x_Added_By', 'Added_By', '`Added_By`', '`Added_By`', 200, -1, FALSE, '`Added_By`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['Added_By'] = &$this->Added_By;

		// Date_Updated
		$this->Date_Updated = new cField('a_payment_transactions', 'a_payment_transactions', 'x_Date_Updated', 'Date_Updated', '`Date_Updated`', 'DATE_FORMAT(`Date_Updated`, \'%Y/%m/%d %H:%i:%s\')', 135, -1, FALSE, '`Date_Updated`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['Date_Updated'] = &$this->Date_Updated;

		// Updated_By
		$this->Updated_By = new cField('a_payment_transactions', 'a_payment_transactions', 'x_Updated_By', 'Updated_By', '`Updated_By`', '`Updated_By`', 200, -1, FALSE, '`Updated_By`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
	...
	function GetMasterFilter() {
		...
		if ($this->getCurrentMasterTable() == "view_sales_outstandings") {
			if ($this->Ref_ID->getSessionValue() <> "")
				$sMasterFilter .= "`Sales_Number`=" . ew_QuotedValue($this->Ref_ID->getSessionValue(), EW_DATATYPE_STRING);
			else
				return "";
		}
		if ($this->getCurrentMasterTable() == "view_purchases_outstandings") {
			if ($this->Ref_ID->getSessionValue() <> "")
				$sMasterFilter .= "`Purchase_Number`=" . ew_QuotedValue($this->Ref_ID->getSessionValue(), EW_DATATYPE_STRING);
			else
				return "";
		}
	...
	function GetDetailFilter() {
		...
		if ($this->getCurrentMasterTable() == "view_sales_outstandings") {
			if ($this->Ref_ID->getSessionValue() <> "")
				$sDetailFilter .= "`Ref_ID`=" . ew_QuotedValue($this->Ref_ID->getSessionValue(), EW_DATATYPE_STRING);
			else
				return "";
		}
		if ($this->getCurrentMasterTable() == "view_purchases_outstandings") {
	...
	// Update Table
	var $UpdateTable = "`a_payment_transactions`";
	...
	function DeleteSQL(&$rs, $where = "") {
		...
		if ($rs) {
			if (array_key_exists('Payment_ID', $rs))
				ew_AddFilter($where, ew_QuotedName('Payment_ID') . '=' . ew_QuotedValue($rs['Payment_ID'], $this->Payment_ID->FldDataType));
		}
		$filter = $this->CurrentFilter;
		ew_AddFilter($filter, $where);
		if ($filter <> "")
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			return $this->KeyUrl("a_payment_transactionsview.php", $this->UrlParm($parm));
		else
			return $this->KeyUrl("a_payment_transactionsview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	}

	// Add URL
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			return "a_payment_transactionsadd.php?" . $this->UrlParm($parm);
		else
			return "a_payment_transactionsadd.php";
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		return $this->KeyUrl("a_payment_transactionsedit.php", $this->UrlParm($parm));
	}
	...
	function GetCopyUrl($parm = "") {
		return $this->KeyUrl("a_payment_transactionsadd.php", $this->UrlParm($parm));
	}
