...
//
// Table class for a_sales_detail
//
class ca_sales_detail extends cTable {
	var $Sales_ID;
	var $Sales_Number;
	var $Supplier_Number;
	var $Stock_Item;
	...
	function __construct() {
		...
		$this->TableVar = 'a_sales_detail';
		$this->TableName = 'a_sales_detail';
...
// End of mofidication Flexibility of Export Records Options, by Masino Sinaga, May 14, 2012
		...
		// Sales_ID
		$this->Sales_ID = new cField('a_sales_detail', 'a_sales_detail', 'x_Sales_ID', 'Sales_ID', '`Sales_ID`', '`Sales_ID`', 3, -1, FALSE, '`Sales_ID`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->Sales_ID->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['Sales_ID'] = &$this->Sales_ID;

		// Sales_Number
		$this->Sales_Number = new cField('a_sales_detail', 'a_sales_detail', 'x_Sales_Number', 'Sales_Number', '`Sales_Number`', '`Sales_Number`', 200, -1, FALSE, '`Sales_Number`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['Sales_Number'] = &$this->Sales_Number;

		// Supplier_Number
		$this->Supplier_Number = new cField('a_sales_detail', 'a_sales_detail', 'x_Supplier_Number', 'Supplier_Number', '`Supplier_Number`', '`Supplier_Number`', 200, -1, FALSE, '`Supplier_Number`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['Supplier_Number'] = &$this->Supplier_Number;

		// Stock_Item
		$this->Stock_Item = new cField('a_sales_detail', 'a_sales_detail', 'x_Stock_Item', 'Stock_Item', '`Stock_Item`', '`Stock_Item`', 200, -1, FALSE, '`Stock_Item`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['Stock_Item'] = &$this->Stock_Item;

		// Sales_Quantity
		$this->Sales_Quantity = new cField('a_sales_detail', 'a_sales_detail', 'x_Sales_Quantity', 'Sales_Quantity', '`Sales_Quantity`', '`Sales_Quantity`', 5, -1, FALSE, '`Sales_Quantity`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['Sales_Quantity'] = &$this->Sales_Quantity;

		// Purchasing_Price
		$this->Purchasing_Price = new cField('a_sales_detail', 'a_sales_detail', 'x_Purchasing_Price', 'Purchasing_Price', '`Purchasing_Price`', '`Purchasing_Price`', 5, -1, FALSE, '`Purchasing_Price`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['Purchasing_Price'] = &$this->Purchasing_Price;

		// Sales_Price
		$this->Sales_Price = new cField('a_sales_detail', 'a_sales_detail', 'x_Sales_Price', 'Sales_Price', '`Sales_Price`', '`Sales_Price`', 5, -1, FALSE, '`Sales_Price`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['Sales_Price'] = &$this->Sales_Price;

		// Sales_Total_Amount
		$this->Sales_Total_Amount = new cField('a_sales_detail', 'a_sales_detail', 'x_Sales_Total_Amount', 'Sales_Total_Amount', '`Sales_Total_Amount`', '`Sales_Total_Amount`', 5, -1, FALSE, '`Sales_Total_Amount`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
	...
	function GetMasterFilter() {
		...
		// Master filter
		$sMasterFilter = "";
		if ($this->getCurrentMasterTable() == "a_sales") {
			if ($this->Sales_Number->getSessionValue() <> "")
				$sMasterFilter .= "`Sales_Number`=" . ew_QuotedValue($this->Sales_Number->getSessionValue(), EW_DATATYPE_STRING);
			else
				return "";
		}
	...
	function GetDetailFilter() {
		...
		if ($this->getCurrentMasterTable() == "a_sales") {
			if ($this->Sales_Number->getSessionValue() <> "")
				$sDetailFilter .= "`Sales_Number`=" . ew_QuotedValue($this->Sales_Number->getSessionValue(), EW_DATATYPE_STRING);
			else
				return "";
		}
	...
	// Master filter
	function SqlMasterFilter_a_sales() {
		return "`Sales_Number`='@Sales_Number@'";
	}

	// Detail filter
	function SqlDetailFilter_a_sales() {
		return "`Sales_Number`='@Sales_Number@'";
	}
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`a_sales_detail`";
	}
	...
	// Update Table
	var $UpdateTable = "`a_sales_detail`";
	...
	function DeleteSQL(&$rs, $where = "") {
		...
		if ($rs) {
			if (array_key_exists('Sales_ID', $rs))
				ew_AddFilter($where, ew_QuotedName('Sales_ID') . '=' . ew_QuotedValue($rs['Sales_ID'], $this->Sales_ID->FldDataType));
		}
		$filter = $this->CurrentFilter;
		ew_AddFilter($filter, $where);
		if ($filter <> "")
	...
	function KeyFilter() {
		...
		$sKeyFilter = str_replace("@Sales_ID@", ew_AdjustSql($this->Sales_ID->CurrentValue), $sKeyFilter); // Replace key value
		return $sKeyFilter;
	}
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			return $this->KeyUrl("a_sales_detailview.php", $this->UrlParm($parm));
		else
			return $this->KeyUrl("a_sales_detailview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	}

	// Add URL
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			return "a_sales_detailadd.php?" . $this->UrlParm($parm);
		else
			return "a_sales_detailadd.php";
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		return $this->KeyUrl("a_sales_detailedit.php", $this->UrlParm($parm));
	}
	...
	function GetCopyUrl($parm = "") {
		return $this->KeyUrl("a_sales_detailadd.php", $this->UrlParm($parm));
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("a_sales_detaildelete.php", $this->UrlParm());
	}
   ...
   // Common render codes
		...
		if (preg_match('/^x(\d)*_Stock_Item$/', $id)) {
			switch (@$gsLanguage) {
				case "id":
					...
					$sWhereWrk = "(`Stock_Number` = '" . ew_AdjustSql($val) . "')";
					break;
				default:
					$sSqlWrk = "SELECT `Purchasing_Price` AS FIELD0, `Selling_Price` AS FIELD1 FROM `a_stock_items`";
					$sWhereWrk = "(`Stock_Number` = '" . ew_AdjustSql($val) . "')";
			...
			if ($rs = ew_LoadRecordset($sSqlWrk)) {
				while ($rs && !$rs->EOF) {
					...
					$this->RenderEditRow();
					$ar[] = (!$this->Purchasing_Price->AutoFillOriginalValue && !is_null($this->Purchasing_Price->EditValue)) ? $this->Purchasing_Price->EditValue : $this->Purchasing_Price->CurrentValue; // http://www.hkvforums.com/viewtopic.php?f=4&t=35232
					$ar[] = (!$this->Sales_Price->AutoFillOriginalValue && !is_null($this->Sales_Price->EditValue)) ? $this->Sales_Price->EditValue : $this->Sales_Price->CurrentValue; // http://www.hkvforums.com/viewtopic.php?f=4&t=35232
					$rowcnt += 1;
					$rsarr[] = $ar;
					$rs->MoveNext();
				}
	...
	function Row_Inserting($rsold, &$rsnew) {
		...
		$sales_number = ($rsold["Sales_Number"] <> "") ? $rsold["Sales_Number"] : $rsnew["Sales_Number"];
		$stock_item = ($rsold["Stock_Item"] <> "") ? $rsold["Stock_Item"] : $rsnew["Stock_Item"];
		$check_stock = ew_ExecuteScalar("SELECT Stock_Item FROM a_sales_detail WHERE Sales_Number = '".$sales_number."' AND Stock_Item = '".$stock_item."'");
	...
	function Row_Inserted($rsold, &$rsnew) {
		...
		//echo "Row Inserted"
		ew_Execute("UPDATE a_stock_items SET Quantity = Quantity - ".$rsnew["Sales_Quantity"]." WHERE Stock_Number = '".$rsnew["Stock_Item"]."'");
		$final_total_amount = ew_ExecuteScalar("SELECT Final_Total_Amount FROM a_sales WHERE Sales_Number = '".$rsnew["Sales_Number"]."'");	
		ew_Execute("UPDATE a_sales SET Final_Total_Amount = ".$final_total_amount." WHERE Sales_Number = '".$rsnew["Sales_Number"]."'");
		$total_payment = ew_ExecuteScalar("SELECT Total_Payment FROM a_sales WHERE Sales_Number = '".$rsnew["Sales_Number"]."'");
		$final_total_amount = ew_ExecuteScalar("SELECT Final_Total_Amount FROM a_sales WHERE Sales_Number = '".$rsnew["Sales_Number"]."'");
		$total_balance = $final_total_amount - $total_payment;
		ew_Execute("UPDATE a_sales SET Total_Balance = ".$total_balance." WHERE Sales_Number = '".$rsnew["Sales_Number"]."'");
		if ($total_balance <> 0) {
			$customer_number = GetCustomerNumber($rsnew["Sales_Number"]);
			$customer_balance = ew_ExecuteScalar("SELECT SUM(Total_Balance) FROM a_sales WHERE Customer_ID = '".$customer_number."'");
			ew_Execute("UPDATE a_customers SET Balance = ".$customer_balance." WHERE Customer_Number = '".$customer_number."'");
		}
	}
	...
	function Row_Updated($rsold, &$rsnew) {
		...
		//echo "Row Updated";
		$sales_total = ew_ExecuteScalar("SELECT SUM(Sales_Total_Amount) FROM a_sales_detail WHERE Sales_Number = '".$rsold["Sales_Number"]."'");
		ew_Execute("UPDATE a_sales SET Total_Amount = ".$sales_total." WHERE Sales_Number = '".$rsold["Sales_Number"]."'");
		$total_payment = ew_ExecuteScalar("SELECT Total_Payment FROM a_sales WHERE Sales_Number = '".$rsold["Sales_Number"]."'");
		$final_total_amount = ew_ExecuteScalar("SELECT Final_Total_Amount FROM a_sales WHERE Sales_Number = '".$rsold["Sales_Number"]."'");
		$total_balance = $final_total_amount - $total_payment;
		ew_Execute("UPDATE a_sales SET Total_Balance = ".$total_balance." WHERE Sales_Number = '".$rsold["Sales_Number"]."'");
		if ($rsold["Sales_Quantity"] <> $rsnew["Sales_Quantity"]) {
			$sales_total = ew_ExecuteScalar("SELECT SUM(Sales_Total_Amount) FROM a_sales_detail WHERE Sales_Number = '".$rsold["Sales_Number"]."'");
			ew_Execute("UPDATE a_sales SET Total_Amount = ".$sales_total." WHERE Sales_Number = '".$rsold["Sales_Number"]."'");
			$total_payment = ew_ExecuteScalar("SELECT Total_Payment FROM a_sales WHERE Sales_Number = '".$rsold["Sales_Number"]."'");
			$final_total_amount = ew_ExecuteScalar("SELECT Final_Total_Amount FROM a_sales WHERE Sales_Number = '".$rsold["Sales_Number"]."'");
	...
	function Grid_Inserting() {
		...
		foreach ($rsnew as $row) { // Loop through the new records		
			$stock_available = GetStockItemsQuantity($row["Stock_Item"]);
			if ($stock_available < $row["Sales_Quantity"]) {
				$rs = ew_Execute("SELECT Sales_Number FROM a_sales_detail WHERE Sales_Number = '".$row["Sales_Number"]."'");
				if ($rs && $rs->RecordCount() < 1) {
					ew_Execute("UPDATE a_sales SET Total_Amount = 0, Total_Payment = 0, Final_Total_Amount = 0 WHERE Sales_Number = '".$row["Sales_Number"]."'");
				}
				$this->setFailureMessage("Stock <strong>".$stock_item."</strong> is only ".$stock_available." available; not enough for sale.");
				return FALSE;
			}
		}
		return TRUE;
	...
	function Row_Deleted(&$rs) {
		...
		//echo "Row Deleted";
		$sales_total = ew_ExecuteScalar("SELECT SUM(Sales_Total_Amount) FROM a_sales_detail WHERE Sales_Number = '".$rs["Sales_Number"]."'");
		$val_sales_total = ($sales_total > 0) ? $sales_total : 0; 
		ew_Execute("UPDATE a_sales SET Total_Amount = ".$val_sales_total." WHERE Sales_Number = '".$rs["Sales_Number"]."'");
		$total_payment = ew_ExecuteScalar("SELECT Total_Payment FROM a_sales WHERE Sales_Number = '".$rs["Sales_Number"]."'");
		$total_amount = ew_ExecuteScalar("SELECT Total_Amount FROM a_sales WHERE Sales_Number = '".$rs["Sales_Number"]."'");
		$total_balance = $total_amount - $total_payment;
		ew_Execute("UPDATE a_sales SET Total_Balance = ".$total_balance." WHERE Sales_Number = '".$rs["Sales_Number"]."'");
	}
	...
	function Row_Rendered() {
		...
		$this->Sales_Quantity->EditAttrs["onkeyup"] = "CalculateGrid(event);GetAmountTotalFinal();GetBalanceTotal();";
		$this->Sales_Quantity->EditAttrs["onclick"] = "CalculateGrid(event);GetAmountTotalFinal();GetBalanceTotal();";
		$this->Sales_Price->EditAttrs["onkeyup"] = "CalculateGrid(event);GetAmountTotalFinal();GetBalanceTotal();";
		$this->Sales_Price->EditAttrs["onclick"] = "CalculateGrid(event);GetAmountTotalFinal();GetBalanceTotal();";
		$this->Sales_Quantity->EditAttrs["onfocus"] = "SetFocusTextGrid(event, 'Sales_Quantity');";
		if ( (CurrentPageID() == "edit" && !isset($_GET["showdetail"])) || (CurrentPageID() == "add" && !isset($_GET["showdetail"])) ) {
			$this->Sales_Quantity->EditAttrs["onkeyup"] = "CalculateSingleSales();";
			$this->Sales_Price->EditAttrs["onchange"] = "CalculateSingleSales();";
		}
