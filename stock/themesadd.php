...
$themes_add = NULL; // Initialize page object first

class cthemes_add extends cthemes {
...
// End of modification Disable/Enable Registration Page, by Masino Sinaga, May 14, 2012
	...
	function Page_Main() {
		...
		if (@$_POST["a_add"] <> "") {
			...
			if (@$_GET["Theme_ID"] != "") {
				...
				$this->setKey("Theme_ID", $this->Theme_ID->CurrentValue); // Set up key
			} else {
				$this->setKey("Theme_ID", ""); // Clear key
		...
		switch ($this->CurrentAction) {
			...
			case "C": // Copy an existing record
				if (!$this->LoadRow()) { // Load record based on key
					...
					$this->Page_Terminate("themeslist.php"); // No matching record, return to list
				}
				break;
			case "A": // Add new record
				...
				if ($this->AddRow($this->OldRecordset)) { // Add successful
					...
					if (ew_GetPageName($sReturnUrl) == "themesview.php")
	...
	function LoadDefaultValues() {
		$this->Theme_ID->CurrentValue = NULL;
		$this->Theme_ID->OldValue = $this->Theme_ID->CurrentValue;
		$this->Theme_Name->CurrentValue = NULL;
		$this->Theme_Name->OldValue = $this->Theme_Name->CurrentValue;
	...
	function RenderRow() {
		...
		if ($this->RowType == EW_ROWTYPE_VIEW) { // View row
			...
			$this->Theme_ID->EditValue = ew_HtmlEncode($this->Theme_ID->CurrentValue);
			$this->Theme_ID->PlaceHolder = ew_RemoveHtml($this->Theme_ID->FldCaption());
	...
	function AddRow($rsold = NULL) {
		...
		// Theme_ID
		$this->Theme_ID->SetDbValueDef($rsnew, $this->Theme_ID->CurrentValue, "", FALSE);

		// Theme_Name
		$this->Theme_Name->SetDbValueDef($rsnew, $this->Theme_Name->CurrentValue, "", FALSE);
		...
		if ($bInsertRow && $this->ValidateKey && strval($rsnew['Theme_ID']) == "") {
...
// Create page object
if (!isset($themes_add)) $themes_add = new cthemes_add();

// Page init
$themes_add->Page_Init();

// Page main
$themes_add->Page_Main();
...
// Form object
var fthemesadd = new ew_Form("fthemesadd");

// Validate form
fthemesadd.Validate = function() {
...
fthemesadd.Form_CustomValidate = 
...
fthemesadd.ValidateRequired = true;
<?php } else { ?>
fthemesadd.ValidateRequired = false; 
<?php } ?>
...
<?php $themes_add->ShowPageHeader(); ?>
<?php
$themes_add->ShowMessage();
?>
<form name="fthemesadd" id="fthemesadd" class="form-horizontal ewForm ewAddForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($themes_add->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $themes_add->Token ?>">
<?php } ?>
...
<input type="text" data-field="x_Theme_ID" name="x_Theme_ID" id="x_Theme_ID" size="30" maxlength="25" placeholder="<?php echo ew_HtmlEncode($themes->Theme_ID->PlaceHolder) ?>" value="<?php echo $themes->Theme_ID->EditValue ?>"<?php echo $themes->Theme_ID->EditAttributes() ?>>
</span>
...
<input type="text" data-field="x_Theme_Name" name="x_Theme_Name" id="x_Theme_Name" size="30" maxlength="25" placeholder="<?php echo ew_HtmlEncode($themes->Theme_Name->PlaceHolder) ?>" value="<?php echo $themes->Theme_Name->EditValue ?>"<?php echo $themes->Theme_Name->EditAttributes() ?>>
</span>
...
<?php
$selwrk = (ew_ConvertToBool($themes->_Default->CurrentValue)) ? " checked=\"checked\"" : "";
?>
<input type="checkbox" data-field="x__Default" name="x__Default[]" id="x__Default[]" value="1"<?php echo $selwrk ?><?php echo $themes->_Default->EditAttributes() ?>>
</span>
...
<script type="text/javascript">
$(document).ready(function(){$("#fthemesadd:first *:input[type!=hidden]:first").focus(),$("input").keydown(function(i){if(13==i.which){var e=$(this).closest("form").find(":input:visible:enabled"),n=e.index(this);n==e.length-1||(e.eq(e.index(this)+1).focus(),i.preventDefault())}else 113==i.which&&$("#btnAction").click()}),$("select").keydown(function(i){if(13==i.which){var e=$(this).closest("form").find(":input:visible:enabled"),n=e.index(this);n==e.length-1||(e.eq(e.index(this)+1).focus(),i.preventDefault())}else 113==i.which&&$("#btnAction").click()}),$("radio").keydown(function(i){if(13==i.which){var e=$(this).closest("form").find(":input:visible:enabled"),n=e.index(this);n==e.length-1||(e.eq(e.index(this)+1).focus(),i.preventDefault())}else 113==i.which&&$("#btnAction").click()})});
</script>
<?php } ?>
<?php if ($themes->Export == "") { ?>
<script type="text/javascript">
$('#btnAction').attr('onclick', 'return alertifyAdd(this)'); function alertifyAdd(obj) { <?php global $Language; ?> if (fthemesadd.Validate() == true ) { alertify.set({buttonFocus: 'cancel'});alertify.confirm("<?php echo $Language->Phrase('AlertifyAddConfirm'); ?>", function (e) { if (e) { $(window).unbind('beforeunload'); alertify.success("<?php echo $Language->Phrase('AlertifyAdd'); ?>"); $("#fthemesadd").submit(); } else { alertify.error("<?php echo $Language->Phrase('AlertifyCancel'); ?>"); } }, "<?php echo $Language->Phrase('AlertifyConfirm'); ?>"); } return false; }
</script>
<?php } ?>
<?php include_once "footer.php" ?>
<?php
$themes_add->Page_Terminate();
?>
