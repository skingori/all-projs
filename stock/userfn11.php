...
function My_Global_Check() {
	global $Language, $gsLanguage, $Security;
	global $page_type;
    $page_type = "TABLE"; 
	...
	if (MS_USE_CONSTANTS_IN_CONFIG_FILE==FALSE) {
		...
		// if ($rs && $rs->RecordCount() > 0) {					
			...
			// This is a must to override the menu constants in the beginning, by Masino Sinaga, July 23, 2014
			if (MS_MENU_HORIZONTAL) {
				@define("MS_TOTAL_WIDTH", MS_SCROLL_TABLE_WIDTH + 50, FALSE);
			} else {  
		...
		if ($rsa && $rsa->RecordCount() > 0) {
			...
			while (!$rsa->EOF) {
				if (IsDateBetweenTwoDates($today_begin, $today_end, $rsa->fields("Date_Start"), $rsa->fields("Date_End"))) {
					...
					ew_Execute("UPDATE ".MS_ANNOUNCEMENT_TABLE." SET Is_Active = 'N'"); // reset all become Not Active
					ew_Execute("UPDATE ".MS_ANNOUNCEMENT_TABLE." SET Is_Active = 'Y' 
				...
				if ($date_end != "") { // Assuming end of maintenance date/time is valid
					if ($date_end<=$date_now) {
						if (MS_AUTO_NORMAL_AFTER_MAINTENANCE==TRUE) {
							...
							echo $cssfile;
							echo '<meta name="generator" content="PHPMaker v11.0.5">
</head>';
...
function getCurrentPageTitle($pt) {
	...
	if (@MS_USE_CONSTANTS_IN_CONFIG_FILE == FALSE) {
		$sSql = "SELECT C.Page_Title
				...
				ORDER BY C.Lft";
		if (($rs = ew_Execute($sSql)) && ($rs->RecordCount() > 0)) {
		...
		$CurrentPageTitle = CurrentPage()->TableName . " &laquo; " . $Language->ProjectPhrase("BodyTitle");
	}
}
...
function GetIntersectTwoDates($sDateCheckBegin, $sDateCheckEnd, $sLang) {
    $sResult = "";
	$rs = ew_Execute("SELECT Announcement_ID, Date_Start, Date_End
...
function UpdateDatesInOtherLanguage($sDateBegin, $sDateEnd, $iID) {
    $sResult = "";
	$rs = ew_Execute("UPDATE " . MS_ANNOUNCEMENT_TABLE . " 
