...
$a_stock_items_grid = NULL; // Initialize page object first

class ca_stock_items_grid extends ca_stock_items {
	...
	// Page object name
	var $PageObjName = 'a_stock_items_grid';

	// Grid form hidden field names
	var $FormName = 'fa_stock_itemsgrid';
...
//			if (!isset($GLOBALS["Table"])) $GLOBALS["Table"] = &$GLOBALS["a_stock_items"];
...
//		$GLOBALS["Table"] = &$GLOBALS["MasterTable"];
		...
		for ($rowindex = 1; $rowindex <= $rowcnt; $rowindex++) {
			...
			if (!$this->EmptyRow()) {
				...
				if ($bGridInsert) {
					...
					$sKey .= $this->Stock_ID->CurrentValue;
	...
	function EmptyRow() {
		...
		if ($objForm->HasValue("x_Supplier_Number") && $objForm->HasValue("o_Supplier_Number") && $this->Supplier_Number->CurrentValue <> $this->Supplier_Number->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_Stock_Number") && $objForm->HasValue("o_Stock_Number") && $this->Stock_Number->CurrentValue <> $this->Stock_Number->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_Stock_Name") && $objForm->HasValue("o_Stock_Name") && $this->Stock_Name->CurrentValue <> $this->Stock_Name->OldValue)
			return FALSE;
	...
	function RenderListOptions() {
		...
		if ($this->CurrentMode == "edit" && is_numeric($this->RowIndex)) {
			$this->MultiSelectKey .= "<input type=\"hidden\" name=\"" . $KeyName . "\" id=\"" . $KeyName . "\" value=\"" . $this->Stock_ID->CurrentValue . "\">";
		}
	...
	function LoadFormValues() {
		...
		$this->Stock_Number->setOldValue($objForm->GetValue("o_Stock_Number"));
		if (!$this->Stock_Name->FldIsDetailKey) {
			$this->Stock_Name->setFormValue($objForm->GetValue("x_Stock_Name"));
		}
		$this->Stock_Name->setOldValue($objForm->GetValue("o_Stock_Name"));
		...
		if (!$this->Stock_ID->FldIsDetailKey && $this->CurrentAction <> "gridadd" && $this->CurrentAction <> "add")
	...
	function LoadOldRecord() {
		...
		if ($cnt >= 1) {
			if (strval($arKeys[0]) <> "")
				$this->Stock_ID->CurrentValue = strval($arKeys[0]); // Stock_ID
			else
				$bValidKey = FALSE;
		} else {
			$bValidKey = FALSE;
	...
	function RenderRow() {
		...
		if ($this->RowType == EW_ROWTYPE_VIEW) { // View row
			...
			$this->Quantity->EditValue = ew_FormatNumber($this->Quantity->EditValue, -2, -2, -2, -2);
	...
	function SetUpMasterParms() {
		...
		if ($sMasterTblVar == "a_suppliers") {
			$this->Supplier_Number->Visible = FALSE;
