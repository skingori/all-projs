...
// End of modification Disable/Enable Registration Page, by Masino Sinaga, May 14, 2012
		...
		switch ($this->CurrentAction) {
			...
			case "A": // Add new record
				...
				if ($this->AddRow($this->OldRecordset)) { // Add successful
					...
					if (ew_GetPageName($sReturnUrl) == "a_sales_detailview.php")
	...
	function AddRow($rsold = NULL) {
		...
		if (strval($this->Sales_Number->CurrentValue) <> "") {
			$sMasterFilter = str_replace("@Sales_Number@", ew_AdjustSql($this->Sales_Number->CurrentValue), $sMasterFilter);
		} else {
...
// Page object
var a_sales_detail_add = new ew_Page("a_sales_detail_add");
a_sales_detail_add.PageID = "add"; // Page ID
var EW_PAGE_ID = a_sales_detail_add.PageID; // For backward compatibility

// Form object
var fa_sales_detailadd = new ew_Form("fa_sales_detailadd");
...
fa_sales_detailadd.Lists["x_Supplier_Number"] = {"LinkField":"x_Supplier_Number","Ajax":null,"AutoFill":false,"DisplayFields":["x_Supplier_Name","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
fa_sales_detailadd.Lists["x_Stock_Item"] = {"LinkField":"x_Stock_Number","Ajax":null,"AutoFill":true,"DisplayFields":["x_Stock_Name","","",""],"ParentFields":["x_Supplier_Number"],"FilterFields":["x_Supplier_Number"],"Options":[]};
...
?>
<form name="fa_sales_detailadd" id="fa_sales_detailadd" class="form-horizontal ewForm ewAddForm" action="<?php echo ew_CurrentPage() ?>" method="post">
...
<?php } else { ?>
<span id="el_a_sales_detail_Sales_Number">
<input type="text" data-field="x_Sales_Number" name="x_Sales_Number" id="x_Sales_Number" size="30" maxlength="20" placeholder="<?php echo ew_HtmlEncode($a_sales_detail->Sales_Number->PlaceHolder) ?>" value="<?php echo $a_sales_detail->Sales_Number->EditValue ?>"<?php echo $a_sales_detail->Sales_Number->EditAttributes() ?>>
</span>
<?php } ?>
...
<?php $a_sales_detail->Supplier_Number->EditAttrs["onchange"] = "ew_UpdateOpt.call(this, ['x_Stock_Item']); " . @$a_sales_detail->Supplier_Number->EditAttrs["onchange"]; ?>
<select data-field="x_Supplier_Number" id="x_Supplier_Number" name="x_Supplier_Number"<?php echo $a_sales_detail->Supplier_Number->EditAttributes() ?>>
<?php
...
<script type="text/javascript">
fa_sales_detailadd.Lists["x_Supplier_Number"].Options = <?php echo (is_array($a_sales_detail->Supplier_Number->EditValue)) ? ew_ArrayToJson($a_sales_detail->Supplier_Number->EditValue, 1) : "[]" ?>;
</script>
</span>
...
<select data-field="x_Stock_Item" id="x_Stock_Item" name="x_Stock_Item"<?php echo $a_sales_detail->Stock_Item->EditAttributes() ?>>
<?php
...
<script type="text/javascript">
fa_sales_detailadd.Lists["x_Stock_Item"].Options = <?php echo (is_array($a_sales_detail->Stock_Item->EditValue)) ? ew_ArrayToJson($a_sales_detail->Stock_Item->EditValue, 1) : "[]" ?>;
</script>
...
<input type="text" data-field="x_Sales_Quantity" name="x_Sales_Quantity" id="x_Sales_Quantity" size="30" placeholder="<?php echo ew_HtmlEncode($a_sales_detail->Sales_Quantity->PlaceHolder) ?>" value="<?php echo $a_sales_detail->Sales_Quantity->EditValue ?>"<?php echo $a_sales_detail->Sales_Quantity->EditAttributes() ?>>
...
<input type="text" data-field="x_Purchasing_Price" name="x_Purchasing_Price" id="x_Purchasing_Price" size="30" placeholder="<?php echo ew_HtmlEncode($a_sales_detail->Purchasing_Price->PlaceHolder) ?>" value="<?php echo $a_sales_detail->Purchasing_Price->EditValue ?>"<?php echo $a_sales_detail->Purchasing_Price->EditAttributes() ?>>
...
<input type="text" data-field="x_Sales_Price" name="x_Sales_Price" id="x_Sales_Price" size="30" placeholder="<?php echo ew_HtmlEncode($a_sales_detail->Sales_Price->PlaceHolder) ?>" value="<?php echo $a_sales_detail->Sales_Price->EditValue ?>"<?php echo $a_sales_detail->Sales_Price->EditAttributes() ?>>
...
<script type="text/javascript">
$('#btnAction').attr('onclick', 'return alertifyAdd(this)'); function alertifyAdd(obj) { <?php global $Language; ?> if (fa_sales_detailadd.Validate() == true ) { alertify.set({buttonFocus: 'cancel'});alertify.confirm("<?php echo $Language->Phrase('AlertifyAddConfirm'); ?>", function (e) { if (e) { $(window).unbind('beforeunload'); alertify.success("<?php echo $Language->Phrase('AlertifyAdd'); ?>"); $("#fa_sales_detailadd").submit(); } else { alertify.error("<?php echo $Language->Phrase('AlertifyCancel'); ?>"); } }, "<?php echo $Language->Phrase('AlertifyConfirm'); ?>"); } return false; }
</script>
<?php } ?>
