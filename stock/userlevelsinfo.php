...
// End of mofidication Flexibility of Export Records Options, by Masino Sinaga, May 14, 2012
		...
		// User_Level_ID
		$this->User_Level_ID = new cField('userlevels', 'userlevels', 'x_User_Level_ID', 'User_Level_ID', '`User_Level_ID`', '`User_Level_ID`', 3, -1, FALSE, '`User_Level_ID`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->User_Level_ID->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['User_Level_ID'] = &$this->User_Level_ID;

		// User_Level_Name
		$this->User_Level_Name = new cField('userlevels', 'userlevels', 'x_User_Level_Name', 'User_Level_Name', '`User_Level_Name`', '`User_Level_Name`', 200, -1, FALSE, '`User_Level_Name`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
	...
	function DeleteSQL(&$rs, $where = "") {
		...
		if ($rs) {
			if (array_key_exists('User_Level_ID', $rs))
				ew_AddFilter($where, ew_QuotedName('User_Level_ID') . '=' . ew_QuotedValue($rs['User_Level_ID'], $this->User_Level_ID->FldDataType));
		}
		$filter = $this->CurrentFilter;
		ew_AddFilter($filter, $where);
		if ($filter <> "")
	...
	function KeyFilter() {
		...
		$sKeyFilter = str_replace("@User_Level_ID@", ew_AdjustSql($this->User_Level_ID->CurrentValue), $sKeyFilter); // Replace key value
		return $sKeyFilter;
	}
	...
	function GetRecordKeys() {
		...
		if (isset($_POST["key_m"])) {
			...
			$arKeys[] = @$_GET["User_Level_ID"]; // User_Level_ID
