...
$(document).ready(function() {
	alertify.set({labels:{ok:"<?php echo $Language->Phrase("MyOKMessage"); ?>",cancel:"<?php echo $Language->Phrase("MyCancelMessage"); ?>"}});
...
function AskToLogout() {
	alertify.confirm("<?php echo $Language->Phrase('AskToLogout'); ?>",function(e){if(e){alertify.success("<?php echo $Language->Phrase('AlertifyProcessing'); ?>"); document.location.href = 'logout.php';}}, "<?php echo $Language->Phrase('Logout'); ?>");alertify.set({buttonFocus: "ok"});
}
</script>
<?php ew_AddClientScript("phpjs/helpdialog.js"); // Add JavaScript ?>
<?php ew_AddStylesheet("alertify/alertify.core.css"); // Add CSS stylesheet ?>
<?php if($Language->Phrase("dir")=="rtl") { ?>
<?php if (MS_ALERTIFY_TRANSITION_STYLE == "fade") { ?>
<?php ew_AddStylesheet("alertify/alertify.bootstrap-rtl.css"); // Add CSS stylesheet ?>
<?php } else { ?>
<?php ew_AddStylesheet("alertify/alertify.bootstrap.zoom-rtl.css"); // Add CSS stylesheet ?>
<?php } ?>
<?php } else { ?>
<?php if (MS_ALERTIFY_TRANSITION_STYLE == "fade") { ?>
<?php ew_AddStylesheet("alertify/alertify.bootstrap.css"); // Add CSS stylesheet ?>
<?php } else { ?>
<?php ew_AddStylesheet("alertify/alertify.bootstrap.zoom.css"); // Add CSS stylesheet ?>
<?php } ?>
<?php } ?>
<?php ew_AddClientScript("alertify/alertify.min.js"); // Add JavaScript ?>
<?php ew_AddStylesheet("bootstrap3/css/bootstrap-modal.css"); // Add CSS stylesheet ?>
<?php
if (MS_DETECT_CHANGES_ON_ADD_FORM) {
  if (CurrentPageID() == "add") {
    ew_AddClientScript("phpjs/changes.js"); // Add JavaScript
  }
}
...
<div class="ewLayout">
	...
	<div id="ewHeaderRow" class="MasinoHeaderRow <?php echo $gsHeaderRowClass ?>">
		...
		<div class="<?php echo MS_HEADER_TEXT_CLASS; ?>" style="height: 20px;">
				<?php if (MS_TEXT_ALIGN_IN_HEADER == "left") { ?>
				  <div class="ewHeaderRow pull-left" id="msUserName" style="padding-top: 8px; padding-bottom: 4px;"><font color="white"><?php if ($Security->IsLoggedIn()=="login") { echo "". $Language->Phrase("UserName").": <strong>" . $Security->CurrentUserName() . "</strong> | <a id=\"logout\" href=\"javascript:void(0);\" onclick=\"alertify.set({buttonFocus: 'cancel'});AskToLogout();return false;\">Logout</a>"; } ?></font></div><br>
				<?php } elseif (MS_TEXT_ALIGN_IN_HEADER == "right") { ?>
				  <div class="ewHeaderRow pull-right" id="msUserName" style="padding-top: 8px; padding-bottom: 4px;"><font color="white"><?php if ($Security->IsLoggedIn()=="login") { echo "". $Language->Phrase("UserName").": <strong>" . $Security->CurrentUserName() . "</strong> | <a id=\"logout\" href=\"javascript:void(0);\" onclick=\"alertify.set({buttonFocus: 'cancel'});AskToLogout();return false;\">Logout</a>"; } ?></font></div><br>
				<?php } else { ?>
				  <div align="center" id="msUserName" style="padding-top: 8px; padding-bottom: 4px;"><strong><font color="white"><?php if ($Security->IsLoggedIn()=="login") { echo "". $Language->Phrase("UserName").": <strong>" . $Security->CurrentUserName() . "</strong> | <a id=\"logout\" href=\"javascript:void(0);\" onclick=\"alertify.set({buttonFocus: 'cancel'});AskToLogout();return false;\">Logout</a>"; } ?></font></strong></div>
				<?php } ?>
		</div>
		<?php } ?>
