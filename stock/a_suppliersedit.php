...
// Page object
var a_suppliers_edit = new ew_Page("a_suppliers_edit");
a_suppliers_edit.PageID = "edit"; // Page ID
var EW_PAGE_ID = a_suppliers_edit.PageID; // For backward compatibility

// Form object
var fa_suppliersedit = new ew_Form("fa_suppliersedit");
...
<form name="fa_suppliersedit" id="fa_suppliersedit" class="form-horizontal ewForm ewEditForm" action="<?php echo ew_CurrentPage() ?>" method="post">
...
<input type="hidden" data-field="x_Supplier_ID" name="x_Supplier_ID" id="x_Supplier_ID" value="<?php echo ew_HtmlEncode($a_suppliers->Supplier_ID->CurrentValue) ?>">
...
<script type="text/javascript">
$('#btnAction').attr('onclick', 'return alertifyEdit(this)'); function alertifyEdit(obj) { <?php global $Language; ?> if (fa_suppliersedit.Validate() == true ) { alertify.set({buttonFocus: 'cancel'});alertify.confirm("<?php echo $Language->Phrase('AlertifyEditConfirm'); ?>", function (e) {	if (e) { $(window).unbind('beforeunload'); alertify.success("<?php echo $Language->Phrase('AlertifyEdit'); ?>"); $("#fa_suppliersedit").submit(); } else {	alertify.error("<?php echo $Language->Phrase('AlertifyCancel'); ?>"); }	}, "<?php echo $Language->Phrase('AlertifyConfirm'); ?>");	} return false; }
</script>
<?php } ?>
