...
$a_customers_view = NULL; // Initialize page object first

class ca_customers_view extends ca_customers {
	...
	// Page object name
	var $PageObjName = 'a_customers_view';
...
// End of modification Disable/Enable Registration Page, by Masino Sinaga, May 14, 2012
	...
	function Page_Main() {
		...
		if ($this->IsPageRequest()) { // Validate request
			...
			switch ($this->CurrentAction) {
				case "I": // Get a record to display
					...
					if (!$bMatchRecord) {
						if ($this->getSuccessMessage() == "" && $this->getFailureMessage() == "")
							$this->setFailureMessage($Language->Phrase("NoRecord")); // Set no record message
						$sReturnUrl = "a_customerslist.php"; // No matching record, return to list
					} else {
			...
			$sReturnUrl = "a_customerslist.php"; // Not page request, return to list
		}
		if ($sReturnUrl <> "")
	...
	function SetupOtherOptions() {
		...
		if ( $this->a_sales_Count > 0 && MS_SHOW_DETAILCOUNT_GREATER_THAN_ZERO_ONLY == TRUE ) {
			if (MS_USE_BADGE_FOR_DETAILCOUNT) {
				...
				$body .= "&nbsp; <span class='badge'>".$this->a_sales_Count."</span>"; 
			} else {
				$body .= "&nbsp;" . str_replace("%c", $this->a_sales_Count, $Language->Phrase("DetailCount"));
			}
	...
	function LoadRowValues(&$rs) {
		...
		$sDetailFilter = str_replace("@Customer_ID@", ew_AdjustSql($this->Customer_Number->DbValue), $sDetailFilter);
...
// Begin of modification Permission Access for Export To Feature, by Masino Sinaga, May 5, 2012
	...
	function ExportData() {
		...
		// Export detail records (a_sales)
		if (EW_EXPORT_DETAIL_RECORDS && in_array("a_sales", explode(",", $this->getCurrentDetailTable()))) {
			global $a_sales;
			if (!isset($a_sales)) $a_sales = new ca_sales;
			$rsdetail = $a_sales->LoadRs($a_sales->GetDetailFilter()); // Load detail records
			if ($rsdetail && !$rsdetail->EOF) {
				...
				if ($this->Export <> "csv" || EW_EXPORT_DETAIL_RECORDS_FOR_CSV) {
					...
					$a_sales->ExportDocument($Doc, $rsdetail, 1, $detailcnt);
				}
	...
	function SetUpDetailParms() {
		...
		if ($sDetailTblVar <> "") {
			...
			if (in_array("a_sales", $DetailTblVar)) {
				...
				if ($GLOBALS["a_sales_grid"]->DetailView) {
					$GLOBALS["a_sales_grid"]->CurrentMode = "view";
...
// Create page object
if (!isset($a_customers_view)) $a_customers_view = new ca_customers_view();

// Page init
$a_customers_view->Page_Init();

// Page main
$a_customers_view->Page_Main();
...
// Page object
var a_customers_view = new ew_Page("a_customers_view");
a_customers_view.PageID = "view"; // Page ID
var EW_PAGE_ID = a_customers_view.PageID; // For backward compatibility

// Form object
var fa_customersview = new ew_Form("fa_customersview");

// Form_CustomValidate event
fa_customersview.Form_CustomValidate = 
...
fa_customersview.ValidateRequired = true;
<?php } else { ?>
fa_customersview.ValidateRequired = false; 
<?php } ?>
...
<?php } ?>
<?php } ?>
<?php $a_customers_view->ExportOptions->Render("body") ?>
<?php
	foreach ($a_customers_view->OtherOptions as &$option)
		$option->Render("body");
?>
<?php if ($a_customers->Export == "") { ?>
...
<?php } ?>
<?php $a_customers_view->ShowPageHeader(); ?>
<?php
$a_customers_view->ShowMessage();
?>
	...
	<?php if (MS_PAGINATION_STYLE==1) { // link ?>
		...
		<?php if ($a_customers_view->Pager->RecordCount > 0) { ?>
				...
				<div class="ewPager ewRec">
					<span><?php echo $Language->Phrase("Record") ?>&nbsp;<?php echo $a_customers_view->Pager->FromIndex ?>&nbsp;<?php echo $Language->Phrase("To") ?>&nbsp;<?php echo $a_customers_view->Pager->ToIndex ?>&nbsp;<?php echo $Language->Phrase("Of") ?>&nbsp;<?php echo $a_customers_view->Pager->RecordCount ?></span>
				</div>
				...
				<div class="ewNumericPage"><ul class="pagination">
					...
					<li><a href="<?php echo $a_customers_view->PageUrl() ?>start=<?php echo $a_customers_view->Pager->FirstButton->Start ?>"><?php echo $Language->Phrase("PagerLast") ?></a></li>
					<?php } else { // else of rtl ?>
					<li><a href="<?php echo $a_customers_view->PageUrl() ?>start=<?php echo $a_customers_view->Pager->FirstButton->Start ?>"><?php echo $Language->Phrase("PagerFirst") ?></a></li>
					<?php } // end of rtl ?>
					<?php } ?>
					<?php if ($a_customers_view->Pager->PrevButton->Enabled) { ?>
					<?php if ($Language->Phrase("dir") == "rtl") { // begin of rtl ?>
					<li><a href="<?php echo $a_customers_view->PageUrl() ?>start=<?php echo $a_customers_view->Pager->PrevButton->Start ?>"><?php echo $Language->Phrase("PagerNext") ?></a></li>
					<?php } else { // else of rtl { ?>
					<li><a href="<?php echo $a_customers_view->PageUrl() ?>start=<?php echo $a_customers_view->Pager->PrevButton->Start ?>"><?php echo $Language->Phrase("PagerPrevious") ?></a></li>
					<?php } // end of rtl { ?>
					<?php } ?>
					<?php foreach ($a_customers_view->Pager->Items as $PagerItem) { ?>
						<li<?php if (!$PagerItem->Enabled) { echo " class=\" active\""; } ?>><a href="<?php if ($PagerItem->Enabled) { echo $a_customers_view->PageUrl() . "start=" . $PagerItem->Start; } else { echo "#"; } ?>"><?php echo $PagerItem->Text ?></a></li>
					<?php } ?>
					<?php if ($a_customers_view->Pager->NextButton->Enabled) { ?>
					<?php if ($Language->Phrase("dir") == "rtl") { // begin of rtl ?>
					<li><a href="<?php echo $a_customers_view->PageUrl() ?>start=<?php echo $a_customers_view->Pager->NextButton->Start ?>"><?php echo $Language->Phrase("PagerPrevious") ?></a></li>
					<?php } else { // else of rtl ?>
					<li><a href="<?php echo $a_customers_view->PageUrl() ?>start=<?php echo $a_customers_view->Pager->NextButton->Start ?>"><?php echo $Language->Phrase("PagerNext") ?></a></li>
					<?php } // end of rtl ?>
					<?php } ?>
					<?php if ($a_customers_view->Pager->LastButton->Enabled) { ?>
					<?php if ($Language->Phrase("dir") == "rtl") { // begin of rtl ?>
					<li><a href="<?php echo $a_customers_view->PageUrl() ?>start=<?php echo $a_customers_view->Pager->LastButton->Start ?>"><?php echo $Language->Phrase("PagerFirst") ?></a></li>
					<?php } else { // else of rtl ?>
					<li><a href="<?php echo $a_customers_view->PageUrl() ?>start=<?php echo $a_customers_view->Pager->LastButton->Start ?>"><?php echo $Language->Phrase("PagerLast") ?></a></li>
					<?php } // end of rtl ?>
					<?php } ?>
				</ul></div>
				</div>
	...
	<?php } elseif (MS_PAGINATION_STYLE==2) { // button ?>
		...
		<?php if ($a_customers_view->Pager->RecordCount > 0) { ?>
				...
				<!--first page button-->
					<?php if ($a_customers_view->Pager->FirstButton->Enabled) { ?>
					<?php if ($Language->Phrase("dir") == "rtl") { // begin of rtl ?>
					<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerFirst") ?>" href="<?php echo $a_customers_view->PageUrl() ?>start=<?php echo $a_customers_view->Pager->FirstButton->Start ?>"><span class="icon-last ewIcon"></span></a>
					<?php } else { // else of rtl ?>
					<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerFirst") ?>" href="<?php echo $a_customers_view->PageUrl() ?>start=<?php echo $a_customers_view->Pager->FirstButton->Start ?>"><span class="icon-first ewIcon"></span></a>
					<?php } // end of rtl ?>
					<?php } else { ?>
					<?php if ($Language->Phrase("dir") == "rtl") { // begin of rtl ?>
				...
				<!--previous page button-->
					<?php if ($a_customers_view->Pager->PrevButton->Enabled) { ?>
					<?php if ($Language->Phrase("dir") == "rtl") { // begin of rtl ?>
					<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerPrevious") ?>" href="<?php echo $a_customers_view->PageUrl() ?>start=<?php echo $a_customers_view->Pager->PrevButton->Start ?>"><span class="icon-next ewIcon"></span></a>
					<?php } else { // else of rtl ?>
					<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerPrevious") ?>" href="<?php echo $a_customers_view->PageUrl() ?>start=<?php echo $a_customers_view->Pager->PrevButton->Start ?>"><span class="icon-prev ewIcon"></span></a>
					<?php } // end of rtl ?>
					<?php } else { ?>
					<?php if ($Language->Phrase("dir") == "rtl") { // begin of rtl ?>
				...
				<!--current page number-->
					<input class="form-control input-sm" type="text" name="<?php echo EW_TABLE_PAGE_NO ?>" value="<?php echo $a_customers_view->Pager->CurrentPage ?>">
				<div class="input-group-btn">
				<!--next page button-->
					<?php if ($a_customers_view->Pager->NextButton->Enabled) { ?>
					<?php if ($Language->Phrase("dir") == "rtl") { // begin of rtl ?>
					<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerNext") ?>" href="<?php echo $a_customers_view->PageUrl() ?>start=<?php echo $a_customers_view->Pager->NextButton->Start ?>"><span class="icon-prev ewIcon"></span></a>
					<?php } else { // else of rtl ?>
					<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerNext") ?>" href="<?php echo $a_customers_view->PageUrl() ?>start=<?php echo $a_customers_view->Pager->NextButton->Start ?>"><span class="icon-next ewIcon"></span></a>
					<?php } // end of rtl ?>
					<?php } else { ?>
					<?php if ($Language->Phrase("dir") == "rtl") { // begin of rtl ?>
				...
				<!--last page button-->
					<?php if ($a_customers_view->Pager->LastButton->Enabled) { ?>
					<?php if ($Language->Phrase("dir") == "rtl") { // begin of rtl ?>
					<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerLast") ?>" href="<?php echo $a_customers_view->PageUrl() ?>start=<?php echo $a_customers_view->Pager->LastButton->Start ?>"><span class="icon-first ewIcon"></span></a>
					<?php } else { // else of rtl ?>
					<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerLast") ?>" href="<?php echo $a_customers_view->PageUrl() ?>start=<?php echo $a_customers_view->Pager->LastButton->Start ?>"><span class="icon-last ewIcon"></span></a>
					<?php } // end of rtl ?>
					<?php } else { ?>
					<?php if ($Language->Phrase("dir") == "rtl") { // begin of rtl ?>
				...
				<span>&nbsp;<?php echo $Language->Phrase("of") ?>&nbsp;<?php echo $a_customers_view->Pager->PageCount ?></span>
				</div>
...
<form name="fa_customersview" id="fa_customersview" class="form-inline ewForm ewViewForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($a_customers_view->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $a_customers_view->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="a_customers">
<table class="table table-bordered table-striped ewViewTable">
<?php if ($a_customers->Customer_ID->Visible) { // Customer_ID ?>
	<tr id="r_Customer_ID">
		<td><span id="elh_a_customers_Customer_ID"><?php echo $a_customers->Customer_ID->FldCaption() ?></span></td>
		<td<?php echo $a_customers->Customer_ID->CellAttributes() ?>>
<span id="el_a_customers_Customer_ID" class="form-group">
<span<?php echo $a_customers->Customer_ID->ViewAttributes() ?>>
<?php echo $a_customers->Customer_ID->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($a_customers->Customer_Number->Visible) { // Customer_Number ?>
	<tr id="r_Customer_Number">
		<td><span id="elh_a_customers_Customer_Number"><?php echo $a_customers->Customer_Number->FldCaption() ?></span></td>
		<td<?php echo $a_customers->Customer_Number->CellAttributes() ?>>
<span id="el_a_customers_Customer_Number" class="form-group">
<span<?php echo $a_customers->Customer_Number->ViewAttributes() ?>>
<?php echo $a_customers->Customer_Number->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($a_customers->Customer_Name->Visible) { // Customer_Name ?>
	<tr id="r_Customer_Name">
		<td><span id="elh_a_customers_Customer_Name"><?php echo $a_customers->Customer_Name->FldCaption() ?></span></td>
		<td<?php echo $a_customers->Customer_Name->CellAttributes() ?>>
<span id="el_a_customers_Customer_Name" class="form-group">
<span<?php echo $a_customers->Customer_Name->ViewAttributes() ?>>
<?php echo $a_customers->Customer_Name->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($a_customers->Address->Visible) { // Address ?>
	<tr id="r_Address">
		<td><span id="elh_a_customers_Address"><?php echo $a_customers->Address->FldCaption() ?></span></td>
		<td<?php echo $a_customers->Address->CellAttributes() ?>>
<span id="el_a_customers_Address" class="form-group">
<span<?php echo $a_customers->Address->ViewAttributes() ?>>
<?php echo $a_customers->Address->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($a_customers->City->Visible) { // City ?>
	<tr id="r_City">
		<td><span id="elh_a_customers_City"><?php echo $a_customers->City->FldCaption() ?></span></td>
		<td<?php echo $a_customers->City->CellAttributes() ?>>
<span id="el_a_customers_City" class="form-group">
<span<?php echo $a_customers->City->ViewAttributes() ?>>
<?php echo $a_customers->City->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($a_customers->Country->Visible) { // Country ?>
	<tr id="r_Country">
		<td><span id="elh_a_customers_Country"><?php echo $a_customers->Country->FldCaption() ?></span></td>
		<td<?php echo $a_customers->Country->CellAttributes() ?>>
<span id="el_a_customers_Country" class="form-group">
<span<?php echo $a_customers->Country->ViewAttributes() ?>>
<?php echo $a_customers->Country->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($a_customers->Contact_Person->Visible) { // Contact_Person ?>
	<tr id="r_Contact_Person">
		<td><span id="elh_a_customers_Contact_Person"><?php echo $a_customers->Contact_Person->FldCaption() ?></span></td>
		<td<?php echo $a_customers->Contact_Person->CellAttributes() ?>>
<span id="el_a_customers_Contact_Person" class="form-group">
<span<?php echo $a_customers->Contact_Person->ViewAttributes() ?>>
<?php echo $a_customers->Contact_Person->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($a_customers->Phone_Number->Visible) { // Phone_Number ?>
	<tr id="r_Phone_Number">
		<td><span id="elh_a_customers_Phone_Number"><?php echo $a_customers->Phone_Number->FldCaption() ?></span></td>
		<td<?php echo $a_customers->Phone_Number->CellAttributes() ?>>
<span id="el_a_customers_Phone_Number" class="form-group">
<span<?php echo $a_customers->Phone_Number->ViewAttributes() ?>>
<?php echo $a_customers->Phone_Number->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($a_customers->_Email->Visible) { // Email ?>
	<tr id="r__Email">
		<td><span id="elh_a_customers__Email"><?php echo $a_customers->_Email->FldCaption() ?></span></td>
		<td<?php echo $a_customers->_Email->CellAttributes() ?>>
<span id="el_a_customers__Email" class="form-group">
<span<?php echo $a_customers->_Email->ViewAttributes() ?>>
<?php echo $a_customers->_Email->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($a_customers->Mobile_Number->Visible) { // Mobile_Number ?>
	<tr id="r_Mobile_Number">
		<td><span id="elh_a_customers_Mobile_Number"><?php echo $a_customers->Mobile_Number->FldCaption() ?></span></td>
		<td<?php echo $a_customers->Mobile_Number->CellAttributes() ?>>
<span id="el_a_customers_Mobile_Number" class="form-group">
<span<?php echo $a_customers->Mobile_Number->ViewAttributes() ?>>
<?php echo $a_customers->Mobile_Number->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($a_customers->Notes->Visible) { // Notes ?>
	<tr id="r_Notes">
		<td><span id="elh_a_customers_Notes"><?php echo $a_customers->Notes->FldCaption() ?></span></td>
		<td<?php echo $a_customers->Notes->CellAttributes() ?>>
<span id="el_a_customers_Notes" class="form-group">
<span<?php echo $a_customers->Notes->ViewAttributes() ?>>
<?php echo $a_customers->Notes->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($a_customers->Balance->Visible) { // Balance ?>
	<tr id="r_Balance">
		<td><span id="elh_a_customers_Balance"><?php echo $a_customers->Balance->FldCaption() ?></span></td>
		<td<?php echo $a_customers->Balance->CellAttributes() ?>>
<span id="el_a_customers_Balance" class="form-group">
<span<?php echo $a_customers->Balance->ViewAttributes() ?>>
<?php echo $a_customers->Balance->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($a_customers->Date_Added->Visible) { // Date_Added ?>
	<tr id="r_Date_Added">
		<td><span id="elh_a_customers_Date_Added"><?php echo $a_customers->Date_Added->FldCaption() ?></span></td>
		<td<?php echo $a_customers->Date_Added->CellAttributes() ?>>
<span id="el_a_customers_Date_Added" class="form-group">
<span<?php echo $a_customers->Date_Added->ViewAttributes() ?>>
<?php echo $a_customers->Date_Added->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($a_customers->Added_By->Visible) { // Added_By ?>
	<tr id="r_Added_By">
		<td><span id="elh_a_customers_Added_By"><?php echo $a_customers->Added_By->FldCaption() ?></span></td>
		<td<?php echo $a_customers->Added_By->CellAttributes() ?>>
<span id="el_a_customers_Added_By" class="form-group">
<span<?php echo $a_customers->Added_By->ViewAttributes() ?>>
<?php echo $a_customers->Added_By->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($a_customers->Date_Updated->Visible) { // Date_Updated ?>
	<tr id="r_Date_Updated">
		<td><span id="elh_a_customers_Date_Updated"><?php echo $a_customers->Date_Updated->FldCaption() ?></span></td>
		<td<?php echo $a_customers->Date_Updated->CellAttributes() ?>>
<span id="el_a_customers_Date_Updated" class="form-group">
<span<?php echo $a_customers->Date_Updated->ViewAttributes() ?>>
<?php echo $a_customers->Date_Updated->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($a_customers->Updated_By->Visible) { // Updated_By ?>
	<tr id="r_Updated_By">
		<td><span id="elh_a_customers_Updated_By"><?php echo $a_customers->Updated_By->FldCaption() ?></span></td>
		<td<?php echo $a_customers->Updated_By->CellAttributes() ?>>
<span id="el_a_customers_Updated_By" class="form-group">
<span<?php echo $a_customers->Updated_By->ViewAttributes() ?>>
<?php echo $a_customers->Updated_By->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
...
<?php // End of modification Customize Navigation/Pager Panel, by Masino Sinaga, May 2, 2012 ?>
<?php
	if (in_array("a_sales", explode(",", $a_customers->getCurrentDetailTable())) && $a_sales->DetailView) {
?>
...
<script type="text/javascript">
fa_customersview.Init();
</script>
<?php
$a_customers_view->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<?php if ($a_customers->Export == "") { ?>
...
<?php
$a_customers_view->Page_Terminate();
?>
