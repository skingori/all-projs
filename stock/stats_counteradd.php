		...
		switch ($this->CurrentAction) {
			...
			case "A": // Add new record
				...
				if ($this->AddRow($this->OldRecordset)) { // Add successful
					...
					if (ew_GetPageName($sReturnUrl) == "stats_counterview.php")
...
// Page object
var stats_counter_add = new ew_Page("stats_counter_add");
stats_counter_add.PageID = "add"; // Page ID
var EW_PAGE_ID = stats_counter_add.PageID; // For backward compatibility

// Form object
var fstats_counteradd = new ew_Form("fstats_counteradd");
...
?>
<form name="fstats_counteradd" id="fstats_counteradd" class="form-horizontal ewForm ewAddForm" action="<?php echo ew_CurrentPage() ?>" method="post">
...
<input type="text" data-field="x_Type" name="x_Type" id="x_Type" size="30" maxlength="50" placeholder="<?php echo ew_HtmlEncode($stats_counter->Type->PlaceHolder) ?>" value="<?php echo $stats_counter->Type->EditValue ?>"<?php echo $stats_counter->Type->EditAttributes() ?>>
</span>
...
<input type="text" data-field="x_Variable" name="x_Variable" id="x_Variable" size="30" maxlength="50" placeholder="<?php echo ew_HtmlEncode($stats_counter->Variable->PlaceHolder) ?>" value="<?php echo $stats_counter->Variable->EditValue ?>"<?php echo $stats_counter->Variable->EditAttributes() ?>>
</span>
...
<script type="text/javascript">
$('#btnAction').attr('onclick', 'return alertifyAdd(this)'); function alertifyAdd(obj) { <?php global $Language; ?> if (fstats_counteradd.Validate() == true ) { alertify.set({buttonFocus: 'cancel'});alertify.confirm("<?php echo $Language->Phrase('AlertifyAddConfirm'); ?>", function (e) { if (e) { $(window).unbind('beforeunload'); alertify.success("<?php echo $Language->Phrase('AlertifyAdd'); ?>"); $("#fstats_counteradd").submit(); } else { alertify.error("<?php echo $Language->Phrase('AlertifyCancel'); ?>"); } }, "<?php echo $Language->Phrase('AlertifyConfirm'); ?>"); } return false; }
</script>
<?php } ?>
