...
$stats_counterlog_edit = NULL; // Initialize page object first

class cstats_counterlog_edit extends cstats_counterlog {
	...
	// Page object name
	var $PageObjName = 'stats_counterlog_edit';
...
// End of modification Disable/Enable Registration Page, by Masino Sinaga, May 14, 2012
	...
	function Page_Main() {
		...
		if (@$_GET["IP_Address"] <> "") {
			...
			$this->RecKey["IP_Address"] = $this->IP_Address->QueryStringValue;
		} else {
		...
		if ($this->TotalRecs <= 0) { // No record found
			...
			$this->Page_Terminate("stats_counterloglist.php"); // Return to list page
			...
			while (!$this->Recordset->EOF) {
				if (strval($this->IP_Address->CurrentValue) == strval($this->Recordset->fields('IP_Address'))) {
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			$this->Hostname->SetDbValueDef($rsnew, $this->Hostname->CurrentValue, NULL, $this->Hostname->ReadOnly);

			// First_Visit
			$this->First_Visit->SetDbValueDef($rsnew, ew_UnFormatDateTime($this->First_Visit->CurrentValue, 9), ew_CurrentDate(), $this->First_Visit->ReadOnly);

			// Last_Visit
			$this->Last_Visit->SetDbValueDef($rsnew, ew_UnFormatDateTime($this->Last_Visit->CurrentValue, 9), ew_CurrentDate(), $this->Last_Visit->ReadOnly);

			// Counter
			$this->Counter->SetDbValueDef($rsnew, $this->Counter->CurrentValue, 0, $this->Counter->ReadOnly);
	...
	function SetupBreadcrumb() {
		...
		$Breadcrumb->Add("list", $this->TableVar, "stats_counterloglist.php", "", $this->TableVar, TRUE);
		$PageId = "edit";
...
// Create page object
if (!isset($stats_counterlog_edit)) $stats_counterlog_edit = new cstats_counterlog_edit();

// Page init
$stats_counterlog_edit->Page_Init();

// Page main
$stats_counterlog_edit->Page_Main();
...
$stats_counterlog_edit->Page_Render();
?>
<?php include_once "header.php" ?>
<script type="text/javascript">

// Page object
var stats_counterlog_edit = new ew_Page("stats_counterlog_edit");
stats_counterlog_edit.PageID = "edit"; // Page ID
var EW_PAGE_ID = stats_counterlog_edit.PageID; // For backward compatibility

// Form object
var fstats_counterlogedit = new ew_Form("fstats_counterlogedit");

// Validate form
fstats_counterlogedit.Validate = function() {
...
fstats_counterlogedit.Form_CustomValidate = 
...
fstats_counterlogedit.ValidateRequired = true;
<?php } else { ?>
fstats_counterlogedit.ValidateRequired = false; 
<?php } ?>
...
<?php $stats_counterlog_edit->ShowPageHeader(); ?>
<?php
$stats_counterlog_edit->ShowMessage();
?>
...
<form name="ewPagerForm" class="form-horizontal ewForm ewPagerForm" action="<?php echo ew_CurrentPage() ?>">
	<?php if (MS_PAGINATION_STYLE==1) { // link ?>
		<?php if (!isset($stats_counterlog_edit->Pager)) $stats_counterlog_edit->Pager = new cNumericPager($stats_counterlog_edit->StartRec, $stats_counterlog_edit->DisplayRecs, $stats_counterlog_edit->TotalRecs, $stats_counterlog_edit->RecRange) ?>
		<?php if ($stats_counterlog_edit->Pager->RecordCount > 0) { ?>
				<?php if (($stats_counterlog_edit->Pager->PageCount==1) && ($stats_counterlog_edit->Pager->CurrentPage == 1) && (MS_SHOW_PAGENUM_IF_REC_NOT_OVER_PAGESIZE==FALSE)  ) { ?>
				<?php } else { // MS_SHOW_PAGENUM_IF_REC_NOT_OVER_PAGESIZE ?>
				<div class="ewPager">
				<div class="ewNumericPage"><ul class="pagination">
					<?php if ($stats_counterlog_edit->Pager->FirstButton->Enabled) { ?>
					<?php if ($Language->Phrase("dir") == "rtl") { // begin of rtl ?>
					<li><a href="<?php echo $stats_counterlog_edit->PageUrl() ?>start=<?php echo $stats_counterlog_edit->Pager->FirstButton->Start ?>"><?php echo $Language->Phrase("PagerLast") ?></a></li>
					<?php } else { // else of rtl ?>
					<li><a href="<?php echo $stats_counterlog_edit->PageUrl() ?>start=<?php echo $stats_counterlog_edit->Pager->FirstButton->Start ?>"><?php echo $Language->Phrase("PagerFirst") ?></a></li>
					<?php } // end of rtl ?>
					<?php } ?>
					<?php if ($stats_counterlog_edit->Pager->PrevButton->Enabled) { ?>
					<?php if ($Language->Phrase("dir") == "rtl") { // begin of rtl ?>
					<li><a href="<?php echo $stats_counterlog_edit->PageUrl() ?>start=<?php echo $stats_counterlog_edit->Pager->PrevButton->Start ?>"><?php echo $Language->Phrase("PagerNext") ?></a></li>
					<?php } else { // else of rtl { ?>
					<li><a href="<?php echo $stats_counterlog_edit->PageUrl() ?>start=<?php echo $stats_counterlog_edit->Pager->PrevButton->Start ?>"><?php echo $Language->Phrase("PagerPrevious") ?></a></li>
					<?php } // end of rtl { ?>
					<?php } ?>
					<?php foreach ($stats_counterlog_edit->Pager->Items as $PagerItem) { ?>
						<li<?php if (!$PagerItem->Enabled) { echo " class=\" active\""; } ?>><a href="<?php if ($PagerItem->Enabled) { echo $stats_counterlog_edit->PageUrl() . "start=" . $PagerItem->Start; } else { echo "#"; } ?>"><?php echo $PagerItem->Text ?></a></li>
					<?php } ?>
					<?php if ($stats_counterlog_edit->Pager->NextButton->Enabled) { ?>
					<?php if ($Language->Phrase("dir") == "rtl") { // begin of rtl ?>
					<li><a href="<?php echo $stats_counterlog_edit->PageUrl() ?>start=<?php echo $stats_counterlog_edit->Pager->NextButton->Start ?>"><?php echo $Language->Phrase("PagerPrevious") ?></a></li>
					<?php } else { // else of rtl ?>
					<li><a href="<?php echo $stats_counterlog_edit->PageUrl() ?>start=<?php echo $stats_counterlog_edit->Pager->NextButton->Start ?>"><?php echo $Language->Phrase("PagerNext") ?></a></li>
					<?php } // end of rtl ?>
					<?php } ?>
					<?php if ($stats_counterlog_edit->Pager->LastButton->Enabled) { ?>
					<?php if ($Language->Phrase("dir") == "rtl") { // begin of rtl ?>
					<li><a href="<?php echo $stats_counterlog_edit->PageUrl() ?>start=<?php echo $stats_counterlog_edit->Pager->LastButton->Start ?>"><?php echo $Language->Phrase("PagerFirst") ?></a></li>
					<?php } else { // else of rtl ?>
					<li><a href="<?php echo $stats_counterlog_edit->PageUrl() ?>start=<?php echo $stats_counterlog_edit->Pager->LastButton->Start ?>"><?php echo $Language->Phrase("PagerLast") ?></a></li>
					<?php } // end of rtl ?>
					<?php } ?>
				</ul></div>
				</div>
	...
	<?php } elseif (MS_PAGINATION_STYLE==2) { // button ?>
		<?php if (!isset($stats_counterlog_edit->Pager)) $stats_counterlog_edit->Pager = new cPrevNextPager($stats_counterlog_edit->StartRec, $stats_counterlog_edit->DisplayRecs, $stats_counterlog_edit->TotalRecs) ?>
		<?php if ($stats_counterlog_edit->Pager->RecordCount > 0) { ?>
				<?php if (($stats_counterlog_edit->Pager->PageCount==1) && ($stats_counterlog_edit->Pager->CurrentPage == 1) && (MS_SHOW_PAGENUM_IF_REC_NOT_OVER_PAGESIZE==FALSE)  ) { ?>
				...
				<!--first page button-->
					<?php if ($stats_counterlog_edit->Pager->FirstButton->Enabled) { ?>
					<?php if ($Language->Phrase("dir") == "rtl") { // begin of rtl ?>
					<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerFirst") ?>" href="<?php echo $stats_counterlog_edit->PageUrl() ?>start=<?php echo $stats_counterlog_edit->Pager->FirstButton->Start ?>"><span class="icon-last ewIcon"></span></a>
					<?php } else { // else of rtl ?>
					<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerFirst") ?>" href="<?php echo $stats_counterlog_edit->PageUrl() ?>start=<?php echo $stats_counterlog_edit->Pager->FirstButton->Start ?>"><span class="icon-first ewIcon"></span></a>
					<?php } // end of rtl ?>
					<?php } else { ?>
					<?php if ($Language->Phrase("dir") == "rtl") { // begin of rtl ?>
				...
				<!--previous page button-->
					<?php if ($stats_counterlog_edit->Pager->PrevButton->Enabled) { ?>
					<?php if ($Language->Phrase("dir") == "rtl") { // begin of rtl ?>
					<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerPrevious") ?>" href="<?php echo $stats_counterlog_edit->PageUrl() ?>start=<?php echo $stats_counterlog_edit->Pager->PrevButton->Start ?>"><span class="icon-next ewIcon"></span></a>
					<?php } else { // else of rtl ?>
					<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerPrevious") ?>" href="<?php echo $stats_counterlog_edit->PageUrl() ?>start=<?php echo $stats_counterlog_edit->Pager->PrevButton->Start ?>"><span class="icon-prev ewIcon"></span></a>
					<?php } // end of rtl ?>
					<?php } else { ?>
					<?php if ($Language->Phrase("dir") == "rtl") { // begin of rtl ?>
				...
				<!--current page number-->
					<input class="form-control input-sm" type="text" name="<?php echo EW_TABLE_PAGE_NO ?>" value="<?php echo $stats_counterlog_edit->Pager->CurrentPage ?>">
				<div class="input-group-btn">
				<!--next page button-->
					<?php if ($stats_counterlog_edit->Pager->NextButton->Enabled) { ?>
					<?php if ($Language->Phrase("dir") == "rtl") { // begin of rtl ?>
					<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerNext") ?>" href="<?php echo $stats_counterlog_edit->PageUrl() ?>start=<?php echo $stats_counterlog_edit->Pager->NextButton->Start ?>"><span class="icon-prev ewIcon"></span></a>
					<?php } else { // else of rtl ?>
					<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerNext") ?>" href="<?php echo $stats_counterlog_edit->PageUrl() ?>start=<?php echo $stats_counterlog_edit->Pager->NextButton->Start ?>"><span class="icon-next ewIcon"></span></a>
					<?php } // end of rtl ?>
					<?php } else { ?>
					<?php if ($Language->Phrase("dir") == "rtl") { // begin of rtl ?>
				...
				<!--last page button-->
					<?php if ($stats_counterlog_edit->Pager->LastButton->Enabled) { ?>
					<?php if ($Language->Phrase("dir") == "rtl") { // begin of rtl ?>
					<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerLast") ?>" href="<?php echo $stats_counterlog_edit->PageUrl() ?>start=<?php echo $stats_counterlog_edit->Pager->LastButton->Start ?>"><span class="icon-first ewIcon"></span></a>
					<?php } else { // else of rtl ?>
					<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerLast") ?>" href="<?php echo $stats_counterlog_edit->PageUrl() ?>start=<?php echo $stats_counterlog_edit->Pager->LastButton->Start ?>"><span class="icon-last ewIcon"></span></a>
					<?php } // end of rtl ?>
					<?php } else { ?>
					<?php if ($Language->Phrase("dir") == "rtl") { // begin of rtl ?>
				...
				<span>&nbsp;<?php echo $Language->Phrase("of") ?>&nbsp;<?php echo $stats_counterlog_edit->Pager->PageCount ?></span>
				</div>
...
<form name="fstats_counterlogedit" id="fstats_counterlogedit" class="form-horizontal ewForm ewEditForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($stats_counterlog_edit->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $stats_counterlog_edit->Token ?>">
<?php } ?>
...
<p class="form-control-static"><?php echo $stats_counterlog->IP_Address->EditValue ?></p></span>
</span>
<input type="hidden" data-field="x_IP_Address" name="x_IP_Address" id="x_IP_Address" value="<?php echo ew_HtmlEncode($stats_counterlog->IP_Address->CurrentValue) ?>">
...
<input type="text" data-field="x_Hostname" name="x_Hostname" id="x_Hostname" size="30" maxlength="50" placeholder="<?php echo ew_HtmlEncode($stats_counterlog->Hostname->PlaceHolder) ?>" value="<?php echo $stats_counterlog->Hostname->EditValue ?>"<?php echo $stats_counterlog->Hostname->EditAttributes() ?>>
</span>
...
<input type="text" data-field="x_First_Visit" name="x_First_Visit" id="x_First_Visit" placeholder="<?php echo ew_HtmlEncode($stats_counterlog->First_Visit->PlaceHolder) ?>" value="<?php echo $stats_counterlog->First_Visit->EditValue ?>"<?php echo $stats_counterlog->First_Visit->EditAttributes() ?>>
</span>
...
<input type="text" data-field="x_Last_Visit" name="x_Last_Visit" id="x_Last_Visit" placeholder="<?php echo ew_HtmlEncode($stats_counterlog->Last_Visit->PlaceHolder) ?>" value="<?php echo $stats_counterlog->Last_Visit->EditValue ?>"<?php echo $stats_counterlog->Last_Visit->EditAttributes() ?>>
</span>
...
<input type="text" data-field="x_Counter" name="x_Counter" id="x_Counter" size="30" placeholder="<?php echo ew_HtmlEncode($stats_counterlog->Counter->PlaceHolder) ?>" value="<?php echo $stats_counterlog->Counter->EditValue ?>"<?php echo $stats_counterlog->Counter->EditAttributes() ?>>
</span>
...
<script type="text/javascript">
fstats_counterlogedit.Init();
</script>
<?php
$stats_counterlog_edit->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
