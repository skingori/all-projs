...
//
class ctimezone extends cTable {
	var $Timezone;
	var $_Default;
...
// End of mofidication Flexibility of Export Records Options, by Masino Sinaga, May 14, 2012
		...
		// Timezone
		$this->Timezone = new cField('timezone', 'timezone', 'x_Timezone', 'Timezone', '`Timezone`', '`Timezone`', 200, -1, FALSE, '`Timezone`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['Timezone'] = &$this->Timezone;

		// Default
		$this->_Default = new cField('timezone', 'timezone', 'x__Default', 'Default', '`Default`', '`Default`', 202, -1, FALSE, '`Default`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`timezone`";
	}
	...
	function DeleteSQL(&$rs, $where = "") {
		...
		if ($rs) {
			if (array_key_exists('Timezone', $rs))
				ew_AddFilter($where, ew_QuotedName('Timezone') . '=' . ew_QuotedValue($rs['Timezone'], $this->Timezone->FldDataType));
		}
		$filter = $this->CurrentFilter;
		ew_AddFilter($filter, $where);
		if ($filter <> "")
	...
	function KeyFilter() {
		...
		$sKeyFilter = str_replace("@Timezone@", ew_AdjustSql($this->Timezone->CurrentValue), $sKeyFilter); // Replace key value
		return $sKeyFilter;
	}
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			return $this->KeyUrl("timezoneview.php", $this->UrlParm($parm));
		else
			return $this->KeyUrl("timezoneview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	}

	// Add URL
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			return "timezoneadd.php?" . $this->UrlParm($parm);
		else
			return "timezoneadd.php";
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		return $this->KeyUrl("timezoneedit.php", $this->UrlParm($parm));
	}
	...
	function GetCopyUrl($parm = "") {
		return $this->KeyUrl("timezoneadd.php", $this->UrlParm($parm));
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("timezonedelete.php", $this->UrlParm());
	}
	...
	function KeyUrl($url, $parm = "") {
		$sUrl = $url . "?";
		if ($parm <> "") $sUrl .= $parm . "&";
		if (!is_null($this->Timezone->CurrentValue)) {
			$sUrl .= "Timezone=" . urlencode($this->Timezone->CurrentValue);
		} else {
	...
	function GetKeyFilter() {
		...
		foreach ($arKeys as $key) {
			...
			$this->Timezone->CurrentValue = $key;
   ...
   // Common render codes
	...
	function Row_Inserting($rsold, &$rsnew) {
		...
		if ($rsnew["Default"] == 'Y') {
		   ew_Execute("UPDATE `".MS_TIMEZONE_TABLE."` SET `Default` = 'N'");
		   ew_Execute("UPDATE `".MS_SETTINGS_TABLE."` SET `Default_Timezone` = '".$rsnew["Timezone"]."' 
	...
	function Row_Updating($rsold, &$rsnew) {
		...
		if ($rsnew["Default"] == 'Y') {
		   ew_Execute("UPDATE `".MS_TIMEZONE_TABLE."` SET `Default` = 'N'");
		   ew_Execute("UPDATE `".MS_SETTINGS_TABLE."` SET `Default_Timezone` = '".$rsold["Timezone"]."' 
		   				WHERE `Option_Default` = 'Y'");
		}
		return TRUE;
	}
