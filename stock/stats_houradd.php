...
$stats_hour_add = NULL; // Initialize page object first

class cstats_hour_add extends cstats_hour {
	...
	// Page object name
	var $PageObjName = 'stats_hour_add';
...
// End of modification Disable/Enable Registration Page, by Masino Sinaga, May 14, 2012
	...
	function Page_Main() {
		...
		if (@$_POST["a_add"] <> "") {
			...
			if (@$_GET["Hour"] != "") {
				...
				$this->setKey("Hour", $this->Hour->CurrentValue); // Set up key
			} else {
				$this->setKey("Hour", ""); // Clear key
				$this->CopyRecord = FALSE;
			}
			if ($this->CopyRecord) {
		...
		switch ($this->CurrentAction) {
			...
			case "C": // Copy an existing record
				if (!$this->LoadRow()) { // Load record based on key
					...
					$this->Page_Terminate("stats_hourlist.php"); // No matching record, return to list
				}
				break;
			case "A": // Add new record
				...
				if ($this->AddRow($this->OldRecordset)) { // Add successful
					...
					if (ew_GetPageName($sReturnUrl) == "stats_hourview.php")
	...
	function RenderRow() {
		...
		if ($this->RowType == EW_ROWTYPE_VIEW) { // View row
			...
			$this->Hour->EditValue = ew_HtmlEncode($this->Hour->CurrentValue);
			$this->Hour->PlaceHolder = ew_RemoveHtml($this->Hour->FldCaption());
	...
	function AddRow($rsold = NULL) {
		...
		// Hour
		$this->Hour->SetDbValueDef($rsnew, $this->Hour->CurrentValue, 0, strval($this->Hour->CurrentValue) == "");
		...
		// Check if key value entered
		if ($bInsertRow && $this->ValidateKey && strval($rsnew['Hour']) == "") {
			$this->setFailureMessage($Language->Phrase("InvalidKeyValue"));
			$bInsertRow = FALSE;
		}
...
// Create page object
if (!isset($stats_hour_add)) $stats_hour_add = new cstats_hour_add();

// Page init
$stats_hour_add->Page_Init();

// Page main
$stats_hour_add->Page_Main();
...
$stats_hour_add->Page_Render();
?>
<?php include_once "header.php" ?>
<script type="text/javascript">

// Page object
var stats_hour_add = new ew_Page("stats_hour_add");
stats_hour_add.PageID = "add"; // Page ID
var EW_PAGE_ID = stats_hour_add.PageID; // For backward compatibility

// Form object
var fstats_houradd = new ew_Form("fstats_houradd");

// Validate form
fstats_houradd.Validate = function() {
...
fstats_houradd.Form_CustomValidate = 
...
fstats_houradd.ValidateRequired = true;
<?php } else { ?>
fstats_houradd.ValidateRequired = false; 
<?php } ?>
...
<?php $stats_hour_add->ShowPageHeader(); ?>
<?php
$stats_hour_add->ShowMessage();
?>
<form name="fstats_houradd" id="fstats_houradd" class="form-horizontal ewForm ewAddForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($stats_hour_add->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $stats_hour_add->Token ?>">
<?php } ?>
...
<input type="text" data-field="x_Year" name="x_Year" id="x_Year" size="30" placeholder="<?php echo ew_HtmlEncode($stats_hour->Year->PlaceHolder) ?>" value="<?php echo $stats_hour->Year->EditValue ?>"<?php echo $stats_hour->Year->EditAttributes() ?>>
</span>
...
<input type="text" data-field="x_Month" name="x_Month" id="x_Month" size="30" placeholder="<?php echo ew_HtmlEncode($stats_hour->Month->PlaceHolder) ?>" value="<?php echo $stats_hour->Month->EditValue ?>"<?php echo $stats_hour->Month->EditAttributes() ?>>
</span>
...
<input type="text" data-field="x_Date" name="x_Date" id="x_Date" size="30" placeholder="<?php echo ew_HtmlEncode($stats_hour->Date->PlaceHolder) ?>" value="<?php echo $stats_hour->Date->EditValue ?>"<?php echo $stats_hour->Date->EditAttributes() ?>>
</span>
...
<input type="text" data-field="x_Hour" name="x_Hour" id="x_Hour" size="30" placeholder="<?php echo ew_HtmlEncode($stats_hour->Hour->PlaceHolder) ?>" value="<?php echo $stats_hour->Hour->EditValue ?>"<?php echo $stats_hour->Hour->EditAttributes() ?>>
</span>
