...
//
class chelp_categories extends cTable {
	var $Category_ID;
	var $_Language;
...
// End of mofidication Flexibility of Export Records Options, by Masino Sinaga, May 14, 2012
		...
		// Category_ID
		$this->Category_ID = new cField('help_categories', 'help_categories', 'x_Category_ID', 'Category_ID', '`Category_ID`', '`Category_ID`', 3, -1, FALSE, '`Category_ID`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->Category_ID->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['Category_ID'] = &$this->Category_ID;

		// Language
		$this->_Language = new cField('help_categories', 'help_categories', 'x__Language', 'Language', '`Language`', '`Language`', 200, -1, FALSE, '`Language`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['Language'] = &$this->_Language;

		// Category_Description
		$this->Category_Description = new cField('help_categories', 'help_categories', 'x_Category_Description', 'Category_Description', '`Category_Description`', '`Category_Description`', 200, -1, FALSE, '`Category_Description`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
	...
	function GetDetailUrl() {
		...
		if ($this->getCurrentDetailTable() == "help") {
			$sDetailUrl = $GLOBALS["help"]->GetListUrl() . "?showmaster=" . $this->TableVar;
			$sDetailUrl .= "&fk_Category_ID=" . urlencode($this->Category_ID->CurrentValue);
		}
		if ($sDetailUrl == "") {
			$sDetailUrl = "help_categorieslist.php";
		}
		return $sDetailUrl;
	}
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`help_categories`";
	}
	...
	// Update Table
	var $UpdateTable = "`help_categories`";
	...
	function DeleteSQL(&$rs, $where = "") {
		...
		if ($rs) {
			if (array_key_exists('Category_ID', $rs))
				ew_AddFilter($where, ew_QuotedName('Category_ID') . '=' . ew_QuotedValue($rs['Category_ID'], $this->Category_ID->FldDataType));
		}
		$filter = $this->CurrentFilter;
		ew_AddFilter($filter, $where);
		if ($filter <> "")
	...
	function getReturnUrl() {
		...
		if (@$_SESSION[$name] <> "") {
			...
			return "help_categorieslist.php";
		}
	}
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			return $this->KeyUrl("help_categoriesview.php", $this->UrlParm($parm));
		else
			return $this->KeyUrl("help_categoriesview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	}

	// Add URL
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			return "help_categoriesadd.php?" . $this->UrlParm($parm);
		else
			return "help_categoriesadd.php";
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		if ($parm <> "")
			return $this->KeyUrl("help_categoriesedit.php", $this->UrlParm($parm));
		else
			return $this->KeyUrl("help_categoriesedit.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	}
	...
	function GetCopyUrl($parm = "") {
		if ($parm <> "")
			return $this->KeyUrl("help_categoriesadd.php", $this->UrlParm($parm));
		else
			return $this->KeyUrl("help_categoriesadd.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("help_categoriesdelete.php", $this->UrlParm());
	}
