		...
		switch ($this->CurrentAction) {
			...
			case "A": // Add new record
				...
				if ($this->AddRow($this->OldRecordset)) { // Add successful
					...
					if (ew_GetPageName($sReturnUrl) == "a_purchases_detailview.php")
...
// Page object
var a_purchases_detail_add = new ew_Page("a_purchases_detail_add");
a_purchases_detail_add.PageID = "add"; // Page ID
var EW_PAGE_ID = a_purchases_detail_add.PageID; // For backward compatibility

// Form object
var fa_purchases_detailadd = new ew_Form("fa_purchases_detailadd");
...
fa_purchases_detailadd.Lists["x_Supplier_Number"] = {"LinkField":"x_Supplier_Number","Ajax":null,"AutoFill":false,"DisplayFields":["x_Supplier_Name","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
fa_purchases_detailadd.Lists["x_Stock_Item"] = {"LinkField":"x_Stock_Number","Ajax":null,"AutoFill":true,"DisplayFields":["x_Stock_Name","","",""],"ParentFields":["x_Supplier_Number"],"FilterFields":["x_Supplier_Number"],"Options":[]};
...
?>
<form name="fa_purchases_detailadd" id="fa_purchases_detailadd" class="form-horizontal ewForm ewAddForm" action="<?php echo ew_CurrentPage() ?>" method="post">
...
<script type="text/javascript">
fa_purchases_detailadd.Lists["x_Supplier_Number"].Options = <?php echo (is_array($a_purchases_detail->Supplier_Number->EditValue)) ? ew_ArrayToJson($a_purchases_detail->Supplier_Number->EditValue, 1) : "[]" ?>;
</script>
</span>
...
<script type="text/javascript">
fa_purchases_detailadd.Lists["x_Stock_Item"].Options = <?php echo (is_array($a_purchases_detail->Stock_Item->EditValue)) ? ew_ArrayToJson($a_purchases_detail->Stock_Item->EditValue, 1) : "[]" ?>;
</script>
...
<script type="text/javascript">
$('#btnAction').attr('onclick', 'return alertifyAdd(this)'); function alertifyAdd(obj) { <?php global $Language; ?> if (fa_purchases_detailadd.Validate() == true ) { alertify.set({buttonFocus: 'cancel'});alertify.confirm("<?php echo $Language->Phrase('AlertifyAddConfirm'); ?>", function (e) { if (e) { $(window).unbind('beforeunload'); alertify.success("<?php echo $Language->Phrase('AlertifyAdd'); ?>"); $("#fa_purchases_detailadd").submit(); } else { alertify.error("<?php echo $Language->Phrase('AlertifyCancel'); ?>"); } }, "<?php echo $Language->Phrase('AlertifyConfirm'); ?>"); } return false; }
</script>
<?php } ?>
