...
$a_sales_grid = NULL; // Initialize page object first

class ca_sales_grid extends ca_sales {
	...
	// Page object name
	var $PageObjName = 'a_sales_grid';

	// Grid form hidden field names
	var $FormName = 'fa_salesgrid';
...
//			if (!isset($GLOBALS["Table"])) $GLOBALS["Table"] = &$GLOBALS["a_sales"];
...
//		$GLOBALS["Table"] = &$GLOBALS["MasterTable"];
	...
	function ClearInlineMode() {
		...
		$this->Discount_Amount->FormValue = ""; // Clear form value
		$this->Tax_Amount->FormValue = ""; // Clear form value
		$this->Final_Total_Amount->FormValue = ""; // Clear form value
	...
	function EmptyRow() {
		...
		if ($objForm->HasValue("x_Sales_Number") && $objForm->HasValue("o_Sales_Number") && $this->Sales_Number->CurrentValue <> $this->Sales_Number->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_Sales_Date") && $objForm->HasValue("o_Sales_Date") && $this->Sales_Date->CurrentValue <> $this->Sales_Date->OldValue)
			return FALSE;
		...
		if ($objForm->HasValue("x_Total_Amount") && $objForm->HasValue("o_Total_Amount") && $this->Total_Amount->CurrentValue <> $this->Total_Amount->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_Discount_Amount") && $objForm->HasValue("o_Discount_Amount") && $this->Discount_Amount->CurrentValue <> $this->Discount_Amount->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_Tax_Amount") && $objForm->HasValue("o_Tax_Amount") && $this->Tax_Amount->CurrentValue <> $this->Tax_Amount->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_Final_Total_Amount") && $objForm->HasValue("o_Final_Total_Amount") && $this->Final_Total_Amount->CurrentValue <> $this->Final_Total_Amount->OldValue)
			return FALSE;
	...
	function LoadDefaultValues() {
		...
		$this->Sales_Date->OldValue = $this->Sales_Date->CurrentValue;
		...
		$this->Discount_Amount->OldValue = $this->Discount_Amount->CurrentValue;
		...
		$this->Final_Total_Amount->OldValue = $this->Final_Total_Amount->CurrentValue;
	...
	function LoadFormValues() {
		...
		$this->Sales_Date->setOldValue($objForm->GetValue("o_Sales_Date"));
	...
	function RenderRow() {
		...
		if ($this->RowType == EW_ROWTYPE_VIEW) { // View row
			...
			$this->Discount_Amount->EditValue = ew_FormatNumber($this->Discount_Amount->EditValue, -2, -2, -2, -2);
			$this->Discount_Amount->OldValue = $this->Discount_Amount->EditValue;
			}
			...
			$this->Tax_Amount->EditValue = ew_FormatNumber($this->Tax_Amount->EditValue, -2, -2, -2, -2);
			$this->Tax_Amount->OldValue = $this->Tax_Amount->EditValue;
			}
			...
			if (strval($this->Final_Total_Amount->EditValue) <> "" && is_numeric($this->Final_Total_Amount->EditValue)) {
			$this->Final_Total_Amount->EditValue = ew_FormatNumber($this->Final_Total_Amount->EditValue, -2, -2, -2, -2);
			$this->Final_Total_Amount->OldValue = $this->Final_Total_Amount->EditValue;
			}
	...
	function SetUpMasterParms() {
		...
		if ($sMasterTblVar == "a_customers") {
			...
			if ($GLOBALS["a_customers"]->EventCancelled) $this->EventCancelled = TRUE;
		}
