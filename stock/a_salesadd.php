...
// End of modification Disable/Enable Registration Page, by Masino Sinaga, May 14, 2012
		...
		switch ($this->CurrentAction) {
			...
			case "A": // Add new record
				...
				if ($this->AddRow($this->OldRecordset)) { // Add successful
					...
					if (ew_GetPageName($sReturnUrl) == "a_salesview.php")
	...
	function AddRow($rsold = NULL) {
		...
		if ($this->Sales_Number->CurrentValue <> "") { // Check field with unique index
			$sFilter = "(Sales_Number = '" . ew_AdjustSql($this->Sales_Number->CurrentValue) . "')";
...
// Page object
var a_sales_add = new ew_Page("a_sales_add");
a_sales_add.PageID = "add"; // Page ID
var EW_PAGE_ID = a_sales_add.PageID; // For backward compatibility

// Form object
var fa_salesadd = new ew_Form("fa_salesadd");
...
fa_salesadd.Lists["x_Customer_ID"] = {"LinkField":"x_Customer_Number","Ajax":null,"AutoFill":false,"DisplayFields":["x_Customer_Name","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
...
?>
<form name="fa_salesadd" id="fa_salesadd" class="form-horizontal ewForm ewAddForm" action="<?php echo ew_CurrentPage() ?>" method="post">
...
<script type="text/javascript">
fa_salesadd.Lists["x_Customer_ID"].Options = <?php echo (is_array($a_sales->Customer_ID->EditValue)) ? ew_ArrayToJson($a_sales->Customer_ID->EditValue, 1) : "[]" ?>;
</script>
</span>
<?php } ?>
...
<script type="text/javascript">
$('#btnAction').attr('onclick', 'return alertifyAdd(this)'); function alertifyAdd(obj) { <?php global $Language; ?> if (fa_salesadd.Validate() == true ) { alertify.set({buttonFocus: 'cancel'});alertify.confirm("<?php echo $Language->Phrase('AlertifyAddConfirm'); ?>", function (e) { if (e) { $(window).unbind('beforeunload'); alertify.success("<?php echo $Language->Phrase('AlertifyAdd'); ?>"); $("#fa_salesadd").submit(); } else { alertify.error("<?php echo $Language->Phrase('AlertifyCancel'); ?>"); } }, "<?php echo $Language->Phrase('AlertifyConfirm'); ?>"); } return false; }
</script>
<?php } ?>
