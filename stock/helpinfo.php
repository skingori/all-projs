...
// End of mofidication Flexibility of Export Records Options, by Masino Sinaga, May 14, 2012
		...
		// Help_ID
		$this->Help_ID = new cField('help', 'help', 'x_Help_ID', 'Help_ID', '`Help_ID`', '`Help_ID`', 3, -1, FALSE, '`Help_ID`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->Help_ID->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['Help_ID'] = &$this->Help_ID;

		// Language
		$this->_Language = new cField('help', 'help', 'x__Language', 'Language', '`Language`', '`Language`', 200, -1, FALSE, '`Language`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['Language'] = &$this->_Language;

		// Topic
		$this->Topic = new cField('help', 'help', 'x_Topic', 'Topic', '`Topic`', '`Topic`', 200, -1, FALSE, '`Topic`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['Topic'] = &$this->Topic;

		// Description
		$this->Description = new cField('help', 'help', 'x_Description', 'Description', '`Description`', '`Description`', 201, -1, FALSE, '`Description`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['Description'] = &$this->Description;

		// Category
		$this->Category = new cField('help', 'help', 'x_Category', 'Category', '`Category`', '`Category`', 3, -1, FALSE, '`Category`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->Category->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['Category'] = &$this->Category;

		// Order
		$this->Order = new cField('help', 'help', 'x_Order', 'Order', '`Order`', '`Order`', 3, -1, FALSE, '`Order`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->Order->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['Order'] = &$this->Order;

		// Display_in_Page
		$this->Display_in_Page = new cField('help', 'help', 'x_Display_in_Page', 'Display_in_Page', '`Display_in_Page`', '`Display_in_Page`', 200, -1, FALSE, '`Display_in_Page`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['Display_in_Page'] = &$this->Display_in_Page;

		// Updated_By
		$this->Updated_By = new cField('help', 'help', 'x_Updated_By', 'Updated_By', '`Updated_By`', '`Updated_By`', 200, -1, FALSE, '`Updated_By`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['Updated_By'] = &$this->Updated_By;

		// Last_Updated
		$this->Last_Updated = new cField('help', 'help', 'x_Last_Updated', 'Last_Updated', '`Last_Updated`', 'DATE_FORMAT(`Last_Updated`, \'%Y/%m/%d %H:%i:%s\')', 135, 9, FALSE, '`Last_Updated`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
	...
	function GetMasterFilter() {
		...
		if ($this->getCurrentMasterTable() == "help_categories") {
			if ($this->Category->getSessionValue() <> "")
				$sMasterFilter .= "`Category_ID`=" . ew_QuotedValue($this->Category->getSessionValue(), EW_DATATYPE_NUMBER);
			else
				return "";
		}
	...
	function GetDetailFilter() {
		...
		if ($this->getCurrentMasterTable() == "help_categories") {
			if ($this->Category->getSessionValue() <> "")
				$sDetailFilter .= "`Category`=" . ew_QuotedValue($this->Category->getSessionValue(), EW_DATATYPE_NUMBER);
			else
				return "";
		}
	...
	function DeleteSQL(&$rs, $where = "") {
		...
		if ($rs) {
			if (array_key_exists('Help_ID', $rs))
				ew_AddFilter($where, ew_QuotedName('Help_ID') . '=' . ew_QuotedValue($rs['Help_ID'], $this->Help_ID->FldDataType));
		}
		$filter = $this->CurrentFilter;
		ew_AddFilter($filter, $where);
		if ($filter <> "")
	...
	function KeyFilter() {
		...
		$sKeyFilter = str_replace("@Help_ID@", ew_AdjustSql($this->Help_ID->CurrentValue), $sKeyFilter); // Replace key value
		return $sKeyFilter;
	}
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			return $this->KeyUrl("helpview.php", $this->UrlParm($parm));
		else
			return $this->KeyUrl("helpview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	}

	// Add URL
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			return "helpadd.php?" . $this->UrlParm($parm);
		else
			return "helpadd.php";
	}
	...
	function GetEditUrl($parm = "") {
		return $this->KeyUrl("helpedit.php", $this->UrlParm($parm));
	}
	...
	function GetCopyUrl($parm = "") {
		return $this->KeyUrl("helpadd.php", $this->UrlParm($parm));
	}
