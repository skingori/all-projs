...
//
// Table class for stats_date
//
class cstats_date extends cTable {
	var $Year;
	var $Month;
	var $Date;
	var $Hits;
...
// End of mofidication Flexibility of Export Records Options, by Masino Sinaga, May 14, 2012
		...
		// Year
		$this->Year = new cField('stats_date', 'stats_date', 'x_Year', 'Year', '`Year`', '`Year`', 2, -1, FALSE, '`Year`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->Year->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['Year'] = &$this->Year;

		// Month
		$this->Month = new cField('stats_date', 'stats_date', 'x_Month', 'Month', '`Month`', '`Month`', 16, -1, FALSE, '`Month`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->Month->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['Month'] = &$this->Month;

		// Date
		$this->Date = new cField('stats_date', 'stats_date', 'x_Date', 'Date', '`Date`', '`Date`', 16, -1, FALSE, '`Date`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->Date->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['Date'] = &$this->Date;

		// Hits
		$this->Hits = new cField('stats_date', 'stats_date', 'x_Hits', 'Hits', '`Hits`', '`Hits`', 20, -1, FALSE, '`Hits`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`stats_date`";
	}
	...
	function SqlKeyFilter() {
		return "`Year` = @Year@ AND `Month` = @Month@ AND `Date` = @Date@";
	}
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			return $this->KeyUrl("stats_dateview.php", $this->UrlParm($parm));
		else
			return $this->KeyUrl("stats_dateview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	}

	// Add URL
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			return "stats_dateadd.php?" . $this->UrlParm($parm);
		else
			return "stats_dateadd.php";
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		return $this->KeyUrl("stats_dateedit.php", $this->UrlParm($parm));
	}
	...
	function GetCopyUrl($parm = "") {
		return $this->KeyUrl("stats_dateadd.php", $this->UrlParm($parm));
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("stats_datedelete.php", $this->UrlParm());
	}
