...
<?php
	...
	<?php echo $form->labelEx($model,'active'); ?>
		<?php echo $form->dropDownList($model, 'active', array(
			Apartment::STATUS_ACTIVE => tt('Active', 'apartments'),
			Apartment::STATUS_INACTIVE => tt('Inactive', 'apartments'),
...
<?php
	...
	Yii::app()->clientScript->registerScript('gmap-init', '
		...
		function findByAddress(){
			...
			var useSSL = "'.Yii::app()->request->getIsSecureConnection().'";

			var openStreetMapGeocoder = GeocoderJS.createGeocoder({provider: "openstreetmap", useSSL : useSSL});
			openStreetMapGeocoder.geocode(address, function(result) {				
				var newAddressLat = result[0].latitude;
				var newAddressLng = result[0].longitude;
				...
				setMarker(newAddressLat, newAddressLng);
			});
		}
