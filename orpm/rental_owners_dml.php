...
function rental_owners_insert(){
	...
	$data['date_of_birth'] = intval($_POST['date_of_birthYear']) . '-' . intval($_POST['date_of_birthMonth']) . '-' . intval($_POST['date_of_birthDay']);
	...
	if(function_exists('rental_owners_after_insert')){
		$res = sql("select * from `rental_owners` where `id`='" . makeSafe($recID) . "' limit 1", $eo);
		if($row = db_fetch_assoc($res)){
			$data = array_map('makeSafe', $row);
		}
		$data['selectedID'] = makeSafe($recID);
		$args=array();
		if(!rental_owners_after_insert($data, getMemberInfo(), $args)){ return (get_magic_quotes_gpc() ? stripslashes($recID) : $recID); }
	}

	// mm: save ownership data
	sql("insert into membership_userrecords set tableName='rental_owners', pkValue='$recID', memberID='".getLoggedMemberID()."', dateAdded='".time()."', dateUpdated='".time()."', groupID='".getLoggedGroupID()."'", $eo);
...
function rental_owners_form($selected_id = '', $AllowUpdate = 1, $AllowInsert = 1, $AllowDelete = 1, $ShowCancel = 0){
	...
	if(is_file(dirname(__FILE__).'/hooks/rental_owners.state.csv')){
		...
		$combo_state->ListItem = explode('||', entitiesToUTF8(convertLegacyOptions("Nairobi;;N/A")));
		$combo_state->ListData = $combo_state->ListItem;
	}
	...
	if($selected_id){
		...
		$templateCode=str_replace('<%%VALUE(date_of_birth)%%>', @date('m/d/Y', @strtotime(htmlspecialchars($row['date_of_birth'], ENT_QUOTES))), $templateCode);
		$templateCode=str_replace('<%%URLVALUE(date_of_birth)%%>', urlencode(@date('m/d/Y', @strtotime(htmlspecialchars($urow['date_of_birth'], ENT_QUOTES)))), $templateCode);
