				...
				<div class="panel-body">
					...
					<form  method="post" name="form" onsubmit="return validateForm()">
						...
						<fieldset>
							...
							<div class="form-group">
								<select name="who" id="category"  class="form-control" required>
...
_END;
					...
					$sql = "SELECT * FROM users WHERE username='$username' AND password='$encrypted' AND category='$level' AND status='active' ";
                ...
                <?php
					...
					else if($row['category'] ==1 AND ($row['pass_status']=='insecure')){
						session_start();
						$_SESSION['username'] = $username;
						$_SESSION['category'] = $category;
						?>
					...
					else if($row['category'] ==2 AND ($row['pass_status']=='insecure')){
						session_start();
						$_SESSION['username'] = $username;
						$_SESSION['category'] = $category;
						?>
					...
					else if($row['category'] ==4 AND ($row['pass_status']=='insecure')){
						session_start();
