...
if(isset($_POST["submit"])){
	...
	case "Save & add another":
		...
		$msg[0]="Sorry professional membership details not added";
		$msg[1]="Professional membership details successfull added";
		AddSuccess($results,$conn,$msg);
		break;
	case "Update & Continue":
		$sql="UPDATE professional SET id,applicantid=$applicantid,association=$association,title_role=$title_role,membersince=$membersince WHERE id=$_POST[id]";
		$results=query($sql,$conn);
		$msg[0]="Sorry professional membership details not updated";
		$msg[1]="Publication membership details successfull updated";
		AddSuccess($results,$conn,$msg);
		break;
	case "Delete":
		$sql = "DELETE FROM professional WHERE id=$id";
		$results=query($sql,$conn);
		$msg[0]="Sorry professional membership details not deleted";
	...
	case "Find":
		...
		break;
	case "Skip>>":
		echo "<meta http-equiv=\"Refresh\" content=\"0;url=./language.php\">";
		break;
	case "<<Back":
...
<title>tarclink - professional membership</title>
...
<table>
 ...
 <td colspan="2" valign="top" align="left"><table border="0" width="100%">
    <tr>
      <th colspan="3" bgcolor="#CCCCCC"><strong>PROFESSIONAL MEMBERSHIP </strong></th>
    </tr>
    <tr>
...
<?php $conn=db_connect(HOST,USER,PASS,DB,PORT);
	...
	echo "<tr class=\"boldtext\"><td>Association Name</td><td>Title/Role</td><td>Member Since</td><td>Action</td></tr>";
	while ($profmem = fetch_object($results)){
		...
		if($j%2==1){
			...
			echo "<td align=\"left\"><input name=\"id\" type=\"hidden\" value=\"$profmem->id\">$profmem->association</td>
				<td align=\"left\">$profmem->title_role</td>
				<td align=\"left\">$profmem->membersince</td>
				<td align=\"left\"><a name=\"editprofessional\" href=\"profmem.php?search=$profmem->id&action=Find\">Edit </a>|
				<a name=\"deleteprofessional\" href=\"profmem.php?search=$profmem->id&action=Delete\" onClick=\"Javascript:return confirm('Are you sure you want Delete this Record?','Confirm Delete')\">Delete</a></td>
			</tr>";
	}
	echo "</table>";
?></td>
