...
if (!isset($id_account)){
	...
	if ($show) $this->onload.="initDynamicOptionLists();"; $this->file_script="jscripts/dol.js";
	...
	if (!$form->process()){
		...
		$form->fields["data"]->value = $this->txt_encrypt("pg=verbookings&nm="._FIND_BY_CRITERIA."&show=0&from=0");
	} else {
...
if ((isset($fields) || $lnk_acc!="") && !$show){
	...
	if ($immo->search_booking($fields,$id_org_session,$from,10,$order_by)){				
		$founds = $immo->found_rows();		
		$adv = new adverts;
		$this->html_out .= $adv->print_advert($immo->select_array(), $from,10,"","pg=immo&id_immo=",null,"pg=verbookings&show=0&keywords=$keywords$lnk_acc", $founds);
