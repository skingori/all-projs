...
{ 
  ...
  if (this.getID()!=arbol.getID() && PERSERVESTATE && !this.isOpen) //closing
  {
...
{ 
  ...
  if (browserVersion == 1 || browserVersion == 3) { 
    ...
    // alert(str) et mostra el link de asobra
    if (str.slice(14,16) != "at") return //AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAaa
  }
  */
  if (!this.isRendered)
...
{ 
  ...
  if (folderObj.id!=arbol.id && PERSERVESTATE)
  {
    currentOpen = GetCookie("clickedFolder")
    if (currentOpen == null)
      currentOpen = ""
...
function titulo(description, hreference)
{ 
  folder = new Folder(description, hreference);
  return folder;
} 
 
function adoc(optionFlags, description, linkData)
{ 
  var fullLink = "";
  var targetFlag = "";
  var target = "";
  ...
  {
    return olddoc(optionFlags, description, linkData)
  }
...
function olddoc(target, description, linkData)
{
  var fullLink = "";
...
function nodo(parentFolder, childFolder)
{ 
  return parentFolder.addChild(childFolder) 
} 
 
function hoja(parentFolder, document)
{ 
    ...
    for (fldPos=0; fldPos<fldArr.length; fldPos++)
    {
      ...
      if (fldStr != "") {
        ...
        if (nodeObj!=null) //may have been deleted
          ...
          //else alert("Internal id is not pointing to a folder anymore. Consider using external IDs")
      }
    }
  }
}
...
function storeAllNodesInClickCookie(treeNodeObj)
{
  ...
  if (typeof treeNodeObj.setState != "undefined") //is folder
  {
    ...
    if (treeNodeObj.getID() != arbol.getID())
...
function CookieBranding(name) {
  if (typeof arbol.treeID != "undefined")
    return name+arbol.treeID //needed for multi-tree sites. make sure treeId does not contain cookieCutter
  else
    return name
}
...
{ 
  ...
  //arbol (with the site's data) is created in an external .js (demoFramesetNode.js, for example)
  arbol.initialize(0, true, "")
  if (supportsDeferral && !STARTALLOPEN)
          arbol.renderOb(null) //delay construction of nodes
  else {
    renderAllTree(arbol, null);

    if (PERSERVESTATE && STARTALLOPEN)
      storeAllNodesInClickCookie(arbol)
